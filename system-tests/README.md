...
1. A test written using the [`systemTests`](./lib/system-tests.ts) Mocha wrapper that lives in [`./test`](./test), and
...
There are many more options available for `systemTests.it` and `systemTests.setup`. You can massage the stdout, do pre-run tasks, set up HTTP/S servers, and more. Explore the typedocs in [`./lib/system-tests.ts`](./lib/system-tests.ts) for more information.
...
Every folder in [`./projects`](./projects) represents a self-contained Cypress project. When you pass the `project` property to `systemTests.it` or `systemTests.exec`, Cypress launches using this project.
...
The exception is some commonly used packages that are scaffolded for all projects, like `debug` and `bluebird`. You can see the list by looking at `scaffoldCommonNodeModules` in [`./lib/dep-installer/index.ts`](./lib/dep-installer/index.ts). These packages do not need to be added to a test project's `package.json`.
