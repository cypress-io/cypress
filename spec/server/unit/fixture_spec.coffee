...
describe "lib/fixture", ->
  ...
  context "image files", ->
    it "returns png as string", ->
      fixture.get(@fixturesFolder, "images/flower.png").then (base64) ->
    ...
    it "returns jpg as string", ->
      fixture.get(@fixturesFolder, "images/sample.jpg").then (base64) ->
    ...
    it "returns gif as string", ->
      fixture.get(@fixturesFolder, "images/word").then (base64) ->
    ...
    it "returns tif as string", ->
      fixture.get(@fixturesFolder, "images/sample.tif").then (base64) ->
  ...
  context ".scaffold", ->
    ...
    it "creates both fixturesFolder and example.json when fixturesFolder does not exist", ->
      ...
      fs.removeAsync(@todosPath).then =>
        fixture.scaffold(@fixturesFolder).then =>
          fs.readFileAsync(@fixturesFolder + "/example.json", "utf8").then (str) ->
            expect(str).to.eq """
            {
              "example": "fixture"
            }
    ...
    it "does not create example.json if fixturesFolder already exists", ->
      ...
      fs.ensureDirAsync(@fixturesFolder).then =>
        ## now scaffold
        fixture.scaffold(@fixturesFolder).then =>
          ## ensure example.json doesnt exist
          fs.statAsync(path.join(@fixturesFolder, "example.json"))
          .then ->
            throw new Error("should have failed but did not")
          .catch ->
