...
describe "lib/fixture", ->
  ...
  context "invalid extension", ->
    it "throws", ->
      fixture.get(@fixturesPath, "foo.exe")
      .then ->
        throw new Error("should have failed but did not")
      .catch (err) ->
  ...
  context "nested fixtures", ->
    it "can pass path to nested fixture", ->
      fixture.get(@fixturesPath, "nested/fixture.js").then (obj) ->
  ...
  context "json files", ->
    it "throws when json is invalid", ->
      ...
      fixture.get(@fixturesPath, "bad_json.json")
      .then ->
        throw new Error("should have failed but did not")
      .catch (err) ->
    ...
    it "reformats json and writes back even on parse error", ->
      ...
      .catch (err) =>
        ...
        fs.readFileAsync(@fixturesPath + "/bad_json.json", "utf8").then (str) ->
          expect(str).to.eq """
            {
              "bad": "json""should": "not parse"
            }
    ...
    it "reformats json and writes this back", ->
      fixture.get(@fixturesPath, "no_format.json").then (obj) =>
        fs.readFileAsync(@fixturesPath + "/no_format.json", "utf8").then (json) ->
          expect(json).to.eq """
            {
              "id": 1,
              "name": "brian"
    ...
    it "does not remove string whitespace", ->
      fixture.get(@fixturesPath, "words.json").then (obj) =>
        fs.readFileAsync(@fixturesPath + "/words.json", "utf8").then (json) ->
          expect(json).to.eq """
            {
    ...
    it "parses json to valid JS object", ->
      fixture.get(@fixturesPath, "users.json").then (users) ->
    ...
    it "reformats empty objects", ->
      fn = =>
        fixture.get(@fixturesPath, "empty_objects")

      Promise.map(Array(500), fn, {concurrency: 5}).then =>
        fs.readFileAsync(@fixturesPath + "/empty_objects.json", "utf8").then (str) ->
          expect(str).to.eq """
            {
              "empty": {
                "object": {\n      \n    },
  ...
  context "js files", ->
    it "returns valid JS object", ->
      fixture.get(@fixturesPath, "user.js").then (user) ->
    ...
    it "rewrites file as a formated valid JS object", ->
      fixture.get(@fixturesPath, "no_format.js").then (obj) =>
        fs.readFileAsync(@fixturesPath + "/no_format.js", "utf8").then (str) ->
          expect(str).to.eq """
            {
              foo: "bar",
              baz: "quux"
    ...
    it "throws on a bad JS object", ->
      ...
      fixture.get(@fixturesPath, "bad_js.js")
      .then ->
        throw new Error("should have failed but did not")
      .catch (err) =>
  ...
  context "coffee files", ->
    it "returns valid coffee object", ->
      fixture.get(@fixturesPath, "account.coffee").then (account) ->
    ...
    it "rewrites file as formatted valid coffee object", ->
      fixture.get(@fixturesPath, "no_format.coffee").then =>
        fs.readFileAsync(@fixturesPath + "/no_format.coffee", "utf8").then (str) ->
          expect(str).to.eq """
            [
              {
                id: 1
    ...
    it "throws on bad coffee object", ->
      ...
      fixture.get(@fixturesPath, "bad_coffee.coffee")
      .then ->
        throw new Error("should have failed but did not")
      .catch (err) ->
  ...
  context "html files", ->
    it "returns html as a string", ->
      fixture.get(@fixturesPath, "index.html").then (index) ->
        expect(index).to.eq """
          <!doctype html>
          <html>
          <head>
    ...
    it "rewrites file as formatted html", ->
      fixture.get(@fixturesPath, "index.html").then =>
        fs.readFileAsync(@fixturesPath + "/index.html", "utf8").then (str) ->
          expect(str).to.eq """
          <!doctype html>
          <html>
          <head>
  ...
  context "txt files", ->
    it "returns text as string", ->
      fixture.get(@fixturesPath, "message.txt").then (index) ->
        expect(index).to.eq "foobarbaz"

  context "image files", ->
    it "returns png as string", ->
      fixture.get(@fixturesPath, "images/flower.png").then (base64) ->
    ...
    it "returns jpg as string", ->
      fixture.get(@fixturesPath, "images/sample.jpg").then (base64) ->
    ...
    it "returns gif as string", ->
      fixture.get(@fixturesPath, "images/word").then (base64) ->
    ...
    it "returns tif as string", ->
      fixture.get(@fixturesPath, "images/sample.tif").then (base64) ->
  ...
  context "zip files", ->
    it "returns zip as base64 string", ->
      fixture.get(@fixturesPath, "example.zip").then (base64) ->
  ...
  context "extension omitted", ->
    it "#1 finds json", ->
      fixture.get(@fixturesPath, "foo").then (obj) ->
        expect(obj).to.deep.eq [
          {json: true}
        ]

    it "#2 finds js", ->
      fixture.get(@fixturesPath, "bar").then (obj) ->
        expect(obj).to.deep.eq {js: true}

    it "throws when no file by any extension can be found", ->
      fixture.get(@fixturesPath, "does-not-exist")
      .then ->
        throw new Error("should have failed but did not")
      .catch (err) =>
        p = @fixturesPath + "/does-not-exist"
  ...
  context "new lines", ->
    it "does not remove trailing new lines on .txt", ->
      fixture.get(@fixturesPath, "trailing_new_line.txt").then (str) =>
        fs.readFileAsync(@fixturesPath + "/trailing_new_line.txt", "utf8").then (str2) ->
          expect(str2).to.eq "foo\nbar\nbaz\n"

    it "does not remove trailing new lines on .json", ->
      fixture.get(@fixturesPath, "trailing_new_line.json").then (str) =>
        fs.readFileAsync(@fixturesPath + "/trailing_new_line.json", "utf8").then (str2) ->
          expect(str2).to.eq """
            {
              "foo": "bar"
            }\n
    ...
    it "does not remove trailing new lines on .js", ->
      fixture.get(@fixturesPath, "trailing_new_line.js").then (str) =>
        fs.readFileAsync(@fixturesPath + "/trailing_new_line.js", "utf8").then (str2) ->
          expect(str2).to.eq """
            {
              foo: "bar"
            }\n
    ...
    it "does not remove trailing new lines on .coffee", ->
      fixture.get(@fixturesPath, "trailing_new_line.coffee").then (str) =>
        fs.readFileAsync(@fixturesPath + "/trailing_new_line.coffee", "utf8").then (str2) ->
          expect(str2).to.eq """
            {
              foo: "bar"
            }\n
    ...
    it "does not remove trailing new lines on .html", ->
      fixture.get(@fixturesPath, "trailing_new_line.html").then (str) =>
        fs.readFileAsync(@fixturesPath + "/trailing_new_line.html", "utf8").then (str2) ->
          expect(str2).to.eq """
            <html>
            <body>foo</body>
            </html>\n
  ...
  context "#scaffold", ->
    ...
    it "does not create example.json if fixturesFolder already exists", ->
      ...
      fs.ensureDirAsync(@fixturesPath).then =>
        ## now scaffold
        fixture.scaffold(@fixturesPath).then =>
          ## ensure example.json doesnt exist
          fs.statAsync(path.join(@fixturesPath, "example.json"))
          .then ->
            throw new Error("should have failed but did not")
          .catch ->
