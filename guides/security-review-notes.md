...
plugin loads as review context.

This is a reviewer's map of the invariants this codebase has to keep, written
for humans and for agents asked to review a change. It states what must remain
true, not an assessment of what is true today, and it does not replace
`/security-review`, Snyk, or human PR reviews.
...
it drives. "This disables a security control" is often correct here, so a review
cannot work from that observation alone. Two questions do the work instead.

**Does the change widen something Cypress already does on purpose?** Each of
those behaviours has a scope — a set of responses it rewrites, a channel it
listens on, a window it applies to. Rewriting responses for the application
under test is the product working; doing the same to the Cypress UI's own origin
is not. The next section lists the deliberate behaviours so a review flags the
widening rather than the behaviour.

**Does it give something a new path to the user's machine, credentials or
connection?** The filesystem, the local endpoints, stored secrets and the
install path are what a user cannot inspect, audit or undo after the fact, so
these carry the most weight. A change that lets a page influence the Cypress UI
matters and belongs in review; a change that lets anything reach the host
matters more.

[`SECURITY.md`](../SECURITY.md) sets out what users are responsible for when
testing — running against applications they control, keeping secrets out of
project files, using a disposable browser profile. The rules here are the other
half of that bargain: what the code owes a user who holds up their end, and what
keeps the cost survivable for one who doesn't.
...
passthrough rather than forwarded upstream.
`x-cypress-internal-loopback-token` gates `proxiedUrl` override and force-proxy
bypass, so **every new passthrough path must strip it** — it is a
process-local secret and must not leave the machine. The guard comments in
`adapters/internal-routes.ts` and `adapters/serve-internal-routes.ts` explain why
the presence of a header alone must never be sufficient: any page in the browser
can set one, so the header proves nothing about where a request came from.
...
The child process runs the user's config and plugin code. The user chose to run
that code, but it still must not reach further than Cypress itself: never `eval`
what crosses back, never interpolate it into a shell string or path, and confine
any path it supplies to the project root before watching or reading it. Validate
config fields consumed as a path, an executable, or a route *after* the IPC hop,
not before.

Keep `fork`/`spawn` on argv arrays for this path. `NODE_OPTIONS` is assembled by
hand from `ORIGINAL_NODE_OPTIONS`, so treat that variable as project input and
keep loader flags out of it. A Node binary path taken from resolved app state should
...
## Page content reaching privileged UI

Users are asked to run Cypress only against applications they control, so
nothing here is the first line of defense. Keep it anyway: someone will point
Cypress at something they don't control, and when they do the cost should stop
short of their machine.

`reifyDomElement` in `driver/src/util/serialization/log.ts` assigns `innerHTML`
from a `postMessage`d payload that originated in a page frame and reifies its
attributes. Widening it with more tag names, event-handler attributes,
`<script>`, `<iframe>`, `srcdoc` or `javascript:` URLs is script execution in
the runner's own origin. The attribute allowlist is the control.
...
`packages/driver/cypress/fixtures/**` reproduce patterns real applications use —
framebusting and `window.top` checks, `<base target>` documents, login forms with
placeholder credentials — and `packages/server/test/**` disables TLS
verification on purpose. Do not report those as vulnerabilities.
