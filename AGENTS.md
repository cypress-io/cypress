...
- **Sync FS calls** — `no-restricted-syntax: 'error'` (except `existsSync`); use the async variants. Where sync I/O is genuinely correct, justify it inline with `eslint-disable-next-line no-restricted-syntax -- <reason>`.
