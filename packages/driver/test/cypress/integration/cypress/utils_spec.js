...
describe("driver/src/cypress/utils", function() {
  context(".reduceProps", () => it("reduces obj to only include props in props", function() {
    let obj = {
      foo: 'foo',
      bar: 'bar',
      baz: 'baz'
  ...
  context(".filterOutOptions", function() {
    it("returns new obj based on the delta from the filter", function() {
    ...
    it("returns undefined if nothing is different", function() {
      const obj = $utils.filterOutOptions({foo: "foo", bar: "bar"}, {foo: "foo"});
      return expect(obj).to.be.undefined;
    });

    return it("normalizes objects with length property", function() {
  ...
  context(".stringify", function() {
    beforeEach(function() {
      return this.str = str => $utils.stringify(str);
    });
    ...
    context("Functions", () => it("function(){}", function() {
      const o = function(foo, bar, baz) {};
      return expect(this.str(o)).to.eq("function(){}");
    }));

    return context("Elements", () => it("stringifyElement", () => cy.visit("/fixtures/dom.html").then(function() {
      const o = Cypress.$("#dom");
      return expect(this.str(o)).to.eq("<div#dom>");
    })));
  });

  return context(".memoize", function() {
    it("runs the function the first time", function() {
    ...
    it("runs the function for unique first arguments", function() {
      const fn = cy.stub().returns("output");
      const memoizedFn = $utils.memoize(fn);
    ...
    it("returns cached return value if first argument is the same", function() {
      const fn = cy.stub().returns("output");
      const memoizedFn = $utils.memoize(fn);
    ...
    return it("accepts a cache instance to use as the second argument", function() {
      const fn = cy.stub().returns("output");
