...
describe('src/cy/commands/actions/select', () => {
  ...
  context('#select', () => {
    it('does not change the subject', () => {
      ...
      return cy.get('select[name=maps]').select('train').then(($select) => {
        expect($select).to.match(select)
      })
    })

    it('selects by value', () => {
      return cy.get('select[name=maps]').select('de_train').then(($select) => {
    ...
    it('selects by trimmed text with newlines stripped', () => {
      return cy.get('select[name=maps]').select('italy').then(($select) => {
        expect($select).to.have.value('cs_italy')
      })
    })

    it('prioritizes value over text', () => {
      return cy.get('select[name=foods]').select('Ramen').then(($select) => {
        expect($select).to.have.value('Ramen')
      })
    })

    it('can select an array of values', () => {
      return cy.get('select[name=movies]').select(['apoc', 'br']).then(($select) => {
        expect($select.val()).to.deep.eq(['apoc', 'br'])
      })
    })

    it('can handle options nested in optgroups', () => {
      return cy.get('select[name=starwars]').select('Jar Jar').then(($select) => {
    ...
    it('can select an array of texts', () => {
      return cy.get('select[name=movies]').select(['The Human Condition', 'There Will Be Blood']).then(($select) => {
    ...
    it('can select a readonly select', () => {
      return cy.get('select[name=hunter]').select('gon').then(($select) => {
    ...
    it('lists the select as the focused element', () => {
      ...
      return cy.get('#select-maps').select('de_train').focused().then(($focused) => {
    ...
    describe('events', () => {
      ...
      it('emits all events in the correct order', () => {
        ...
        _.each(events, (event) => {
          return cy.$$('select[name=maps]').one(event, () => {
            return fired.push(event)
          })
        })

        return cy.get('select[name=maps]').select('train').then(() => {
