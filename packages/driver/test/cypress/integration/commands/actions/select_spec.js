...
describe("src/cy/commands/actions/select", function() {
  ...
  return context("#select", function() {
    ...
    it("can handle options with same value selected by text", () => cy.get("select[name=startrek-same]").select("Uhura").then(function($select) {
    ...
    it("can handle options with some same values selected by text", () => cy.get("select[name=startrek-some-same]").select("Uhura").then(function($select) {
      expect($select.val()).to.equal("same");
      expect($select.find("option:selected")).to.have.text("Uhura");
      expect($select[0].selectedIndex).to.equal(2);
      return expect($select[0].selectedOptions[0]).to.eql($select.find("option:selected")[0]);
    ...
    it("clears previous values when providing an array", function() {
    ...
    it("lists the select as the focused element", function() {
    ...
    it("causes previous input to receive blur", function(done) {
    ...
    it("can forcibly click even when element is invisible", function(done) {
    ...
    it("can forcibly click when select is disabled", function() {
        cy.get("select[name=disabled]")
          //# default select value
        .invoke("val").should("eq", "foo");

        return cy.get("select[name=disabled]")
        .select("bar", { force: true })
        .invoke("val").should("eq", "bar");
    });

    it("retries until <option> can be selected", function() {
    ...
    it("retries until <select> is no longer disabled", function() {
    ...
    it("retries until <optgroup> is no longer disabled", function() {
      const select = cy.$$("select[name=optgroup-disabled]");

      cy.on("command:retry", _.once(() => {
        return select.find("optgroup").prop("disabled", false);
      })
      );

      return cy.get("select[name=optgroup-disabled]").select("bar")
        .invoke("val").should("eq", "bar");
    });

    it("retries until <options> are no longer disabled", function() {
    ...
    describe("events", function() {
      ...
      it("emits focus event", function(done) {
        cy.$$("select[name=maps]").one("focus", () => done());

        return cy.get("select[name=maps]").select("train");
      });

      it("emits input event", function(done) {
        cy.$$("select[name=maps]").one("input", () => done());

        return cy.get("select[name=maps]").select("train");
      });

      return it("emits all events in the correct order", function() {
        const fired = [];
    ...
    describe("errors", function() {
      ...
      it("throws when subject is not in the document", function(done) {
        ...
        var $select = cy.$$("#select-maps").change(function(e) {
          selected += 1;
      ...
      it("throws when more than 1 element in the collection", function(done) {
      ...
      it("throws on anything other than a select", function(done) {
        cy.on("fail", function(err) {
      ...
      it("throws when finding duplicate values", function(done) {
        cy.on("fail", function(err) {
      ...
      it("throws when passing an array to a non multiple select", function(done) {
        cy.on("fail", function(err) {
      ...
      it("throws when value or text does not exist", function(done) {
        cy.on("fail", function(err) {
      ...
      it("throws when the <select> itself is disabled", function(done) {
        cy.on("fail", function(err) {
      ...
      it("throws when optgroup is disabled", function(done) {
        cy.on("fail", function(err) {
      ...
      it("throws when options are disabled", function(done) {
        cy.on("fail", function(err) {
    ...
    return describe(".log", function() {
      ...
      it("logs out select", () => cy.get("#select-maps").select("de_dust2").then(function() {
        const {
          lastLog
        } = this;
      ...
      it("passes in $el", () => cy.get("#select-maps").select("de_dust2").then(function($select) {
        const {
          lastLog
        } = this;
      ...
      it("snapshots after clicking", () => cy.get("#select-maps").select("de_dust2").then(function($select) {
        const {
          lastLog
        } = this;
      ...
      it("ends", () => cy.get("#select-maps").select("de_dust2").then(function() {
        const {
          lastLog
        } = this;
      ...
      it("#consoleProps", () => cy.get("#select-maps").select("de_dust2").then(function($select) {
      ...
      return it("logs deltaOptions", () => cy.get("#select-maps").select("de_dust2", {force: true, timeout: 1000}).then(function() {
        const {
          lastLog
        } = this;
