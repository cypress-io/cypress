        ...
        return it("provides specific #onFail", function(done) {
          cy.on("fail", err => {
            ...
            _.each(obj, (value, key) => {
              return expect(value).deep.eq(lastLog.get(key), `expected key: ${key} to eq value: ${value}`);
            });
...
`
  ...
  return context("options immutability", function() {
    it("does not mutate options for cy.server()", function() {
      const options = { enable: false };

      return cy
      .server(options)
      .window().then(() => expect(options).to.deep.eq({ enable: false }));
    });

    return it("does not mutate options for cy.route()", function() {
      const options = {
        url: /foo/,
        respond: false
      };
      ...
      .server()
      .route(options)
      .window().then(() => expect(options).to.deep.eq({
        url: /foo/,
        respond: false
      }));
    });
