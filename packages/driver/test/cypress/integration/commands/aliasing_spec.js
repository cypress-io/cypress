...
describe('src/cy/commands/aliasing', () => {
  ...
  context('#as', () => {
    ...
    it('does not change the subject', () => {
      ...
      cy.get('body').as('b').then(($body) => expect($body.get(0)).to.eq(body.get(0)))
    })
    ...
    it('stores the resulting subject as the alias', () => {
      ...
      cy.get('body').as('b').then(() => expect(cy.state('aliases').b.subject.get(0)).to.eq($body.get(0)))
    })

    it('stores subject of chained aliases', () => {
      const li = cy.$$('#list li').eq(0)

      cy.get('#list li').eq(0).as('firstLi').then(($li) => expect($li).to.match(li))
    })
    ...
    it('recognizes dot and non dot with same alias names', () => {
      ...
      cy.contains('foo').as('body.foo').then(() => {
        ...
        cy.get('@body.foo').then((bodyFoo) => cy.get('@body').should('not.equal', bodyFoo))
      })
    })
    ...
    context('DOM subjects', () => {
      it('assigns the remote jquery instance', () => {
        ...
        cy.get('input:first').as('input').then(function ($input) {
    ...
    context('#assign', () => {
      ...
      describe('nested hooks', () => {
        ...
        it('assigns bar', () => cy.noop('bar').as('bar'))
      })
  ...
  context('#replayCommandsFrom', () => {
    ...
    describe('subject not in document', () => {
      ...
      it('resets the chainerId allow subjects to be carried on', () => {
        ...
        // chainer id's
        cy.get('@button').then(($button) => expect($button).to.have.id('button'))
      })
