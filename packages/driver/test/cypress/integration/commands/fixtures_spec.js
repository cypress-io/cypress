...
describe("src/cy/commands/fixtures", function() {
  ...
  return context("#fixture", function() {
    ...
    it("triggers 'fixture' on Cypress", function() {
      Cypress.backend.withArgs("get:fixture").resolves({foo: "bar"});

      return cy.fixture("foo").as("f").then(function(obj) {
    ...
    it("can have encoding as second argument", function() {
      Cypress.backend.withArgs("get:fixture").resolves({foo: "bar"});

      return cy.fixture("foo", "ascii").then(function(obj) {
        expect(obj).to.deep.eq({foo: "bar"});
    ...
    it("can have encoding as second argument and options as third argument", function() {
      Cypress.backend.withArgs("get:fixture").resolves({foo: "bar"});

      return cy.fixture("foo", "ascii", {timeout: 1000}).then(function(obj) {
        expect(obj).to.deep.eq({foo: "bar"});
    ...
    it("looks for csv without extension", () => cy.fixture("comma-separated").should("equal", `\
One,Two,Three
1,2,3
\
`
    ...
    it("handles files with unknown extensions, reading them as utf-8", () => cy.fixture("yaml.yaml").should("equal", `\
- foo
- bar
- 
\
`
    ...
    describe("timeout", function() {
      ...
      return it("clears the current timeout and restores after success", function() {
        ...
        return cy.fixture("foo").then(function() {
    ...
    return describe("caching", function() {
      beforeEach(() => Cypress.backend
      .withArgs("get:fixture", "foo")
      .resolves({foo: "bar"})
      .withArgs("get:fixture", "bar")
      .resolves({bar: "baz"}));

      it("caches fixtures by name", () => cy.fixture("foo").then(obj => {
        expect(obj).to.deep.eq({foo: "bar"});
      ...
      return it("clones fixtures to prevent accidental mutation", () => cy.fixture("foo").then(function(obj) {
        //# mutate the object
        obj.baz = "quux";

        return cy.fixture("foo").then(function(obj2) {
          obj2.lorem = "ipsum";
          expect(obj2).not.to.have.property("baz");

          return cy.fixture("foo").then(obj3 => expect(obj3).not.to.have.property("lorem"));}).then(() => expect(Cypress.backend).to.be.calledOnce);
      }));
    });
  });
});
