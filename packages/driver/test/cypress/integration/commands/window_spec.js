...
describe("src/cy/commands/window", function() {
  context("#window", function() {
    ...
    describe("assertion verification", function() {
      ...
      it("eventually passes the assertion", function() {
        cy.on("command:retry", _.after(2, () => {
          return this.remoteWindow.foo = "bar";
        })
        );
      ...
      it("eventually fails the assertion", function(done) {
        cy.on("command:retry", _.after(2, () => {
          return this.remoteWindow.foo = "foo";
        })
        );
    ...
    return describe(".log", function() {
      ...
      it("can turn off logging", () => cy.window({log: false}).then(function() {
      ...
      it("snapshots after resolving", () => cy.window().then(function() {
        const {
          lastLog
        } = this;
      ...
      it("logs obj", () => cy.window().then(function() {
        const obj = {
          name: "window",
          message: ""
        };
      ...
      return it("#consoleProps", () => cy.window().then(function(win) {
  ...
  context("#document", function() {
    ...
    describe("assertion verification", function() {
      ...
      it("eventually passes the assertion", function() {
        cy.on("command:retry", _.after(2, () => {
          return this.remoteDocument.foo = "bar";
        })
        );
      ...
      it("eventually fails the assertion", function(done) {
        cy.on("command:retry", _.after(2, () => {
          return this.remoteDocument.foo = "foo";
        })
        );
    ...
    return describe(".log", function() {
      ...
      it("can turn off logging", () => cy.document({log: false}).then(function() {
        return expect(this.log).to.be.undefined;
      }));
      ...
      it("snapshots after resolving", () => cy.document().then(function() {
        const {
          lastLog
        } = this;
      ...
      it("logs obj", () => cy.document().then(function() {
        const obj = {
          name: "document",
          message: ""
        };
      ...
      return it("#consoleProps", () => cy.document().then(function(win) {
        return expect(this.lastLog.invoke("consoleProps")).to.deep.eq({
          Command: "document",
          Yielded: win
        });}));
  ...
  context("#title", function() {
    ...
    it("returns the pages title as a string", function() {
      const title = cy.$$("title").text();
      return cy.title().then(text => expect(text).to.eq(title));
    });

    it("retries finding the title", function() {
      cy.$$("title").remove();
    ...
    it("uses the first title element", function() {
    ...
    it("uses document.title setter over <title>", function() {
    ...
    it("is empty string when no <title>", function() {
      cy.$$("title").remove();
    ...
    return describe(".log", function() {
      ...
      it("can turn off logging", () => cy.title({log: false}).then(function() {
        return expect(this.log).to.be.undefined;
      }));
      ...
      it("snapshots after clicking", () => cy.title().then(function() {
        const {
          lastLog
        } = this;
      ...
      it("logs obj", () => cy.title().then(function() {
        const obj = {
          name: "title"
        };
      ...
      return it("#consoleProps", () => cy.title().then(function() {
        return expect(this.lastLog.invoke("consoleProps")).to.deep.eq({
          Command: "title",
  ...
  return context("#viewport", function() {
    it("triggers 'viewport:changed' event with dimensions object", function() {
      let expected = false;

      cy.on("viewport:changed", function(viewport, fn) {
        expected = true;
    ...
    it("does not trigger 'viewport:changed' when changing to the default", function() {
      const fn = function() {
    ...
    it("does not trigger 'viewport:changed' when changing to the same viewport", function() {
    ...
    it("triggers 'viewport:changed' if width changes", function(done) {
      let finished = false;
      setTimeout(function() {
        if (!finished) {
      ...
      , 1000);
      let triggeredOnce = false;
      cy.on("viewport:changed", function(viewport) {
        if (triggeredOnce) {
    ...
    it("triggers 'viewport:changed' if height changes", function(done) {
      let finished = false;
      setTimeout(function() {
        if (!finished) {
          return done("Timed out before 'viewport:changed'");
    ...
    it("does not modify viewportWidth and viewportHeight in config", function() {
      let expected = false;
      cy.on("viewport:changed", function() {
        expected = true;
    ...
    context("changing viewport", () => it("changes viewport and then resets back to the original", function() {
      const { viewportHeight, viewportWidth } = Cypress.config();

      return cy.viewport(500, 400).then(() => Cypress.action("runner:test:before:run:async", {})
      .then(function() {
    ...
    context("presets", function() {
      ...
      it("can change the orientation to landspace", function(done) {
        cy.on("viewport:changed", function(viewport) {
      ...
      it("can change the orientation to portrait", function(done) {
        cy.on("viewport:changed", function(viewport) {
          expect(viewport).to.deep.eq({viewportWidth: 320, viewportHeight: 568});
          return done();
        });
      ...
      return it("samsung-note9", function(done) {
        cy.on("viewport:changed", function(viewport) {
    ...
    return context(".log", function() {
      ...
      it("logs viewport", () => cy.viewport(800, 600).then(function() {
        const {
          lastLog
        } = this;
      ...
      it("logs viewport with width, height", () => cy.viewport(800, 600).then(function() {
        const {
          lastLog
        } = this;
      ...
      it("logs viewport with preset", () => cy.viewport("ipad-2").then(function() {
        const {
          lastLog
        } = this;
      ...
      it("sets state to success immediately", () => cy.viewport(800, 600).then(function() {
        const {
          lastLog
        } = this;
      ...
      it("snapshots immediately", () => cy.viewport(800, 600).then(function() {
        const {
          lastLog
        } = this;
      ...
      it("can turn off logging viewport command", () => cy.viewport(800, 600, {log: false}).then(function() {
        return expect(this.log).not.to.be.ok;
      }));

      it("can turn off logging viewport when using preset", () => cy.viewport("macbook-15", {log: false}).then(function() {
        return expect(this.log).not.to.be.ok;
      }));

      it("sets viewportWidth and viewportHeight directly", () => cy.viewport(800, 600).then(function() {
        const {
          lastLog
        } = this;
      ...
      it(".consoleProps with preset", () => cy.viewport("ipad-mini").then(function() {
        return expect(this.lastLog.invoke("consoleProps")).to.deep.eq({
          Command: "viewport",
          Preset: "ipad-mini",
          Width: 768,
      ...
      return it(".consoleProps without preset", () => cy.viewport(1024, 768).then(function() {
        return expect(this.lastLog.invoke("consoleProps")).to.deep.eq({
          Command: "viewport",
          Width: 1024,
          Height: 768
