...
describe('src/cy/commands/net_stubbing', function () {
  ...
  context('#route', function () {
    ...
    //       cy.spy($errUtils, 'warning')

    //       cy.route('GET', '/foo%25bar').then(function () {
    //         expect($errUtils.warning).to.be.calledWith('A URL with percent-encoded characters was passed to cy.route(), but cy.route() expects a decoded URL.\n\nDid you mean to pass "/foo%bar"?')
    //       })
    //     })
    ...
    //         expect($errUtils.warning).to.not.be.called
    //       })
    //     })
