  ...
  describe('with jquery', function () {
    ...
    context('#spread', () => {
      it('spreads an array into individual arguments', () => {
        return cy.noop([1, 2, 3]).spread((one, two, three) => {
          expect(one).to.eq(1)
          expect(two).to.eq(2)
      ...
      it('spreads a jQuery wrapper into individual arguments', () => {
        return cy.noop($('div')).spread((first, second) => {
      ...
      return describe('errors', () => {
        ...
        it('throws when subject isn\'t array-like', (done) => {
          ...
          return cy.noop({}).spread(() => {})
        })

        return it('throws when promise timeout', (done) => {
          const logs = []
          ...
          return cy.noop([1, 2, 3]).spread({ timeout: 20 }, () => new Promise((resolve, reject) => {}))
        })
      })
    })
    ...
    context('#then', function () {
      ...
      describe('errors', function () {
        ...
        it('throws when promise timeout', function (done) {
          ...
          return cy.then({ timeout: 150 }, () => new Promise((resolve, reject) => {}))
        })
      ...
      return describe('yields to remote jQuery subject', function () {
        ...
        return it('does not store the remote jquery object as the subject', () => {
          return cy
          .get('div:first').then(function ($div) {
            expect($div).to.be.instanceof(this.remoteWindow.$)
    ...
    context('#invoke', function () {
      ...
      describe('function property', function () {
        ...
        [null, undefined].forEach((val) => {
          return it(`changes subject to '${val}' without throwing default assertion existence`, () => {
            const obj = {
              foo () {
                return val
              },
        ...
        return describe('errors', function () {
          ...
          return it('throws when reduced prop is not a function', (done) => {
            const obj = {
              foo: {
                bar: 'bar',
              },
      ...
      describe('.log', function () {
        ...
        it('logs $el if subject is element', () => {
          return cy.get('div:first').invoke('hide').then(function ($el) {
            const {
              lastLog,
            } = this
        ...
        return it('#consoleProps as a function on DOM element', () => {
          return cy.get('div:first').invoke('hide').then(function ($btn) {
    ...
    return context('#its', function () {
      ...
      describe('.log', function () {
        ...
        it('snapshots after invoking', () => {
          return cy.noop({ foo: 'foo' }).its('foo').then(function () {
            const {
              lastLog,
            } = this
      ...
      return describe('errors', function () {
        ...
        [null, undefined].forEach((val) => {
          ...
          return it(`throws on initial '${val}' subject`, (done) => {
            cy.on('fail', (err) => {
  ...
  return describe('without jquery', function () {
    ...
    return context('#each', function () {
      ...
      it('passes the existing subject downstream without side effects', () => {
        ...
        return cy.get('#list li').each(($li, i, arr) => {
          expect(i).to.eq(count)
          count += 1
          expect(arr.length).to.eq(3)

          return cy.wrap($li).should('have.class', 'item')
        }).then(($lis) => {
      ...
      it('supports array like structures', () => {
        ...
        return cy.wrap(arr).each((num, i, a) => {
          expect(a).to.eq(arr)
          expect(i).to.eq(count)
          count += 1

          return expect(num).to.eq(count)
      ...
      it('works with nested eaches', () => {
        ...
        return cy.get('#list li').each(($li, i, arr) => {
          return cy.wrap($li).parent().siblings('#asdf').find('li').each(($li2, i2, arr2) => count += 1).then(($lis) => {
      ...
      return describe('errors', function () {
        ...
        it('can time out', function (done) {
          ...
          return cy.get('ul').each(($ul) => new Promise((resolve) => {}))
        })
