...
const _insertSubstring = (curText, newText, [start, end]) => curText.substring(0, start) + newText + curText.substring(end);

var _getHostContenteditable = function(el) {
  let curEl = el;
...
const _collapseInputOrTextArea = (el, toIndex) => setSelectionRange(el, toIndex, toIndex);
...
const moveCursorUp = el => _moveCursorUpOrDown(el, true);

const moveCursorDown = el => _moveCursorUpOrDown(el, false);

var _moveCursorUpOrDown = function(el, up) {
  ...
  //# which is to increment the step/interval
    if ($elements.isType(el, "number")) {
      if (up) { if (typeof el.stepUp === 'function') {
        el.stepUp();
      } } else { if (typeof el.stepDown === 'function') {
        el.stepDown();
      } }
    }
    return;
...
var getSelectionBounds = function(el) {
...
const moveSelectionToEnd = () => _moveSelectionTo(false);

const moveSelectionToStart = () => _moveSelectionTo(true);

var _moveSelectionTo = function(toStart) {
...
var replaceSelectionContents = function(el, key) {
  if ($elements.isContentEditable(el)) {
