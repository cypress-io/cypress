...
const focusable =
  "body,a[href],link[href],button,select,[tabindex],input,textarea,[contenteditable]";
...
declare global {
  ...
  interface Window extends WindowClasses {
    SVGElement: typeof SVGElement;
    EventTarget: typeof EventTarget;
  }
}

interface PropertyDescriptorWithVal<V> {
  get?(): V;
  set?(v: V): void;
}

const descriptor = <
  T extends keyof WindowClasses,
  P extends keyof WindowClasses[T]["prototype"]
>(
  klass: T,
  prop: P
) => {
  return Object.getOwnPropertyDescriptor(
    ...
    prop
  ) as PropertyDescriptorWithVal<WindowClasses[T]["prototype"][P]>;
};

const _getValue = function(this: HTMLInputElement) {
  if (isInput(this)) {
...
const _setValue = function(this: HTMLInputElement) {
  if (isInput(this)) {
...
const _getSelectionStart = function(
  ...
  throw new Error(
    "getSelectionStart should only be called on HTMLInputElement | HTMLTextAreaElement"
  );
};
...
const _getSelectionEnd = function(
  ...
  throw new Error(
    "getSelectionEnd should only be called on HTMLInputElement | HTMLTextAreaElement"
  );
};

const _nativeFocus = function(this: HTMLOrSVGElement | Window) {
...
const _nativeBlur = function(this: HTMLOrSVGElement | Window) {
  if ($window.isWindow(this)) {
    return window.blur;
  }
...
const _nativeSelect = function(this: HTMLInputElement | HTMLTextAreaElement) {
  if (isInput(this)) {
...
const _isContentEditable = function(this: HTMLElement) {
  if (isSvg(this)) {
    return false;
  }
...
const _setType = function(this: HTMLInputElement | HTMLButtonElement) {
  if (isInput(this)) {
...
const _getType = function(this: HTMLInputElement | HTMLButtonElement) {
  if (isInput(this)) {
...
const tryCallNativeMethod: typeof callNativeMethod = (...args) => {
  try {
...
const callNativeMethod = function(
  obj: HTMLElement,
  fn: keyof typeof nativeMethods,
  ...args: any[]
) {
  ...
  let retFn = (nativeFn as any).apply(obj, args);
