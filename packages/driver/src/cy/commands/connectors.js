...
module.exports = function (Commands, Cypress, cy, state, config) {
  ...
  const thenFn = function (subject, options, fn) {
    ...
    const enqueuedCommand = () => invokedCyCommand = true
    ...
    if (next && (next.get('chainerId') === current.get('chainerId'))) {
      const checkSubject = function (newSubject, args) {
  ...
  const invokeBaseFn = function (options, subject, str, ...args) {
    // name could be invoke or its!
    let resolveValue
    const name = state('current').get('name')
    ...
    const traverseObjectAtPath = function (acc, pathsArray, index = 0) {
