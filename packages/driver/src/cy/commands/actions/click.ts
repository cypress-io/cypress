    ...
    const perform = (el, index) => {
      ...
      if (options.log) {
        ...
        if (options.x !== null && options.x !== undefined) {
          clickOptions.x = options.x
        }

        if (options.y !== null && options.y !== undefined) {
          clickOptions.y = options.y
        }
        ...
        if (Object.keys(clickOptions).length > 0) {
          ...
          if (clickOptions.position) {
            message += clickOptions.position

            if (clickOptions.x || clickOptions.y) {
              message += ', '
            }
          }

          if (clickOptions.x) {
            message += `${clickOptions.x }, `
          }

          if (clickOptions.y) {
            message += clickOptions.y
          }

          if (message.length > 0) {
            message += Object.keys(_.omit(clickOptions, ['position', 'x', 'y'])).length ? `, ${JSON.stringify(_.omit(clickOptions, ['position', 'x', 'y']))}` : ''
          } else {
            message = `${JSON.stringify(clickOptions)}`
          }
