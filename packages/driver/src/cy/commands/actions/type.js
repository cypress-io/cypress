...
module.exports = function (Commands, Cypress, cy, state, config) {
  ...
  const type = (subject, chars, options = {}) => {
    let updateTable
    ...
    // validateTyping(options.$el, chars, options._log)
    ...
    const type = function () {
      ...
      const isSimulated = (el) => {
        return $elements.isNeedSingleValueChangeInputElement(el) || options.force
      }
      ...
      return keyboard.type({
        ...
        // onBeforeKey: (el, chars, options) => {
        //   const nextIndex = validateTyping($dom.wrap(el), chars, options._log)
        // },
    ...
    const handleFocused = function () {
      ...
      return $actionability.verify(cy, options.$el, options, {
        ...
        onReady ($elToClick) {
          ...
          let needsForceFocusEl = cy.needsForceFocus()

          if (needsForceFocusEl) {
            cy.fireFocus(needsForceFocusEl)
          }
  ...
  return Commands.addAll({ prevSubject: 'element' }, { type, clear })
}
