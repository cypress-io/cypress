...
module.exports = function(Commands, Cypress, cy, state, config) {
  Cypress.on("test:before:run", () => $Keyboard.resetModifiers(state("document"), state("window")));
  ...
  return Commands.addAll({ prevSubject: "element" }, {
    type(subject, chars, options = {}) {
      ...
      if (options.log) {
        ...
        const getRow = (id, key, which) =>
          table[id] || (function() {
            ...
            if (modifiers.length) { obj.modifiers = modifiers.join(", "); }
            if (key) {
              obj.typed = key;
              if (which) { obj.which = which; }
            }
            return obj;
          })()
        ;
        ...
        const getTableData = () =>
          ...
          _.reduce(_.values(table), function(memo, value, index) {
            memo[index + 1] = value;
            return memo;
          }
          , {})
      ...
      const getDefaultButtons = form =>
        form.find("input, button").filter(function(__, el) {
      ...
      const type = function() {
        const simulateSubmitHandler = function() {
          ...
          const multipleInputsAllowImplicitSubmissionAndNoSubmitElements = function(form) {
            ...
            const implicitSubmissionInputs = form.find("input").filter(function(__, input) {
          ...
          if (multipleInputsAllowImplicitSubmissionAndNoSubmitElements(form)) { return; }
          ...
          const getDefaultButton = form => getDefaultButtons(form).first();

          const defaultButtonisDisabled = button => button.prop("disabled");

          const defaultButton = getDefaultButton(form);

          //# bail if the default button is in a 'disabled' state
          if (defaultButtonisDisabled(defaultButton)) { return; }
        ...
        const needSingleValueChange = () => $elements.isNeedSingleValueChangeInputElement(options.$el.get(0));
        ...
        return $Keyboard.type({
          ...
          onEvent(id, key, column, which, value) {
            if (updateTable) { return updateTable.apply(window, arguments); }
          },
          ...
          onValueChange(originalText, el) {
            ...
            return state("changeEvent", function(id, readOnly) {
