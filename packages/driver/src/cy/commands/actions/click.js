  ...
  return Commands.addAll(
    { prevSubject: 'element' },
    {
      click (subject, positionOrX, y, options = {}) {
        ...
        ;({ options, position, x, y } = $actionability.getPositionFromArguments(
          positionOrX,
          y,
          options
        ))
        ...
        const click = (el, index) => {
          ...
          const afterMouseDown = function ($elToClick, coords) {
            ...
            const consoleProps = function () {
              const consoleObj = _.defaults(
                consoleObj != null ? consoleObj : {},
                {
          ...
          .verify(cy, $el, options, {
            ...
            onReady ($elToClick, coords) {
              ...
              const moveCursor = () => {
                // if the clicked element is an input/textarea/contenteditable, we move the cursor to the end to
                // mimic what most users would do if clicking an input

                // TODO: think about changing this to NOT move cursor if coords are specified
                // TODO: maybe remove this for contenteditables, since content could span multiple pages
                // TODO: maybe remove this for textareas, since content can also span muliple pages (Github's editor)
                if (
                  $elements.isInput(el) ||
              ...
              domEvents.mouseDown = $Mouse.mouseDown(
                $elToClick,
                coords.fromViewport
              )
              ...
              //# send a focus event
              if (
                domEvents.mouseDown.preventedDefault ||
                  !$dom.isAttached($elToClick)
              ) {
                return afterMouseDown($elToClick, coords)
              }
        ...
        return Promise.each(options.$el.toArray(), click).then(() => {
      ...
      dblclick (subject, options = {}) {
        ...
        const dblclick = (el, index) => {
          ...
          const p = cy
          .now('focus', $el, { $el, error: false, verify: false, log: false })
          .then(() => {
