...

{
  ...
  // or object literal with all expanded options
    request (...args) {
      let a; let f; let h; let o; let originOrBase; let path
      let options = (o = {})
      ...
      originOrBase = config('baseUrl') || cy.getRemoteLocation('origin')
      ...
      if (options.log) {
        options._log = Cypress.log({
          ...
          renderProps () {
            ...
            if (indicator == null) {
              indicator = (options.response != null ? options.response.isOkStatusCode : undefined) ? 'successful' : 'bad'
            }
      ...
      .then((response) => {
        ...
        return $errUtils.throwErrByPath('request.loading_failed', {
          onFail: options._log,
          args: {
            error: err.message,
            stack: err.stack,
