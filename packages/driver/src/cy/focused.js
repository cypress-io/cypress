...
const create = function(state) {
  ...
  var simulateBlurEvent = function(el, win) {
  ...
  var simulateFocusEvent = function(el, win) {
    //# todo handle relatedTarget's per the spec
  ...
  const needsFocus = function($elToFocus, $previouslyFocusedEl) {
    ...
    if ($previouslyFocusedEl.get(0) !== $focused.get(0)) { return true; }
