...
chai.use(function(chai, u) {
  ...
  setFormatValueHook(function(ctx, val) {
  ...
  const overrideChaiInspect = () => chai.util.inspect = inspect;

  const overrideChaiObjDisplay = () =>
    chai.util.objDisplay = function(obj) {
      const str = chai.util.inspect(obj);
      const type = Object.prototype.toString.call(obj);
      if (chai.config.truncateThreshold && (str.length >= chai.config.truncateThreshold)) {
        if (type === '[object Function]') {
          if (!obj.name || (obj.name === '')) { return '[Function]'; } else { return `[Function: ${obj.name}]`; }
  ...
  var createPatchedAssert = assertFn =>
    function(...args) {
      let err;
