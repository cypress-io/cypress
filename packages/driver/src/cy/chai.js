...
const $ChaiFactory = require('../cypress/chai_factory');
...
const chaiUtils = $ChaiFactory.utils;

const { getMessage } = chaiUtils;

const removeOrKeepSingleQuotesBetweenStars = message => //# remove any single quotes between our **, preserving escaped quotes
//# and if an empty string, put the quotes back
//# fix empty strings that end up as ****
message.replace(allBetweenFourStars, match => match
...
const replaceArgMessages = (args, str) => _.reduce(args, (memo, value, index) => {
  if (_.isString(value)) {
    value = value
...
const overrideChaiAsserts = function(chai, assertFn) {
  let fn1, fn2;
  ...
  Assertion.overwriteMethod("match", _super => (function(regExp) {
  ...
  const containFn1 = _super => (function(text) {
    let obj = this._obj;
  ...
  const containFn2 = _super => (function() {
    return _super.apply(this, arguments);
  });
  ...
  Assertion.overwriteChainableMethod("length",
    ...
    if ($dom.isDetached(obj)) {
      obj = (this._obj = obj.filter((index, el) => $dom.isAttached(el)));
    }
  ...
  Assertion.overwriteProperty("exist", _super => (function() {
    const obj = this._obj;
  ...
  return Assertion.prototype.assert = overrideAssertionAssert(Assertion, assertFn);
};

const overrideExpect = chai => //# only override assertions for this specific
//# expect function instance so we do not affect
//# the outside world
(val, message) => //# make the assertion
new chai.Assertion(val, message);

const overrideAssert = function(chai) {
  const fn = (express, errmsg) => chai.assert(express, errmsg);

  const fns = _.functions(chai.assert);

  _.each(fns, name => fn[name] = function() {
...
var overrideAssertionAssert = function(Assertion, assertFn) {
  const assertProto = Assertion.prototype.assert;

  return Assertion.prototype.assert = function(...args) {
    let err;
...
const useChaiJquery = chai => $chaiJquery(chai, chai.util, {
...
const create = function(specWindow, assertFn) {
  ...
  const expect = overrideExpect(chai);
  const assert = overrideAssert(chai);
