...
interface GlobalState {
  (key: "mouseCoords"): MouseCoords | undefined;
  (key: "mouseCoords", value: MouseCoords): void;
  (key: "mouseLastHoveredEl"): Element | undefined;
  (key: "mouseLastHoveredEl", el: Element): void;
}
...
const getMouseCoords = (state: GlobalState) => {
  return state("mouseCoords");
};

const create = (state: GlobalState, focused) => {
  const mouse = {
    _getDefaultMouseOptions(x, y, win) {
      const _activeModifiers = $Keyboard.getActiveModifiers(state);
      const modifiersEventOptions = $Keyboard.toModifiersEventOptions(
        _activeModifiers
      );
    ...
    mouseMove({ x, y }, force = false) {
      ...
      if (
        _.isEqual(coords, getMouseCoords(state)) &&
    ...
    _mouseMoveEvents(el, coords) {
      ...
      if (hoveredElChanged && lastHoveredEl) {
        ...
        pointerout = () => {
          sendPointerout(
            lastHoveredEl,
            _.extend({}, defaultPointerOptions, { relatedTarget: el })
          );
        };
        mouseout = () => {
          sendMouseout(
            lastHoveredEl,
            _.extend({}, defaultMouseOptions, { relatedTarget: el })
          );
        };
      ...
      if (hoveredElChanged) {
        if (el && $elements.isAttachedEl(el)) {
          ...
          while (
            curParent &&
            curParent.ownerDocument &&
            curParent !== commonAncestor
          ) {
    ...
    _mouseDownEvents(
      ...
      let pointerdownProps = sendPointerdown(el, pointerEvtOptions);
    ...
    mouseDown(
      ...
      const mouseDownEvents = mouse._mouseDownEvents(
        coords,
        force,
        pointerEvtOptionsExtend,
        mouseEvtOptionsExtend
      ...
      if (
        mouseDownEvents.pointerdownProps.preventedDefault ||
        mouseDownEvents.mousedownProps.preventedDefault ||
      ...
      if (
        $elements.isInput(el) ||
        $elements.isTextarea(el) ||
        $elements.isContentEditable(el)
      ) {
    ...
    mouseClick(
      ...
      const mouseDownEvents = mouse.mouseDown(
        fromViewport,
        forceEl,
        pointerEvtOptionsExtend,
        mouseEvtOptionsExtend
      ...
      const skipMouseupEvent =
        mouseDownEvents.pointerdownProps.skipped ||
        mouseDownEvents.pointerdownProps.preventedDefault;

      const mouseUpEvents = mouse.mouseUp(
        fromViewport,
        forceEl,
        skipMouseupEvent,
        pointerEvtOptionsExtend,
      ...
      const skipClickEvent = $elements.isDetachedEl(
    ...
    _mouseUpEvents(
      ...
      let defaultOptions = mouse._getDefaultMouseOptions(
        fromViewport.x,
        fromViewport.y,
        win
      );
    ...
    _mouseClickEvents(
      ...
      const defaultOptions = mouse._getDefaultMouseOptions(
        fromViewport.x,
        fromViewport.y,
        win
      );
    ...
    dblclick(fromViewport, force = false, mouseEvtOptionsExtend = {}) {
      const click = clickNum => {
        const clickEvents = mouse.mouseClick(
          fromViewport,
          force,
          {},
          { detail: clickNum }
      ...
      const dblclickEvtProps = _.extend(
        mouse._getDefaultMouseOptions(fromViewport.x, fromViewport.y, win),
        {
          buttons: 0,
          detail: 2
        },
...
const sendEvent = (
  ...
  const evt = new constructor(
    evtName,
    _.extend({}, evtOptions, { bubbles, cancelable })
  );
