...
const getPositionFromArguments = function(positionOrX, y, options) {
  ...
  switch (false) {
    ...
    case !_.every([positionOrX, y], _.isFinite):
      position = null;
      x = positionOrX;
      break;
...
const ensureElIsNotCovered = function(cy, win, $el, fromElViewport, options, log, onScroll) {
  ...
  const ensureDescendentsAndScroll = function() {
    try {
      ...
      var getAllScrollables = function(scrollables, $el) {
        //# nudge algorithm
      ...
      var scrollContainers = function(scrollables) {
        ...
        var possiblyScrollMultipleTimes = function($fixed) {
...
var getCoordinatesForEl = function(cy, $el, options) {
  ...
  return Promise.try(function() {
    ...
    const runAllChecks = function() {
      ...
      if (force !== true) {
        ...
        if (options.ensure.notDisabled) { cy.ensureNotDisabled($el, _log); }
        ...
        //# ensure its visible
        if (options.ensure.visibility) { cy.ensureVisibility($el, _log); }
