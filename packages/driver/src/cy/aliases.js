...
const getXhrTypeByAlias = function(alias) {
  if (requestXhrRe.test(alias)) { return "request"; } else { return "response"; }
};
...
const create = function(state) {
  const addAlias = function(ctx, aliasObj) {
    ...
    const aliases = (left = state("aliases")) != null ? left : {};
    ...
    return ctx[alias] = remoteSubject != null ? remoteSubject : subject;
  };
  ...
  const getAvailableAliases = function() {
    let aliases;
    if (!(aliases = state("aliases"))) { return []; }

    return _.keys(aliases);
  };

  var aliasNotFoundFor = function(name, cmd, log) {
    let displayName;
    ...
    if (cmd == null) { cmd = (log && log.get("name")) || state("current").get("name"); }
    displayName = aliasDisplayName(name);
