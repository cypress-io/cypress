...
chai.use((chai, u) => {
  ...
  objDisplay = clone(chai.util.objDisplay)
  getMessage = chai.util.getMessage

  _inspect = chai.util.inspect

  const createdChaiInspect = chaiInspect.create(chai)

  inspect = clone(createdChaiInspect.inspect)

  const { setFormatValueHook } = createdChaiInspect

  setFormatValueHookCb = (ctx, val) => {
  ...
  setFormatValueHook(setFormatValueHookCb)
})

// remove any single quotes between our **,
// except escaped quotes, empty strings and number strings.
export const removeOrKeepSingleQuotesBetweenStars = (message) => {
...
export const escapeMarkdown = (message) => {
  return message.replace(imageMarkdown, '``$&``')
}

export const replaceArgMessages = (args, str) => {
...
export const restoreAsserts = function () {
...
export const overrideChaiInspect = () => {
  return chai.util.inspect = inspect
}

export const overrideChaiObjDisplay = () => {
...
export const overrideChaiAsserts = function (specWindow, state, assertFn) {
...
export const captureUserInvocationStack = (specWindow, state, ssfi) => {
...
export const createPatchedAssert = (specWindow, state, assertFn) => {
...
export const overrideExpect = (specWindow, state) => {
...
export const overrideAssert = function (specWindow, state) {
...
export const setSpecWindowGlobals = function (specWindow, state) {
...
export const create = function (specWindow, state, assertFn) {
  restoreAsserts()
