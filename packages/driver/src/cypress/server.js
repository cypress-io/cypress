...
//# on chrome < 71
const isAbortedThroughUnload = xhr => (xhr.canceled !== true) &&
  (xhr.readyState === 4) &&
    (xhr.status === 0) &&
...
xhr.method === "GET" && regularResourcesRe.test(xhr.url);
...
const create = function(options = {}) {
  ...
  return server = {
    ...
    getRouteForXhr(xhr) {
      ...
      if (!routes.length) { return nope(); }

      //# bail if this xhr matches our whitelist
      if (server.isWhitelisted(xhr)) { return nope(); }
    ...
    urlsMatch(routePattern, fullyQualifiedUrl) {
      ...
      const testStr = (url1, url2) => (routePattern === url1) ||
        (routePattern === url2) ||
        ...
        props.forEach(function(prop) {
          ...
          return Object.defineProperty(xhr, prop, {
            get() {
              ...
              if (_.isFunction(bak)) {
                return function() { return bak.apply(xhr, arguments); };
              } else {
