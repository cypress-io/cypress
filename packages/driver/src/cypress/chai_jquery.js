...
const wrap = ctx => //# reset the obj under test
//# to be re-wrapped in our own
//# jquery, so we can control
//# the methods on it
$(ctx._obj);

const $chaiJquery = function(chai, chaiUtils, callbacks = {}) {
  const { inspect, flag } = chaiUtils;

  const assertDom = function(ctx, method, ...args) {
  ...
  const assert = function(ctx, method, bool, ...args) {
  ...
  const assertPartial = function(ctx, method, actual, expected, message, notMessage, ...args) {
  ...
  chai.Assertion.addMethod("data", function() {
    assertDom(this, "data");
  ...
  chai.Assertion.overwriteProperty("empty", _super => (function() {
  ...
  chai.Assertion.overwriteMethod("match", _super => (function(selector) {
    if ($dom.isDom(this._obj)) {
      return assert(
        this,
        "match",
  ...
  _.each(selectors, (selectorName, selector) => chai.Assertion.addProperty(selector, function() {
    return assert(
      this,
      selector,
      wrap(this).is(":" + selector),
  ...
  return _.each(attrs, (description, attr) => chai.Assertion.addMethod(attr, function(name, val) {
    assertDom(
      this,
      attr,
