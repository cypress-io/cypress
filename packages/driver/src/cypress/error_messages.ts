...
export default {
  ...
  config: {
    cypress_config_api: {
      ...
      suite_or_test_only: {
        message: `\`Cypress.config()\` cannot override \`{{invalidConfigKey}}\` during test execution because it would affect the next test rather than the current one and is not reflected in Test Replay. Set \`{{invalidConfigKey}}\` in the test configuration of a \`describe\`/\`context\` or \`it\` block, or use \`cy.viewport()\` instead.`,
        docsUrl: 'https://on.cypress.io/config',
      },
    },
