...
export default {
  ...
  navigation: {
    cross_origin ({ message, superDomainOriginPolicy, configFile, projectRoot }) {
      return {
        message: stripIndent`\
    ...
    cannot_visit_previous_origin (args) {
      return {
        message: stripIndent`\
          ...
          \`cy.origin('${args.previousUrl.superDomainOriginPolicy}', () => {\`
    ...
    invalid_cross_origin_on_load (args) {
      return {
        message: stripIndent`${cmd('visit')} was called to visit a cross origin site with an \`onLoad\` callback. \`onLoad\` callbacks can only be used with same origin sites.
          ...
          \`cy.origin('${args.autLocation.superDomainOriginPolicy}', () => {\`
          \`  cy.on('window:load', () => {\`
    ...
    cannot_visit_different_origin (args) {
      return {
        message: stripIndent`\
          ...
          \`cy.origin('${args.attemptedUrl.superDomainOriginPolicy}', () => {\`
          \`  cy.visit('${args.originalUrl}')\`
          \`  <commands targeting ${args.attemptedUrl.origin} go here>\`
          \`})\`
