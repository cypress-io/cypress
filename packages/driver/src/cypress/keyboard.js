...
const $Keyboard = {
  ...
  specialCharsNative: {
    ...
    '{del}' () {
      return $native.keypress(this.getKeyInfo('Delete'))
    },
    '{backspace}' () {
      return $native.keypress(this.getKeyInfo('Backspace'))
    },
    '{esc}' () {
      return $native.keypress(this.getKeyInfo('Escape'))
    },
    '{{}' () {
      return $native.keypress(this.getKeyInfo('{'))
    },
    '{enter}' () {
      return $native.keypress(this.getKeyInfo('Enter'))
    },
    '{leftarrow}' () {
      return $native.keypress(this.getKeyInfo('ArrowLeft'))
    },
    '{rightarrow}' () {
      return $native.keypress(this.getKeyInfo('ArrowRight'))
    },
    '{uparrow}' () {
      return $native.keypress(this.getKeyInfo('ArrowUp'))
    },
    '{downarrow}' () {
      return $native.keypress(this.getKeyInfo('ArrowDown'))
    },
  },
  ...
  type (options = {}) {
    ...
    return Promise.each(keys, (key, i) => {
      ...
      let charsNeedingType = options.onAfterType(
        $elements.getActiveElement(el.ownerDocument)
      )
    ...
    switch (false) {
      case !this.isSpecialChar(chars):
        return Promise.resolve(
          this.handleSpecialChars(el, chars, options)
        ).delay(options.delay)

      case !this.isModifier(chars):
        return Promise.resolve(this.handleModifier(el, chars, options)).delay(
          options.delay
        )
      ...
      case !charsBetweenCurlyBracesRe.test(chars):
        ...
        //# char or modifier
        const allChars = _.keys(this.specialCharsNative)
        .concat(_.keys(this.modifierChars))
        .join(', ')
      ...
      default:
        ...
        if (simulated) {
          return Promise.each(charsArr, (char) => {
            return Promise.resolve(this.typeKey(el, char, options)).delay(delay)
          })
        }

        if (delay) {
          return Promise.each(charsArr, (char) => {
            return $native.keypress(this.getKeyInfo(char)).delay(delay)
          })
        }

        return $native.keypressAll(this.getKeyInfo(charsArr))
    }
  },
  ...
  simulateKey (el, eventType, key, options) {
    ...
    switch (eventType) {
      ...
      case 'keyup':
        charCodeAt =
          options.charCode != null
            ? options.charCode
            : this.getCharCode(key.toUpperCase())
      ...
      case 'keypress':
        charCodeAt =
          options.charCode != null ? options.charCode : this.getCharCode(key)
  ...
  typeKey (el, key, options) {
    return this.ensureKey(el, key, options, () => {
      ...
      const isNumberInputType =
        $elements.isInput(el) && $elements.isType(el, 'number')
  ...
  getKeyInfo (keyString) {
    ...
    keyInfo.modifiers = this.getModifiersValue()
  ...
  resetModifiers (el, window) {
    return (() => {
      ...
      for (let modifier in this.modifiers) {
        ...
        if (isActivated) {
          result.push(
            this.simulateModifier(el, 'keyup', modifier, {
              window,
              onBeforeEvent () {},
              onEvent () {},
            })
