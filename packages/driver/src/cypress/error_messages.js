...
const divider = (num, char) => Array(num).join(char);
...
const formatConfigFile = function(configFile) {
  if (configFile === false) {
...
const formatRedirect = redirect => `  - ${redirect}`;

const formatRedirects = (redirects = []) => _.map(redirects, formatRedirect);

const formatProp = function(memo, field) {
...
const cmd = function(command, args = "") {
  const prefix = command.startsWith("Cypress") ? "" : "cy.";

  return `\`${prefix}${command}(${args})\``;
};

const getScreenshotDocsPath = function(cmd) {
  if (cmd === "Cypress.Screenshot.defaults") { return "screenshot-api"; } else { return "screenshot"; }
};

const getRedirects = function(obj, phrase) {
  const redirects = obj.redirects != null ? obj.redirects : [];

  if (!redirects.length) { return ""; }

  const word = redirects.length > 1 ? "times" : "time";

  const list = formatRedirects(redirects);
...
`,
    ...
    pointer_events_none(obj) { return {
      message: `\
...
`,
  ...
  miscellaneous: {
    custom_command_interface_changed(obj) { return {
      message: `\
...
`,
    ...
    returned_value_and_commands_from_custom_command(obj) { return {
      message: `\
...
`,
    ...
    returned_value_and_commands(obj) { return {
      message: `\
...
`,
    ...
    command_returned_promise_and_commands(obj) { return {
      message: `\
...
`,
    ...
    mixing_promises_and_commands(obj) { return {
      message: `\
...
`,
  ...
  navigation: {
    cross_origin({ message, originPolicy, configFile }) { return {
      message: `\
...
`,
    ...
    timed_out({ ms, configFile }) { return `\
...
`,
  ...
  route: {
    ...
    url_percentencoding_warning({ decodedUrl }) { return {
      message: `\
...
`,
    ...
    error(obj) {
      const {msg, source, lineno} = obj;

      return msg + (source && lineno ? ` (${source}:${lineno})` : "");
    },
