...
export class PrimaryOriginCommunicator extends EventEmitter {
  ...
  onMessage ({ data, source }) {
    ...
    if (data?.event?.startsWith(CROSS_ORIGIN_PREFIX)) {
      ...
      if (messageName === 'bridge:ready' && source) {
        this.crossOriginDriverWindows[data.superDomainOriginPolicy] = source as Window
      }
      ...
      this.emit(messageName, data.data, data.superDomainOriginPolicy, source)
  ...
  toSpecBridge (superDomainOriginPolicy: string, event: string, data?: any) {
    debug('=> to spec bridge', superDomainOriginPolicy, event, data)
    ...
    // If there is no crossOriginDriverWindows, there is no need to send the message.
    this.crossOriginDriverWindows[superDomainOriginPolicy]?.postMessage({
      event,
      data: preprocessedData,
    }, '*')
  }
   ...
   */
  toSpecBridgePromise<T> (superDomainOriginPolicy: string, event: string, data?: any) {
    return new Promise<T>((resolve, reject) => {
      const dataToSend = sharedPromiseSetup({
        ...
        data,
        event,
        specBridgeName: superDomainOriginPolicy,
        communicator: this,
      })

      this.toSpecBridge(superDomainOriginPolicy, event, dataToSend)
    })
  }
}
...
export class SpecBridgeCommunicator extends EventEmitter {
  ...
  toPrimary (event: string, data?: Cypress.ObjectLike, options: { syncGlobals: boolean } = { syncGlobals: false }) {
    const { superDomainOriginPolicy } = $Location.create(window.location.href)
    ...
    debug('<= to Primary ', event, data, superDomainOriginPolicy)
