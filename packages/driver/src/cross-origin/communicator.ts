...
export class PrimaryOriginCommunicator extends EventEmitter {
  ...
  onMessage ({ data, source }) {
    ...
    if (data?.event?.startsWith(CROSS_ORIGIN_PREFIX)) {
      ...
      this.emit(messageName, data.data, data.superDomainOrigin, source)
  ...
  toSpecBridge (superDomainOrigin: string, event: string, data?: any) {
    ...
    // If there is no crossOriginDriverWindows, there is no need to send the message.
    this.crossOriginDriverWindows[superDomainOrigin]?.postMessage({
      event,
      data: preprocessedData,
    }, '*')
  }
   ...
   */
  toSpecBridgePromise<T> (superDomainOrigin: string, event: string, data?: any) {
    return new Promise<T>((resolve, reject) => {
      ...
      this.toSpecBridge(superDomainOrigin, event, dataToSend)
    })
  }
}
