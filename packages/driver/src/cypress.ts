...
class $Cypress {
  ...
  stop () {
    ...
    // [LEAK-FIX cypress-services#13184] Unwrap cross-realm references on the
    // AUT contentWindow before we drop it. overrides.ts patches prototypes
    // (HTMLElement.focus/blur, SVGElement.focus/blur, HTMLInputElement.select,
    // document.hasFocus, CSSStyleSheet.insertRule/deleteRule) with closures
    // that capture `focused` and `snapshots` from the runner's $Cy instance.
    // listeners.ts also patches alert/confirm. These closures form cross-realm
    // edges AUT Window ↔ runner realm, which Blink does not collect until the
    // JS edges are cut — leaving the Window pinned even after its <iframe> is
    // detached and GC'd. Restoring natives via delete breaks the cycle.
    try {
      const autWindow = this.$autIframe?.[0]?.contentWindow

      if (autWindow) {
        // Deleting own properties on prototypes restores the native inherited
        // implementation (HTMLElement.prototype.focus falls back to the
        // Element/HTMLOrSVGElement native).
        try {
          delete autWindow.HTMLElement?.prototype?.focus
        } catch {}
        try {
          delete autWindow.HTMLElement?.prototype?.blur
        } catch {}
        try {
          delete autWindow.SVGElement?.prototype?.focus
        } catch {}
        try {
          delete autWindow.SVGElement?.prototype?.blur
        } catch {}
        try {
          delete autWindow.HTMLInputElement?.prototype?.select
        } catch {}
        try {
          delete autWindow.CSSStyleSheet?.prototype?.insertRule
        } catch {}
        try {
          delete autWindow.CSSStyleSheet?.prototype?.deleteRule
        } catch {}
        try {
          delete autWindow.document?.hasFocus
        } catch {}
        // alert/confirm are set directly on the window (not prototype).
        try {
          delete autWindow.alert
        } catch {}
        try {
          delete autWindow.confirm
        } catch {}
        // WebKit patched autWindow.Error — restoring native here avoids the
        // closure over `originalError` holding onto the AUT realm.
        try {
          delete autWindow.Error
        } catch {}
      }
    } catch { /* noop */ }

    // [LEAK-FIX candidate for cypress-services#13184]
    // Null the jQuery wrappers around the AUT iframe elements. Without this,
    // on rerun the detached <iframe> elements (and therefore their
    // contentWindow and the entire loaded AUT heap) stay reachable via
    // Cypress.$autIframe, because the $Cypress instance itself is retained
    // by $Cy / ProxyLogging / SessionManager / Window for the duration of
    // the Cypress app's lifetime. Confirmed by heap-snapshot retainer chain:
    //   Detached <iframe class="aut-iframe"> ← [0] in jQuery.fn.init
    //     ← $autIframe in $Cypress ← Window.
    this.$autIframe = null
    this.$autSnapshotIframe = null
    this.$autSnapshotIframes = null
    this.$autIframesContainer = null
    ...
    // cy.state also retains the iframe via state('$autIframe') (set in cy.ts
    // initialize()) and state('window'/'document') (set by setWindowDocumentProps).
    // cy.reset() intentionally preserves these across per-test resets, so they
    // survive past stop() unless we clear them here. Heap-snapshot retainer:
    //   Detached <iframe> ← [0] in jQuery.fn.init ← $autIframe in {state bag}
    //     ← context in SetterGetter() ← state in $Cypress.
    try {
      this.cy?.state?.('$autIframe', null)
      this.cy?.state?.('window', null)
      this.cy?.state?.('document', null)
      // [LEAK-FIX cypress-services#13184] $Cy keeps specWindow (constructor +
      // state('specWindow')) and assigns specWindow.cy = this. After the spec
      // iframe is torn down on rerun, that edge runner ↔ detached spec Window
      // keeps the entire spec realm alive (Detached HTMLDocument, onerror
      // accessors, etc.) until we drop it here — same pattern as window/document.
      const specWin = this.cy?.specWindow

      console.log('clearing specWindow')

      if (specWin) {
        try {
          delete specWin.cy
        } catch { /* cross-origin or non-configurable */ }
      }

      if (this.cy) {
        this.cy.specWindow = null
      }

      this.cy?.state?.('specWindow', null)
    } catch { /* state may not be initialized; noop */ }
