...
for (const experimentalFastVisibility of [true, false]) {
  describe(`Virtual Scrolling Stress Tests, experimentalFastVisibility: ${experimentalFastVisibility}`, {
    experimentalFastVisibility, // if this is set to false, this spec will crash the browser
  }, () => {
    const counts = [
      100,
      1000,
    ...
    for (const count of counts) {
      describe(`basic list (${count} count)`, () => {
        ...
        it(`should load the virtual list (${count} count) without crashing`, () => {
      ...
      describe('dynamic list', () => {
        beforeEach(() => {
          cy.get('input[data-cy="item-count"]').clear().type(i)
          cy.get('input[data-cy="list-id"]').clear().type('dynamic-list')
          cy.get('select[data-cy="list-type"]').select('dynamic')
          cy.get('button[data-cy="add-list"]').click()
        })
      })

      describe('extreme list', () => {
        beforeEach(() => {
          cy.get('input[data-cy="item-count"]').clear().type(i)
          cy.get('input[data-cy="list-id"]').clear().type('extreme-list')
          cy.get('select[data-cy="list-type"]').select('extreme')
          cy.get('button[data-cy="add-list"]').click()
        })
      })
      ...
      describe('multiple lists', () => {
        beforeEach(() => {
          cy.get('input[data-cy="item-count"]').clear().type(count)
          cy.get('input[data-cy="list-id"]').clear().type('multiple-list')
          cy.get('select[data-cy="list-type"]').select('multiple')
          cy.get('button[data-cy="add-list"]').click()
        })
      })
      */
    ...
    // NOTE: we don't even need the more complex tests, the basic tests are enough to cause the crash
