...
describe('src/cypress/location', () => {
  ...
  context('#getAuthObj', () => {
    it('returns an object with username and password', () => {
      const obj = setup('auth').getAuthObj()
  ...
  context('#getHostName', () => {
    it('returns host without port', () => {
      const str = setup('signin').getHostName()
  ...
  context('#getPathName', () => {
    it('returns the path', () => {
      const str = setup('signin').getPathName()

      expect(str).to.eq('/signin')
    })

    it('returns a / with no path', () => {
      const str = setup('google').getPathName()

      expect(str).to.eq('/')
    })

    it('returns the full pathname without a host', () => {
      const str = setup('pathname').getPathName()
  ...
  context('#getProtocol', () => {
    it('returns the http protocol', () => {
      const str = setup('signin').getProtocol()

      expect(str).to.eq('http:')
    })

    it('returns the https protocol', () => {
      const str = setup('google').getProtocol()
  ...
  context('#getSearch', () => {
    it('returns the search params with ? prepended', () => {
      const str = setup('search').getSearch()

      expect(str).to.eq('?q=books')
    })

    it('returns an empty string with no search params', () => {
      const str = setup('google').getSearch()
  ...
  context('#getToString', () => {
    it('returns the toString function', () => {
      const str = setup('signin').getToString()
  ...
  context('#getOrigin', () => {
    it('handles ip addresses', () => {
      const str = setup('local').getOrigin()

      expect(str).to.eq('http://127.0.0.1:8080')
    })

    it('handles 1 part localhost', () => {
      const str = setup('users').getOrigin()

      expect(str).to.eq('http://localhost:2020')
    })

    it('handles 2 parts stack', () => {
      const str = setup('stack').getOrigin()
  ...
  context('#getSuperDomainOrigin', () => {
    it('handles ip addresses', () => {
      const str = setup('local').getSuperDomainOrigin()

      expect(str).to.eq('http://127.0.0.1:8080')
    })

    it('handles 1 part localhost', () => {
      const str = setup('users').getSuperDomainOrigin()

      expect(str).to.eq('http://localhost:2020')
    })

    it('handles 2 parts stack', () => {
      const str = setup('stack').getSuperDomainOrigin()

      expect(str).to.eq('https://stackoverflow.com')
    })

    it('handles subdomains google', () => {
      const str = setup('google').getSuperDomainOrigin()
    ...
    // https://github.com/cypress-io/cypress/issues/255
    it('handles a second domain inside the query string', () => {
      const str = setup('email').getSuperDomainOrigin()

      expect(str).to.eq('http://localhost:3500')
    })

    it('handles private tlds in the public suffix', () => {
      const str = setup('heroku').getSuperDomainOrigin()

      expect(str).to.eq('https://example.herokuapp.com')
    })

    it('handles subdomains of private tlds in the public suffix', () => {
      const str = setup('herokuSub').getSuperDomainOrigin()

      expect(str).to.eq('https://example.herokuapp.com')
    })

    it('falls back to dumb check when invalid tld', () => {
      const str = setup('unknown').getSuperDomainOrigin()
