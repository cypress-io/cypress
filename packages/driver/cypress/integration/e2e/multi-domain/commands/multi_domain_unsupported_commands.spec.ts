...
context('multi-domain unsupported commands', { experimentalSessionSupport: true, experimentalMultiDomain: true }, () => {
  ...
  it('throws an error that the `cy.server()` method is deprecated and forbidden when attempted use is in multi-domain', (done) => {
    cy.on('fail', (err) => {
      expect(err.message).to.equal('`cy.server()` has been deprecated and use is forbidden in `cy.switchToDomain()`. Consider migrating to using `cy.intercept()` instead.')
      done()
    })
