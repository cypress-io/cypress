...
describe('src/cy/commands/querying - shadow dom', () => {
  ...
  context('#get', () => {
    ...
    it.skip('finds elements within shadow roots with cross-boundary selector', () => {
      cy.get('#parent-of-shadow-container-0 .shadow-3', { includeShadowDom: true }).should(
        'have.text',
        'Shadow Content 3'
      )
    })
  ...
  context('#contains', () => {
    ...
    it('finds element within shadow dom based on text content, scoped to selector', () => {
      cy.contains('.shadow-11-scope', 'Find Me With cy.contains', { includeShadowDom: true }).should('have.length', 1)
    })
