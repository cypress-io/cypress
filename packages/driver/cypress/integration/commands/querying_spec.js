...
describe('src/cy/commands/querying', () => {
        ...
        it('fails waiting for the focused element not to exist', (done) => {
          ...
          cy.on('fail', (err) => {
            expect(err.message).to.include(
              'Expected <input#input> not to exist in the DOM, but it was continuously found.'
            )
  ...
  context(
    ...
    () => {
      ...
      it('does not factor in the total time the test has been running', () => {
        ...
        cy.wait(defaultCommandTimeout + 100)
      ...
      describe('custom elements', () => {
        ...
        it('can alias a custom element', () => {
          cy.get('foobarbazquux:last').as('foo').get('div:first').get('@foo').should('contain', 'custom element')
        })

        it('can find a custom alias again when detached from DOM', () => {
          cy.get('foobarbazquux:last')
          .as('foo')
          .then(() => {
      ...
      describe("should('have.length', n)", () => {
        it('resolves once length equals n', () => {
          ...
          cy.get('form')
          .should('have.length', forms.length)
          .then(($forms) => {
      ...
      describe('assertion verification', () => {
        ...
        it('eventually resolves an alias', () => {
          ...
          cy.get('button:first').as('btn').get('@btn').should('have.class', 'foo-bar-baz')
        })
      })
        ...
        () => {
          ...
          describe('route aliases', () => {
            ...
            it('returns an array of xhrs', () => {
              ...
              .window()
              .then({ timeout: 2000 }, (win) => {
                return Promise.all([win.$.get('/users', { num: 1 }), win.$.get('/users', { num: 2 })])
              })
              .get('@getUsers.all')
              .then((xhrs) => {
      ...
      describe(
        ...
        () => {
          ...
          it('throws on too many elements after timing out waiting for length', (done) => {
            ...
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                `Too many elements found. Found '${buttons.length}', expected '${buttons.length - 1}'.`
              )
          ...
          it('throws on too few elements after timing out waiting for length', (done) => {
            ...
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                `Not enough elements found. Found '${buttons.length}', expected '${buttons.length + 1}'.`
              )
          ...
          it('throws after timing out while not trying to find an element', (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                'Expected <div#dom> not to exist in the DOM, but it was continuously found.'
              )
          ...
          it('throws after timing out trying to find a visible element', (done) => {
            ...
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                'This element `<button#button>` is not visible because it has CSS property: `display: none`'
              )
          ...
          it('throws when alias property is `0`', (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                '`0` is not a valid alias property. Are you trying to ask for the first response? If so write `@getUsers.1`'
              )

              done()
            })

            cy.server().route(/users/, {}).as('getUsers').get('@getUsers.0')
          })

          it('throws when alias property isnt just a digit', (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                '`1b` is not a valid alias property. Only `numbers` or `all` is permitted.'
              )

              done()
            })

            cy.server().route(/users/, {}).as('getUsers').get('@getUsers.1b')
          })

          it('throws when alias property isnt a digit or `all`', (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                '`all ` is not a valid alias property. Only `numbers` or `all` is permitted.'
              )

              done()
            })

            cy.server().route(/users/, {}).as('getUsers').get('@getUsers.all ')
          })
          ...
          _.each(['', 'foo', [], 1, null], (value) => {
            it(`throws when options property is not an object. Such as: ${value}`, (done) => {
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  `only accepts an options object for its second argument. You passed ${value}`
                )
  ...
  context('#contains', () => {
    ...
    it('finds the furthest descendent when filter matches more than 1 element', () => {
      cy.get('#contains-multiple-filter-match')
      .contains('li', 'Maintenance')
      .then(($row) => {
    ...
    describe("should('not.exist')", () => {
      it('returns null when no content exists', () => {
        cy.contains('alksjdflkasjdflkajsdf')
        .should('not.exist')
        .then(($el) => {
          expect($el).to.be.null
        })
    ...
    describe("should('be.visible')", () => {
      it('returns invisible element', () => {
        ...
        cy.contains('span', 'my hidden content')
        .should('not.be.visible')
        .then(($span) => {
          expect($span.get(0)).to.eq(span.get(0))
        })
      ...
      it('throws when content has "i" flag while matchCase: true', (done) => {
        cy.on('fail', (err) => {
          expect(err.message).to.eq(
            'You passed a regular expression with the case-insensitive (_i_) flag and `{ matchCase: true }` to `cy.contains()`. Those options conflict with each other, so please choose one or the other.'
          )
    ...
    describe(
      ...
      () => {
        ...
        it('throws when there is a filter', (done) => {
          cy.on('fail', (err) => {
            expect(err.message).to.include(
              "Expected to find content: 'brand new content' within the selector: 'span' but never did."
            )
        ...
        it('throws when there is no filter but there is a subject', (done) => {
          cy.on('fail', (err) => {
            expect(err.message).to.include(
              "Expected to find content: '0' within the element: <div.badge> but never did."
            )
        ...
        it('throws when there is both a subject and a filter', (done) => {
          cy.on('fail', (err) => {
            expect(err.message).to.include(
              "Expected to find content: 'foo' within the element: <div#edge-case-contains> and with the selector: 'ul' but never did."
            )
