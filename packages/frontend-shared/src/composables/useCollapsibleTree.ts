...
import type { buildSpecTree, DirectoryMap } from '@packages/app/src/specsBackend/buildtree'
...
export function useCollapsibleTree <T extends RawNode<T>> (payload: ReturnType<typeof buildSpecTree>, options: UseCollapsibleTreeOptions = {}) {
  options.expandInitially = options.expandInitially ?? true
  sortTree(payload.tree)
  // TODO: type
  const collapsibleTree = buildTree<T>(payload.tree as any, options)
