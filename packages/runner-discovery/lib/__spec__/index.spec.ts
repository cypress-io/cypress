...
describe('runner-discovery contract', () => {
  ...
  describe('probe route', () => {
    it('builds the probe path from the shared prefix', () => {
      expect(runnerDiscoveryProbePath('abc')).toBe(`${RUNNER_DISCOVERY_ROUTE_PREFIX}abc`)
      expect(runnerDiscoveryProbePath('abc')).toBe('/__cypress/runner-discovery/abc')
    })
  })
