...
const formatBrowser = browser => //# todo finish browser
_.compact([
  browser.displayName,
...
const iterateThroughSpecs = function(options = {}) {
  ...
  const serialWithRecord = () => Promise
  .mapSeries(specs, (spec, index, length) => beforeSpecRun(spec)
  .then(({ estimated }) => runEachSpec(spec, index, length, estimated)).tap(results => afterSpecRun(spec, results, config)));

  var parallelWithRecord = runs => beforeSpecRun()
  .then(function({ spec, claimedInstances, totalInstances, estimated }) {
    ...
    .tap(function(results) {
      ...
      return afterSpecRun(spec, results, config);}).then(() => //# recurse
...
const getProjectId = Promise.method(function(project, id) {
  ...
  return project
  .getProjectId()
  .catch(() => //# no id no problem
  null);
});

const reduceRuns = (runs, prop) => _.reduce(runs, (memo, run) => memo += _.get(run, prop)
, 0);

const getRun = (run, prop) => _.get(run, prop);

const writeOutput = (outputPath, results) => Promise.try(function() {
...
const onWarning = err => console.log(chalk.yellow(err.message));

const openProjectCreate = (projectRoot, socketId, options) => //# now open the project to boot the server
...
.catch({portInUse: true}, err => //# TODO: this needs to move to emit exitEarly
...
const removeOldProfiles = () => browsers.removeOldProfiles()
.catch(err => //# dont make removing old browsers profiles break the build
...
const trashAssets = function(config = {}) {
  ...
  .catch(err => //# dont make trashing assets fail the build
  errors.warning("CANNOT_TRASH_ASSETS", err.stack));
};

//# if we've been told to record and we're not spawning a headed browser
const browserCanBeRecorded = browser => (browser.name === "electron") && browser.isHeadless;
...
const maybeStartVideoRecording = Promise.method(function(options = {}) {
  ...
  const videoPath = suffix => path.join(videosFolder, spec.name + suffix);
...
module.exports = {
  ...
  getElectronProps(isHeaded, project, writeVideoFrame) {
    ...
    if (writeVideoFrame) {
      ...
      electronProps.onPaint = (event, dirty, image) => writeVideoFrame(image.toJPEG(100));
    }
  ...
  postProcessRecording(end, name, cname, videoCompression, shouldUploadVideo) {
    ...
    }).catch(err => //# log that post processing was attempted
