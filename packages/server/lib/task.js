...
module.exports = {
  run(pluginsFilePath, options) {
    ...
    .try(function() {
      ...
      return plugins.execute("task", options.task, options.arg);}).then(function(result) {
      if (result === "__cypress_unhandled__") {
        debug("task is unhandled");
        return plugins.execute("_get:task:keys").then(keys => throwKnownError(`The task '${options.task}' was not handled in the plugins file. The following tasks are registered: ${keys.join(", ")}${fileAndDocsUrl}`));
      }
      ...
      return result;}).timeout(options.timeout)
