...
const removeLastCharacter = s => s.substr(0, s.length - 1);
...
const sanitizeAndConvertNestedArgs = function(str) {
  ...
  .split(",")
  .map(pair => pair.split(everythingAfterFirstEqualRe)).fromPairs()
...
module.exports = {
  toObject(argv) {
    ...
    if (spec = options.spec) {
      const resolvePath = p => path.resolve(options.cwd, p);
  ...
  toArray(obj = {}) {
    ...
    .chain(obj)
    .pick(...whitelist)
    .mapValues((val, key) => `--${key}=${stringify(val)}`).values()
    .value();
  }
};
