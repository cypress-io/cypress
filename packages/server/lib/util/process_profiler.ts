...
export const groupCyProcesses = ({ list }: si.Systeminformation.ProcessesData) => {
  ...
  const isBrowserProcess = (proc: Process): boolean => {
    ...
    const pids: (number | undefined)[] = instance?.allPids ? instance.allPids : [instance?.pid]
...
export const _renameBrowserGroup = (processes: Process[]) => {
  ...
  processes.forEach((proc) => {
    ...
    if (proc.group === 'browser') {
      // the profiler table names the browser itself, so the display name is
      // intentionally stored outside the `Group` union
