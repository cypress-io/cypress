...
const getPatternRelativeToProjectRoot = (specPattern, projectRoot) =>
  _.map(specPattern, p => path.relative(projectRoot, p))
;
...
const find = function(config, specPattern) {
  ...
  //# absolute path
  const javascriptsPaths = _.map(config.javascripts, js => path.join(config.projectRoot, js));
  ...
  const relativePathFromIntegrationFolder = file => path.relative(integrationFolderPath, file);

  const relativePathFromProjectRoot = file => path.relative(config.projectRoot, file);
  ...
  const doesNotMatchAllIgnoredPatterns = file =>
    ...
    _.every(ignorePatterns, pattern => !minimatch(file, pattern, MINIMATCH_OPTIONS))
  ;
  ...
  const matchesSpecPattern = function(file) {
    ...
    const matchesPattern = pattern => minimatch(file, pattern, MINIMATCH_OPTIONS);
  ...
  .map(setNameParts)
  .tap(files => debug("found %d spec files: %o", files.length, files));
};
