...
const join = (char, ...pieces) => _.chain(pieces).compact().join(char).value();

const toCamelObject = (obj, key) => _.set(obj, _.camelCase(key), process.env[key]);

const extract = envKeys => _.transform(envKeys, toCamelObject, {});

const isCodeshipBasic = () => process.env.CI_NAME && (process.env.CI_NAME === "codeship") && process.env.CODESHIP;

const isCodeshipPro = () => process.env.CI_NAME && (process.env.CI_NAME === "codeship") && !process.env.CODESHIP;

const isGitlab = () => process.env.GITLAB_CI || (process.env.CI_SERVER_NAME && /^GitLab/.test(process.env.CI_SERVER_NAME));
...
const isWercker = () => process.env.WERCKER || process.env.WERCKER_MAIN_PIPELINE_STARTED;
...
const provider = function() {
  let left;
  return (left = _detectProviderName()) != null ? left : null;
};
...
const ciParams = () => _get(_providerCiParams);

const commitParams = () => _get(_providerCommitParams);
...
const commitDefaults = function(existingInfo) {
  ...
  const combined = _.transform(existingInfo, (memo, value, key) => memo[key] = _.defaultTo(value != null ? value : commitParamsObj[key], null));
...
const list = () => _.keys(CI_PROVIDERS);
