...
const getByType = type => windows[type];

const getCookieUrl = props => extension.getCookieUrl(props);
...
module.exports = {
  ...
  get(type) {
    let left;
    return (left = getByType(type)) != null ? left : (() => { throw new Error(`No window exists for: '${type}'`); })();
  },
  ...
  hideAllUnlessAnotherWindowIsFocused() {
    ...
    if (BrowserWindow.getFocusedWindow() || recentlyCreatedWindow) { return; }
  ...
  open(projectRoot, options = {}) {
    ...
    .spread(function(url) {
      ...
      return recentlyCreatedWindow = false;}).thenReturn(win);
  },
