...
const getRemoteDebuggingPort = Promise.method(function() {
  let port;
...
const pluginsBeforeBrowserLaunch = function(browser, args) {
  ...
  if (!plugins.has("before:browser:launch")) { return args; }
...
const _normalizeArgExtensions = function(dest, args) {
  let userExtensions;
  const loadExtension = _.find(args, arg => arg.includes(LOAD_EXTENSION));
...
const _navigateUsingCRI = function(url) {
  ...
  return function(client) {
    ...
    .then(() => client.send("Page.navigate", { url }));
  };
};
  ...
  _writeExtension(browser, isTextTerminal, proxyUrl, socketIoRoute) {
    ...
    .then(function(str) {
      ...
      .then(() =>
        ...
        fs.writeFileAsync(extensionBg, str)).return(extensionDest);
    });
  },
  ...
  open(browser, url, options = {}, automation) {
    ...
    .try(() => {
      ...
      .spread(function(extDest) {
        ...
        return utils.launch(browser, "about:blank", args);}).then(launchedBrowser => {
