...
const _getAutomation = async function (win, options, parent) {
    ...
    if (!browserCriClient) {
        browserCriClient = await browser_cri_client_1.BrowserCriClient.create(['127.0.0.1'], port, 'electron', options.onError, () => { });
    }
    const pageCriClient = await browserCriClient.attachToTargetUrl('about:blank');
    const sendClose = () => {
        win.destroy();
    };
    const automation = await cdp_automation_1.CdpAutomation.create(pageCriClient.send, pageCriClient.on, sendClose, parent);
