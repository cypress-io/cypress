...
(function (module, ns) {
  ...
  let Element = ns.require('element')
  let Exception = ns.require('error')
  let message = ns.require('./message')

  let DEFAULT_SCRIPT_TIMEOUT = 60000
  let DEFAULT_SEARCH_TIMEOUT = 60000

  let DEFAULT_WAIT_FOR_INTERVAL = 100
  let DEFAULT_WAIT_FOR_TIMEOUT = 60000

  let SCOPE_TO_METHOD = Object.freeze({
  ...
  Client.prototype = {
     ...
     * @param {Object} options options for driver.
     */
    resetWithDriver (driver, options) {
    ...
    plugin (name, plugin, options) {
      ...
      let result = invokedMethod(this, options)
    ...
    _handleCallback () {
      ...
      if (args[0] && !(args[0] instanceof Exception)) {
        let err = new Exception(this, args[0])
     ...
     */
    _newSession (callback, desiredCapabilities) {
      let newSession = function (data) {
    ...
    waitFor (test, options, callback) {
      ...
      let sync = this.isSync

      // must handle default callback case for sync code
      callback = callback || this.defaultCallback

      // wallclock timer
      let timeout = Date.now() + (options.timeout || DEFAULT_WAIT_FOR_TIMEOUT)

      // interval between test being fired.
      let interval = options.interval || DEFAULT_WAIT_FOR_INTERVAL
     ...
     */
    waitForSync (test, callback, interval, timeout) {
      let err; let result
      let testFunc = function (_err, _result) {
        err = _err
        result = _result
      }
     ...
     * @param {number} timeout time at which we fail in ms.
     */
    waitForAsync (test, callback, interval, timeout) {
      if (Date.now() >= timeout) {
        this.onScriptTimeout && this.onScriptTimeout()
      ...
      test(function (err, result) {
        ...
        let next = this.waitForAsync.bind(
          this,
          test,
          callback,
          interval,
    ...
    startSession: function startSession (callback, desiredCapabilities) {
      ...
      let newSession = function (err, res) {
        if (err) {
          callback(err)
        } else {
          this.sessionId = res.sessionId
          this.capabilities = res.capabilities
          this.runHook('startSession', function () {
            callback(err, res)
          })
        }
      }.bind(this)
    ...
    deleteSession: function destroySession (callback) {
      let cmd = { name: 'deleteSession' }

      let closeDriver = function closeDriver () {
    ...
    sessionCapabilities: function sessionCapabilities (callback) {
      let cmd = { name: 'getSessionCapabilities' }
    ...
    switchToWindow: function switchToWindow (id, callback) {
      let cmd = { name: 'switchToWindow', parameters: { value: id } }
     ...
     * @param {Function} callback called with boolean.
     */
    importScript (script, callback) {
      let cmd = { name: 'importScript', parameters: { script } }
    ...
    switchToFrame: function switchToFrame (id, options, callback) {
      ...
      let cmd = { name: 'switchToFrame', parameters: {} }
    ...
    switchToShadowRoot: function switchToShadowRoot (host, callback) {
      ...
      let cmd = { name: 'switchToShadowRoot', parameters: {} }
    ...
    setContext: function setContext (context, callback) {
      ...
      let cmd = { name: 'setContext', parameters: { value: context } }
    ...
    setScriptTimeout: function setScriptTimeout (timeout, callback) {
      let cmd = { name: 'setScriptTimeout', parameters: { ms: timeout } }
    ...
    setSearchTimeout: function setSearchTimeout (timeout, callback) {
      let cmd = { name: 'setSearchTimeout', parameters: { ms: timeout } }
    ...
    goUrl: function goUrl (url, callback) {
      let cmd = { name: 'goUrl', parameters: { url } }
    ...
    log: function log (msg, level, callback) {
      let cmd = { name: 'log', parameters: { level, value: msg } }
    ...
    screenshot: function screenshot (options, callback) {
      let cmd = { name: 'screenShot', parameters: {} }
     ...
     *     Reference to self.
     */
    executeAsyncScript (script, args, callback, sandbox) {
      if (typeof args === 'function') {
        callback = args
        args = null
      }
    ...
    _findElement: function _findElement (type, query, method, id, callback) {
      ...
      let processElements = function (err, res) {
        let rv
