...
(function (module, ns) {
  ...
  let Command = require('../message').Command
  let Response = require('../message').Response
  ...
  Abstract.prototype = {
    ...
    _nextCommand: function _nextCommand () {
      if (!this._waiting && this._sendQueue.length) {
        this._waiting = true
        let next = this._sendQueue.shift()
    ...
    _onDeviceResponse: function _onDeviceResponse (data) {
      if (this.ready && data.id === this.connectionId) {
        ...
        if (this.marionetteProtocol >= 3) {
          let msg = Response.fromMsg(data.response)
        ...
        let cb = this._responseQueue.shift()
