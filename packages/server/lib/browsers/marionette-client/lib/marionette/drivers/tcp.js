...
let CommandStream = require('../../marionette/command-stream')
let debug = require('debug')('marionette:tcp')
let net = require('net')
let retrySocket = require('socket-retry-connect')
...
Tcp.prototype._connect = function connect (callbackpromises) {
  ...
  retrySocket.waitForSocket(options, function (err, socket) {
    debug('got socket starting command stream %o', { err })
