...
export class CdpFetchTransport {
  ...
  private interceptRequest = async (event: Protocol.Fetch.RequestPausedEvent, sessionId?: string): Promise<void> => {
    ...
    try {
      ...
      this.cleanup(networkId, deferred, sessionId)
    } catch (err) {
      if (networkId) {
        if (requestContinued) {
  ...
  private cleanup (networkId: string, deferred?: pDefer.DeferredPromise<CdpFetchTransportResponse>, sessionId?: string): void {
    if (deferred && this.inFlightRequests.get(networkId) !== deferred) {
      return
    }

    this.inFlightRequests.delete(networkId)
    // Network-side extraInfo tracking shares the flow's request id; error
    // pauses never consume it, so clear it with the flow. (Unmatched pauses
    // clear their own tracking at the pause site — they have no flow.)
