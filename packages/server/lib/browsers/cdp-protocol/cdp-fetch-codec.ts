...
/**
 * Fulfill exactly when the Network record would otherwise lie about what the
 * page received; continue everything else, since continueResponse preserves
 * the wire semantics (extraInfo events, streaming, HTTP caching).
 *
 ...
 * stylesheet handling keys off mimeType) would see the stale value. So the
 * record lies in exactly two cases, and both take Fetch.fulfillRequest:
 *   - a body that differs from the origin bytes
 *   - a changed content-type
