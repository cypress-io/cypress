...
export class NetworkModule extends AbstractModule {
  ...
  /**
   * Listens for the 'network.beforeRequestSent' event
   * @see https://w3c.github.io/webdriver-bidi/#event-network-beforeSendRequest
   * @param callback - the callback function invoked when the 'network.beforeRequestSent' event is called
   */
  onBeforeRequestSent<T> (callback: (params: Bidi.Network.BeforeRequestSent) => T) {
    this._bidiSocket.bindEvent<Bidi.Network.BeforeRequestSent, T>('network.beforeRequestSent', callback)
  }
