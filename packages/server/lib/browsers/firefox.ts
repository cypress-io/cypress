  ...
  try {
    if (browser.family === 'firefox') {
      if (browser.majorVersion) {
        ...
        // right now we are gating on firefox version 130 to turn on BiDi.
...
export async function open (browser: Browser, url: string, options: BrowserLaunchOpts, automation: Automation): Promise<BrowserInstance> {
   ...
   */
    ...
    const client = await firefoxUtil.setup({ automation, url, foxdriverPort, webdriverClient, remotePort: cdpPort, useWebDriverBiDi: USE_WEBDRIVER_BIDI, onError: options.onError })
