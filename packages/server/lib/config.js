...
const toWords = str => str.trim().split(oneOrMoreSpacesRe);

const isCypressEnvLike = key => _.chain(key)
.invoke('toUpperCase')
.startsWith(CYPRESS_ENV_PREFIX)
.value() &&
  !_.includes(CYPRESS_RESERVED_ENV_VARS, key);

const removeEnvPrefix = key => key.slice(CYPRESS_ENV_PREFIX_LENGTH);
...
const convertRelativeToAbsolutePaths = (projectRoot, obj, defaults = {}) => _.reduce(folders, function(memo, folder) {
...
const validateNoBreakingConfig = cfg => _.each(breakingConfigKeys, function(key) {
  if (_.has(cfg, key)) {
    switch (key) {
...
const validate = (cfg, onErr) => _.each(cfg, function(value, key) {
...
const validateFile = file => settings => validate(settings, errMsg => errors.throw("SETTINGS_VALIDATION_ERROR", file, errMsg));
...
module.exports = {
  ...
  getConfigKeys() { return configKeys.concat(experimentalConfigKeys); },
  ...
  mergeDefaults(config = {}, options = {}) {
    ...
    validate(config, errMsg => errors.throw("CONFIG_VALIDATION_ERROR", errMsg));
  ...
  setNodeBinary: Promise.method(function(obj, onWarning) {
    ...
    .then(function({ path, version }) {
      obj.resolvedNodePath = path;
      return obj.resolvedNodeVersion = version;}).catch(function(err) {
      onWarning(errors.get('COULD_NOT_FIND_SYSTEM_NODE', process.versions.node));
      return obj.resolvedNodeVersion = process.versions.node;}).return(obj);
  }),
  ...
  setSupportFileAndFolder(obj) {
    if (!obj.supportFile) { return Promise.resolve(obj); }
    ...
    .try(function() {
      ...
      return debug("resolved support file %s", obj.supportFile);}).then(function() {
  ...
  setPluginsFile: Promise.method(function(obj) {
    ...
    .try(function() {
      ...
      return debug(`set pluginsFile to ${obj.pluginsFile}`);}).catch({code: "MODULE_NOT_FOUND"}, function() {
  ...
  parseEnv(cfg, envCLI, resolved = {}) {
    ...
    const resolveFrom = (from, obj = {}) => _.each(obj, (val, key) => envVars[key] = {
      value: val,
      from
    });
    ...
    let envProc = (left = this.getProcessEnvVars(process.env)) != null ? left : {};
    ...
    const configFromEnv = _.reduce(envProc, function(memo, val, key) {
      let cfgKey;
