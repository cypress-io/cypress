...
module.exports = {
  ...
  getSpecs(spec, config) {
    ...
    const getSpecsHelper = () => {
      ...
      if (spec === "__all") {
        return specsUtil.find(config)
        .then(R.tap(specs => debug("found __all specs %o", specs))).filter(function(spec) {
          if (experimentalComponentTestingEnabled) {
            return spec.specType === "integration";
          } else {
            return true;
          }}).then(R.tap(specs => debug("filtered __all specs %o", specs))).map(spec => //# grab the name of each
        spec.absolute).map(convertSpecPath);
      } else {
  ...
  getTitle(test) {
    if (test === "__all") { return "All Tests"; } else { return test; }
  },
  ...
  getJavascripts(config) {
    ...
    const paths = _.map(files, file => path.resolve(projectRoot, file));

    return Promise
    .map(paths, function(p) {
      //# is the path a glob?
      if (!glob.hasMagic(p)) { return p; }
