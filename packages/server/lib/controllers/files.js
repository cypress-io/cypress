  ...
  getSpecs (spec, config, extraOptions = {}) {
    ...
    const getSpecsHelper = async () => {
      ...
      if (spec === '__all') {
        ...
        // - the e2e.specExcludePattern
        return ctx.project.findSpecs(config.projectRoot, 'e2e', e2ePatterns.specPattern, e2ePatterns.specExcludePattern, componentPatterns.specPattern)
        .then((specs) => {
