...
import { netStubbingState, NetStubbingState, getRoutesForRequest } from '@packages/net-stubbing'
...
export class ServerBase<TSocket extends SocketE2E | SocketCt> {
  ...
  private skipDomainInjectionForDomains?: string[]
  private _urlResolver?: Bluebird<Record<string, any>>
  ...
  destroyAut () {
    if (this.testingType === 'component') {
      return (this.socket as any).destroyAut()
    }
  }
