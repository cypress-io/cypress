...
import { matchRoutes, createHttpInterceptWithDefaultMiddleware } from '@packages/network-interception'
...
import type { BrowserPreRequest, ProxyNetworkServices } from '@packages/proxy'
...
export class ServerBase<TSocket extends SocketE2E | SocketCt> {
  ...
  createNetworkProxy ({ config, remoteStates, shouldCorrelatePreRequests, getCurrentBrowser }) {
    ...
    const networkServices = createProxyNetworkServices()
