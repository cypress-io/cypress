...
export class OpenProject {
  ...
  async create (path: string, args: InitializeProjectOptions, options: OpenProjectLaunchOptions) {
    ...
    try {
      ...
      const { specPattern, specExcludePattern, additionalIgnorePattern } = await this._ctx.actions.project.setSpecsFoundBySpecPattern({
        path,
        testingType,
      ...
      this._ctx.project.startSpecWatcher(path, testingType, specPattern, specExcludePattern, additionalIgnorePattern)
