  ...
  async launch (browser, spec: Cypress.Cypress['spec'], prevOptions?: OpenProjectLaunchOpts) {
    ...
    this.relaunchBrowser = async () => {
      ...
      if (process.env.CYPRESS_INTERNAL_E2E_TESTING_SELF) {
        return await browsers.connectToExisting(browser, options, automation, this._ctx?.coreData.servers.appSocketServer)
      }
      ...
      if (options.shouldLaunchNewTab && browser.name !== 'electron') {
        ...
        return await browsers.connectToNewSpec(browser, { onInitializeNewBrowserTab, ...options }, automation, this._ctx?.coreData.servers.appSocketServer)
      }
