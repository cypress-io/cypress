...
const extension_1 = __importDefault(require('@packages/extension'))
...
const debug = debug_1.default('cypress:server:automation:cookies')
...
exports.normalizeGetCookies = (cookies) => {
...
exports.normalizeGetCookieProps = (props) => {
  if (!props) {
    return props
  }
  ...
  return lodash_1.default.omit(cookie, 'hostOnly')
}
...
class Cookies {
  ...
  getCookies (data, automate) {
    ...
    .then((cookies) => {
      cookies = exports.normalizeGetCookies(cookies)
  ...
  getCookie (data, automate) {
    ...
    .then((cookie) => {
      if (this.isNamespaced(cookie)) {
        ...
        cookie = exports.normalizeGetCookieProps(cookie)
  ...
  clearCookies (data, automate) {
    const cookies = lodash_1.default.reject(normalizeCookies(data), (cookie) => this.isNamespaced(cookie))
    ...
    return bluebird_1.default.map(cookies, clear)
  }
  changeCookie (data) {
