...
const log       = require("debug")("cypress:server:scaffold");
const fs        = require("./util/fs");
const glob      = require("./util/glob");
const cwd       = require("./cwd");
const debug     = require("debug")("cypress:server:scaffold");
const { equals, head, isEmpty, always } = require("ramda");
...
const getPathFromIntegrationFolder = file => file.substring(file.indexOf("integration/") + "integration/".length);

const isDifferentNumberOfFiles = (files, exampleSpecs) => files.length !== exampleSpecs.length;
...
const getExampleSpecs = () =>
  ...
  .then(function(fullPaths) {
    ...
    const shortPaths = _.map(fullPaths, file => getPathFromIntegrationFolder(file));

    //# index for quick lookup and for getting full path from short path
    const index = _.transform(shortPaths, (memo, spec, i) => memo[spec] = fullPaths[i]
    , {});

    return { fullPaths, shortPaths, index };})
;

const getIndexedExample = (file, index) => index[getPathFromIntegrationFolder(file)];

const filesNamesAreDifferent = (files, index) =>
  _.some(files, file => !getIndexedExample(file, index))
;

const getFileSize = file => fs.statAsync(file).get("size");
...
const filesSizesAreSame = (files, index) =>
  ...
  .spread((fileSizes, originalFileSizes) =>
    _.every(fileSizes, (size, i) => size === originalFileSizes[i]))
;

const isNewProject = function(integrationFolder) {
  ...
  .then(function(files) {
    ...
    if (isEmpty(files)) { return true; } // 1

    return getExampleSpecs()
    .then(function(exampleSpecs) {
...
module.exports = {
  ...
  integrationExampleName() { return exampleFolderName; },

  integration(folder, config) {
    debug(`integration folder ${folder}`);

    //# skip if user has explicitly set integrationFolder
    if (!isDefault(config, "integrationFolder")) { return Promise.resolve(); }
  ...
  fixture(folder, config) {
    ...
    if (!config.fixturesFolder || !isDefault(config, "fixturesFolder")) { return Promise.resolve(); }
  ...
  support(folder, config) {
    ...
    if (!config.supportFile || !isDefault(config, "supportFile")) { return Promise.resolve(); }
  ...
  verifyScaffolding(folder, fn) {
    ...
    .then(() => debug(`folder ${folder} already exists`)).catch(() => {
  ...
  fileTree(config = {}) {
    ...
    const getFilePath = (dir, name) => path.relative(config.projectRoot, path.join(dir, name));

    return getExampleSpecs()
    .then(specs => {
      let files = _.map(specs.shortPaths, file => getFilePath(config.integrationFolder, file));
  ...
  _fileListToTree(files) {
    ...
    return _.reduce(files, function(tree, file) {
      ...
      _.each(parts, function(part, index) {
        let entry;
        if (!(entry = _.find(placeholder, {name: part}))) {
          entry = {name: part};
