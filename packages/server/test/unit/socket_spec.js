  ...
  context("integration", function() {
    beforeEach(function(done) {
      ...
      .then(() => {
        ...
        return this.client = socketIo.client(proxyUrl, {
          agent: this.agent,
          path: socketIoRoute,
    ...
    it("can emit a circular object without crashing", function(done) {
      ...
      return this.client.emit("backend:request", "exec", "quuz", function(res) {
        expect(res.response).to.deep.eq(foo);
        return done();
      });
    });

    context("on(automation:request)", function() {
      describe("#onAutomation", function() {
        ...
        before(function() {
          ...
          return extensionBackgroundPage =  require('@packages/extension/app/background');
        });

        beforeEach(function(done) {
          this.socket.io.on("connection", extClient => {
            this.extClient = extClient;
            return this.extClient.on("automation:client:connected", () => done());
          });
        ...
        it("does not return cypress namespace or socket io cookies", function(done) {
          ...
          return this.client.emit("automation:request", "get:cookies", {domain: "localhost"}, function(resp) {
        ...
        it("does not clear any namespaced cookies", function(done) {
          ...
          return this.client.emit("automation:request", "clear:cookies", cookies, function(resp) {
            expect(resp).to.deep.eq({
              response: [
                {name: "session", value: "key", path: "/", domain: "google.com", secure: true, httpOnly: true, expiry: 123}
              ]
        ...
        it("throws when no clients connected to automation", function(done) {
          ...
          return this.client.emit("automation:request", "get:cookies", {domain: "foo"}, function(resp) {
        ...
        it("returns true when tab matches magic string", function(done) {
          ...
          return this.client.emit("is:automation:client:connected", {element: "__cypress-string", string: "string"}, function(resp) {
        ...
        it("returns false when times out", function(done) {
          ...
          return this.client.emit("is:automation:client:connected", {element: "__cypress-string", string: "string", timeout: 100}, function(resp) {
            expect(resp).to.be.false;
            return done();
          });
        });

        return it("retries multiple times and stops after timing out", function(done) {
          //# just force isSocketConnected to return false until the 4th retry
          const iSC = sinon.stub(this.socket, "isSocketConnected");
      ...
      return describe("options.onAutomationRequest", function() {
        beforeEach(function() {
          return this.ar = sinon.stub(this.automation, "request");
        });

        it("calls onAutomationRequest with message and data", function(done) {
          this.ar.withArgs("focus", {foo: "bar"}).resolves([]);

          return this.client.emit("automation:request", "focus", {foo: "bar"}, function(resp) {
        ...
        it("does not return __cypress or __socket.io namespaced cookies", function() {});

        it("throws when onAutomationRequest rejects");

        return it("is:automation:client:connected returns true", function(done) {
          this.ar.withArgs("is:automation:client:connected", {string: "foo"}).resolves(true);

          return this.client.emit("is:automation:client:connected", {string: "foo"}, function(resp) {
            expect(resp).to.be.true;
            return done();
          });
        });
    ...
    context("on(automation:push:request)", function() {
      beforeEach(function(done) {
        this.socketClient.on("automation:client:connected", () => done());

        return this.client.emit("automation:client:connected");
      });

      return it("emits 'automation:push:message'", function(done) {
        const data = {cause: "explicit", cookie: {name: "foo", value: "bar"}, removed: true};

        const emit = sinon.stub(this.socket.io, "emit");

        return this.client.emit("automation:push:request", "change:cookie", data, function() {
    ...
    context("on(open:finder)", function() {
      beforeEach(() => sinon.stub(open, "opn").resolves());

      return it("calls opn with path", function(done) {
    ...
    context("on(watch:test:file)", () => it("calls socket#watchTestFileByPath with config, spec argument", function(done) {
    ...
    context("on(app:connect)", () => it("calls options.onConnect with socketId and socket", function(done) {
    ...
    context("on(get:fixture)", function() {
      ...
      return it("errors when fixtures fails", function(done) {
        const cb = function(resp) {
    ...
    context("on(http:request)", function() {
      it("calls socket#onRequest", function(done) {
        sinon.stub(this.options, "onRequest").resolves({foo: "bar"});

        return this.client.emit("backend:request", "http:request", "foo", function(resp) {
      ...
      return it("catches errors and clones them", function(done) {
    ...
    context("on(exec)", function() {
      ...
      return it("errors when execution fails, passing through timedOut", function(done) {
    ...
    context("on(firefox:force:gc)", function() {
      ...
      return it("errors when collectGarbage throws", function(done) {
        const err = new Error('foo');
    ...
    return context("on(save:app:state)", () => it("calls onSavedStateChanged with the state", function(done) {
  ...
  return context("unit", function() {
    beforeEach(function() {
      ...
      .then(() => {
        ...
        return this.socket = this.server._socket;
      });
    });
    ...
    context("constructor", function() {
      it("listens for 'file:updated' on preprocessor", function() {
        ...
        return expect(preprocessor.emitter.on).to.be.calledWith("file:updated");
      });

      return it("does not listen for 'file:updated' if config.watchForFileChanges is false", function() {
        preprocessor.emitter.on.reset();
        this.cfg.watchForFileChanges = false;
        const socket = new Socket(this.cfg);
        return expect(preprocessor.emitter.on).not.to.be.called;
      });
    });
    ...
    context("#close", function() {
      it("calls close on #io", function() {
        this.socket.close();
        return expect(this.socket.io.close).to.be.called;
      });

      return it("does not error when io isnt defined", function() {
        return this.socket.close();
      });
    });

    context("#watchTestFileByPath", function() {
      beforeEach(function() {
      ...
      it("closes existing watched test file", function() {
        ...
        return this.socket.watchTestFileByPath(this.cfg, "test2.js").then(() => {
          return expect(preprocessor.removeFile).to.be.calledWithMatch("test1.js", this.cfg);
        });
      });

      it("sets #testFilePath", function() {
        return this.socket.watchTestFileByPath(this.cfg, `integration${path.sep}test1.js`).then(() => {
          return expect(this.socket.testFilePath).to.eq(`tests${path.sep}test1.js`);
        });
      });
      ...
      it("watches file by path", function() {
        ...
        return expect(preprocessor.getFile).to.be.calledWith(`tests${path.sep}test2.coffee`, this.cfg);
      });
      ...
      it("watches file by relative path in spec object", function() {
        ...
        return expect(preprocessor.getFile).to.be.calledWith(spec.relative, this.cfg);
      });

      return it("triggers watched:file:changed event when preprocessor 'file:updated' is received", function(done) {
    ...
    return context("#startListening", function() {
      it("sets #testsDir", function() {
        ...
        return expect(this.socket.testsDir).to.eq(this.cfg.integrationFolder);
      });

      describe("watch:test:file", function() {
        it("listens for watch:test:file event", function() {
          this.socket.startListening(this.server.getHttpServer(), this.automation, this.cfg, {});
          return expect(this.mockClient.on).to.be.calledWith("watch:test:file");
        });

        return it("passes filePath to #watchTestFileByPath", function() {
          ...
          this.socket.startListening(this.server.getHttpServer(), this.automation, this.cfg, {});
          return expect(watchTestFileByPath).to.be.calledWith(this.cfg, "foo/bar/baz");
        });
      });

      return describe("#onTestFileChange", function() {
        beforeEach(() => sinon.spy(fs, "statAsync"));

        it("calls statAsync on .js file", function() {
          return this.socket.onTestFileChange("foo/bar.js").catch(function() {}).then(() => {
            return expect(fs.statAsync).to.be.calledWith("foo/bar.js");
          });
        });

        it("calls statAsync on .coffee file", function() {
          return this.socket.onTestFileChange("foo/bar.coffee").then(() => {
            return expect(fs.statAsync).to.be.calledWith("foo/bar.coffee");
          });
        });

        return it("does not emit if stat throws", function() {
          return this.socket.onTestFileChange("foo/bar.js").then(() => {
            return expect(this.io.emit).not.to.be.called;
          });
        });
      });
    });
