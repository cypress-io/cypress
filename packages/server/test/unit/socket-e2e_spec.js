...
describe('lib/socket', () => {
  ...
  context('unit - Socket-E2E', () => {
    ...
    context('#close', () => {
      it('calls removes \'file:updated\' from preprocessor', function () {
        this.socket.close()

        expect(preprocessor.emitter.removeListener).to.be.calledWith('file:updated')
      })
    })
    ...
    context('#startListening', () => {
      describe('watch:test:file', () => {
        ...
        it('listens for studio:init event')
        it('listens for studio:save event')
        it('listens for get:commands:text event')
      })
    })
    ...
    it('#onStudioTestFileChange')
    it('#removeOnStudioTestFileChange')
  })
})
