...
describe("lib/fixture", function() {
  beforeEach(function() {
    ...
    return config.get(this.todosPath).then(cfg => {
      return ({fixturesFolder: this.fixturesFolder} = cfg);
    });
  });

  afterEach(() => FixturesHelper.remove());

  context("file not found", () => it("throws when file cannot be found", function() {
    const p = "does-not-exist.json";

    return fixture.get(this.fixturesFolder, p)
    .then(function() {
      throw new Error("should have failed but did not");}).catch(err => {
      expect(err.message).to.include("A fixture file could not be found");
      return expect(err.message).to.include(p);
    });
  }));

  context("unicode escape syntax", () => it("can parse unicode escape in JSON", function() {
    return fixture.get(this.fixturesFolder, "unicode_escape.json").then(obj => expect(obj).to.deep.eq({
      name: "\u2665"
    }));
}));

  context("nested fixtures", () => it("can pass path to nested fixture", function() {
    return fixture.get(this.fixturesFolder, "nested/fixture.js").then(obj => expect(obj).to.deep.eq({
      nested: "fixture"
    }));
}));

  context("json files", function() {
...
`;
      ...
      .then(function() {
        throw new Error("should have failed but did not");}).catch(function(err) {
        if (isWindows()) {
          ...
          return expect(err.message).to.include("Expecting 'EOF', '}', ':', ',', ']', got 'STRING'");
        } else {
          // on other platforms can match the error directly
          return expect(eol.auto(err.message)).to.eq(eol.auto(e));
        }
      });
    });
    ...
    it("does not reformat json on parse error", function() {
      ...
      .then(function() {
        ...
        return fs.readFileAsync(this.fixturesFolder + "/bad_json.json", "utf8").then(str => expect(str).to.eq(`\
{
"bad": "json"
"should": "not parse"
}\
`
    ...
    it("does not reformat json or write fixture file", function() {
      return fixture.get(this.fixturesFolder, "no_format.json").then(obj => {
        return fs.readFileAsync(this.fixturesFolder + "/no_format.json", "utf8").then(json => expect(json).to.eq('{"id": 1, "name": "brian"}'));
      });
    });

    it("does not remove string whitespace", function() {
      return fixture.get(this.fixturesFolder, "words.json").then(obj => {
        return fs.readFileAsync(this.fixturesFolder + "/words.json", "utf8").then(json => expect(json).to.eq(`\
{
...
`
    ...
    it("parses json to valid JS object", function() {
      return fixture.get(this.fixturesFolder, "users.json").then(users => expect(users).to.deep.eq([
        {
          id: 1,
          name: "brian"
        },{
    ...
    it("does not reformat empty objects", function() {
      ...
      return Promise.map(Array(500), fn, {concurrency: 5}).then(() => {
        return fs.readFileAsync(this.fixturesFolder + "/empty_objects.json", "utf8").then(str => expect(str).to.eq(`\
{
"empty": {
  "object": {},
  "array": [],
...
`
    ...
    return it("can load a fixture with no extension when a same-named folder also exists", function() {
      ...
      .then(cfg => {
        ...
        .then(fixture => expect(fixture).to.deep.eq({ "bar": "baz" }));
      });
    });
  });

  context("js files", function() {
    it("returns valid JS object", function() {
      return fixture.get(this.fixturesFolder, "user.js").then(user => expect(user).to.deep.eq({
        id: 1,
        name: "brian",
        age: 29,
        posts: []
    ...
    it("does not rewrite file as a formated valid JS object", function() {
      return fixture.get(this.fixturesFolder, "no_format.js").then(obj => {
        return fs.readFileAsync(this.fixturesFolder + "/no_format.js", "utf8").then(str => expect(str).to.eq('{foo: "bar", baz: "quux"}'));
      });
    });

    return it("throws on a bad JS object", function() {
      const e =
        `\
bad_js.js:3
  bar: "bar
...
`;
      ...
      .then(function() {
        throw new Error("should have failed but did not");}).catch(err => {
        return expect(err.message).to.eq(`'bad_js.js' is not a valid JavaScript object.\n${e}`);
      });
    });
  });

  context("coffee files", function() {
    it("returns valid coffee object", function() {
      return fixture.get(this.fixturesFolder, "account.coffee").then(account => expect(account).to.deep.eq({
        name: "cypress",
        users: []
      }));
  });
    ...
    it("does not rewrite coffee files", function() {
      return fixture.get(this.fixturesFolder, "no_format.coffee").then(() => {
        return fs.readFileAsync(this.fixturesFolder + "/no_format.coffee", "utf8").then(str => expect(str).to.eq(`\
[
{id: 1}
{id: 2}
]\
`
    ...
    return it("throws on bad coffee object", function() {
      return fixture.get(this.fixturesFolder, "bad_coffee.coffee")
      .then(function() {
        throw new Error("should have failed but did not");}).catch(err => expect(err.message).to.eq(`\
...
`
  ...
  context("html files", function() {
    it("returns html as a string", function() {
      return fixture.get(this.fixturesFolder, "index.html").then(index => expect(index).to.eq(`\
<!doctype html>
<html>
<head>
...
`
    ...
    return it("does not rewrite file as formatted html", function() {
      return fixture.get(this.fixturesFolder, "index.html").then(() => {
        return fs.readFileAsync(this.fixturesFolder + "/index.html", "utf8").then(str => expect(str).to.eq(`\
<!doctype html>
<html>
<head>
<title>index.html</title>
...
`
  ...
  context("txt files", () => it("returns text as string", function() {
    return fixture.get(this.fixturesFolder, "message.txt").then(index => expect(index).to.eq("foobarbaz"));
  }));

  context("csv files", () => it("returns text as string", function() {
    return fixture.get(this.fixturesFolder, "data.csv").then(index => expect(index).to.eq(`\
...
`
  ...
  context("file with unknown extension", () => it("returns text as string", function() {
    return fixture.get(this.fixturesFolder, "unknown_ext.yaml").then(index => expect(index).to.eq(`\
- foo
- bar
- 
\
`
  ...
  context("file with unknown extension and encoding specified", () => it("returns text encoded as specified", function() {
    return fixture.get(this.fixturesFolder, "ascii.foo", {encoding: "ascii"}).then(index => expect(index).to.eq("o#?\n"));
  }));

  context("image files", function() {
    beforeEach(function() {
      return this.read = (folder, image, encoding) => {
    ...
    it("returns png as string", function() {
      ...
      .then(str => {
        ...
        .then(base64 => expect(base64).to.eq(str));
      });
    });
    ...
    return it("returns png as binary", function() {
      ...
      .then(bin => {
        ...
        .then(bin2 => expect(bin).to.eq(bin2));
      });
    });
  });

  context("zip files", () => it("returns zip as base64 string", function() {
    return fixture.get(this.fixturesFolder, "example.zip").then(function(base64) {
      ...
      return expect(base64).to.eq(str);
    });
  }));

  context("extension omitted", function() {
    it("#1 finds json", function() {
      return fixture.get(this.fixturesFolder, "foo").then(obj => expect(obj).to.deep.eq([
        {json: true}
      ]));
  });

    it("#2 finds js", function() {
      return fixture.get(this.fixturesFolder, "bar").then(obj => expect(obj).to.deep.eq({js: true}));
  });

    return it("throws when no file by any extension can be found", function() {
      return fixture.get(this.fixturesFolder, "does-not-exist")
      .then(function() {
        throw new Error("should have failed but did not");}).catch(err => {
        const p = this.fixturesFolder + "/does-not-exist";
        expect(err.message).to.include("A fixture file could not be found");
        return expect(err.message).to.include("/does-not-exist");
      });
    });
  });

  return context("new lines", function() {
    it("does not remove trailing new lines on .txt", function() {
      return fixture.get(this.fixturesFolder, "trailing_new_line.txt").then(str => {
        return fs.readFileAsync(this.fixturesFolder + "/trailing_new_line.txt", "utf8").then(str2 => expect(str2).to.eq("foo\nbar\nbaz\n"));
      });
    });
    ...
    it("does not remove trailing new lines on .json", function() {
      return fixture.get(this.fixturesFolder, "trailing_new_line.json").then(str => {
        return fs.readFileAsync(this.fixturesFolder + "/trailing_new_line.json", "utf8").then(str2 => expect(str2).to.eq('{"foo": "bar"}\n'));
      });
    });

    it("does not remove trailing new lines on .js", function() {
      return fixture.get(this.fixturesFolder, "trailing_new_line.js").then(str => {
        return fs.readFileAsync(this.fixturesFolder + "/trailing_new_line.js", "utf8").then(str2 => expect(str2).to.eq('{foo: "bar"}\n'));
      });
    });
    ...
    it("does not remove trailing new lines on .coffee", function() {
      return fixture.get(this.fixturesFolder, "trailing_new_line.coffee").then(str => {
        return fs.readFileAsync(this.fixturesFolder + "/trailing_new_line.coffee", "utf8").then(str2 => expect(str2).to.eq('{ foo: "bar" }\n'));
      });
    });

    return it("does not remove trailing new lines on .html", function() {
      return fixture.get(this.fixturesFolder, "trailing_new_line.html").then(str => {
        return fs.readFileAsync(this.fixturesFolder + "/trailing_new_line.html", "utf8").then(str2 => expect(str2).to.eq('<html><body>foo</body></html>\n'));
      });
    });
  });
});
