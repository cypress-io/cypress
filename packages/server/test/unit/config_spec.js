...
describe("lib/config", function() {
  beforeEach(function() {
    ...
    return process.env = _.omit(process.env, "CYPRESS_DEBUG");
  });
  ...
  context("environment name check", function() {
    it("throws an error for unknown CYPRESS_INTERNAL_ENV", function() {
      ...
      return expect(errors.throw).have.been.calledOnce;
    });

    return it("allows production CYPRESS_INTERNAL_ENV", function() {
      ...
      const options = {};
      config.mergeDefaults(cfg, options);
      return expect(errors.throw).not.to.be.called;
    });
  });
  ...
  context(".get", function() {
    beforeEach(function() {
      ...
      return this.setup = (cypressJson = {}, cypressEnvJson = {}) => {
    ...
    it("sets projectRoot", function() {
      ...
      .then(obj => {
        ...
        return expect(obj.env).to.deep.eq({foo: "bar"});
      });
    });
    ...
    it("sets projectName", function() {
      this.setup({}, {foo: "bar"});
      return config.get(this.projectRoot)
      .then(obj => expect(obj.projectName).to.eq("project"));
    });
    ...
    context("port", function() {
      ...
      it("can override default port", function() {
        ...
        .then(obj => expect(obj.port).to.eq(8080));
      });

      it("updates browserUrl", function() {
        return config.get(this.projectRoot, {port: 8080})
        .then(obj => expect(obj.browserUrl).to.eq("http://localhost:8080/__/"));
      });

      return it("updates proxyUrl", function() {
        return config.get(this.projectRoot, {port: 8080})
        .then(obj => expect(obj.proxyUrl).to.eq("http://localhost:8080"));
      });
    });

    return context("validation", function() {
      beforeEach(function() {
        this.expectValidationPasses = () => {
          return config.get(this.projectRoot); //# shouldn't throw
        };

        return this.expectValidationFails = (errorMessage = "validation error") => {
          return config.get(this.projectRoot)
          .then(function() {
            throw new Error("should throw validation error");}).catch(err => expect(err.message).to.include(errorMessage));
        };
      });
      ...
      context("animationDistanceThreshold", function() {
        it("passes if a number", function() {
          this.setup({animationDistanceThreshold: 10});
          return this.expectValidationPasses();
        });

        return it("fails if not a number", function() {
      ...
      context("baseUrl", function() {
        ...
        return it("fails if not a fully qualified url", function() {
          this.setup({baseUrl: "localhost"});
          return this.expectValidationFails("be a fully qualified URL");
        });
      });

      context("chromeWebSecurity", function() {
        it("passes if a boolean", function() {
          this.setup({chromeWebSecurity: false});
          return this.expectValidationPasses();
        });

        return it("fails if not a boolean", function() {
      ...
      context("modifyObstructiveCode", function() {
        it("passes if a boolean", function() {
      ...
      context("fileServerFolder", function() {
        it("passes if a string", function() {
          this.setup({fileServerFolder: "_files"});
          return this.expectValidationPasses();
        });

        return it("fails if not a string", function() {
      ...
      context("fixturesFolder", function() {
        it("passes if a string", function() {
      ...
      context("ignoreTestFiles", function() {
        ...
        return it("fails if not an array of strings", function() {
          this.setup({ignoreTestFiles: [5]});
      ...
      context("integrationFolder", function() {
        it("passes if a string", function() {
      ...
      context("numTestsKeptInMemory", function() {
        it("passes if a number", function() {
      ...
      context("pageLoadTimeout", function() {
        it("passes if a number", function() {
      ...
      context("requestTimeout", function() {
        it("passes if a number", function() {
      ...
      context("responseTimeout", function() {
        it("passes if a number", function() {
      ...
      context("trashAssetsBeforeRuns", function() {
        it("passes if a boolean", function() {
      ...
      context("videoCompression", function() {
        it("passes if a number", function() {
      ...
      context("videoUploadOnPasses", function() {
        it("passes if a boolean", function() {
      ...
      context("viewportHeight", function() {
        it("passes if a number", function() {
      ...
      context("viewportWidth", function() {
        it("passes if a number", function() {
      ...
      context("waitForAnimations", function() {
        it("passes if a boolean", function() {
      ...
      context("watchForFileChanges", function() {
        it("passes if a boolean", function() {
      ...
      context("blacklistHosts", function() {
        it("passes if a string", function() {
      ...
      return context("firefoxGcInterval", function() {
        it("passes if a number", function() {
        ...
        return it("fails if invalid object", function() {
  ...
  context(".getConfigKeys", function() {
    beforeEach(function() {
      return this.includes = key => expect(config.getConfigKeys()).to.include(key);
    });

    return it("includes blacklistHosts", function() {
      return this.includes("blacklistHosts");
    });
  });

  context(".resolveConfigValues", function() {
    beforeEach(function() {
      return this.expected = function(obj) {
        const merged = config.resolveConfigValues(obj.config, obj.defaults, obj.resolved);
        return expect(merged).to.deep.eq(obj.final);
      };
    });
    ...
    return it("ignores values not found in configKeys", function() {
      return this.expected({
  ...
  context(".mergeDefaults", function() {
    beforeEach(function() {
      return this.defaults = (prop, value, cfg = {}, options = {}) => {
        cfg.projectRoot = "/foo/bar/";
        return config.mergeDefaults(cfg, options)
        .then(R.prop(prop))
        .then(result => expect(result).to.deep.eq(value));
      };
    });
    ...
    it("resets numTestsKeptInMemory to 0 when runMode", () => config.mergeDefaults({projectRoot: "/foo/bar/"}, {isTextTerminal: true})
    .then(cfg => expect(cfg.numTestsKeptInMemory).to.eq(0)));

    it("resets watchForFileChanges to false when runMode", () => config.mergeDefaults({projectRoot: "/foo/bar/"}, {isTextTerminal: true})
    .then(cfg => expect(cfg.watchForFileChanges).to.be.false));

    it("can override morgan in options", () => config.mergeDefaults({projectRoot: "/foo/bar/"}, {morgan: false})
    .then(cfg => expect(cfg.morgan).to.be.false));

    it("can override isTextTerminal in options", () => config.mergeDefaults({projectRoot: "/foo/bar/"}, {isTextTerminal: true})
    .then(cfg => expect(cfg.isTextTerminal).to.be.true));

    it("can override socketId in options", () => config.mergeDefaults({projectRoot: "/foo/bar/"}, {socketId: 1234})
    .then(cfg => expect(cfg.socketId).to.eq(1234)));
    ...
    it("deletes envFile", function() {
      ...
      .then(function(cfg) {
        ...
        return expect(cfg).not.to.have.property("envFile");
      });
    });

    it("merges env into @config.env", function() {
      const obj = {
        projectRoot: "/foo/bar/",
        env: {
          host: "localhost",
      ...
      .then(cfg => expect(cfg.env).to.deep.eq({
        host: "localhost",
        user: "brian",
        version: "0.13.1",
        foo: "bar"
    ...
    return describe(".resolved", function() {
      it("sets reporter and port to cli", function() {
        const obj = {
          projectRoot: "/foo/bar"
        };
        ...
        return config.mergeDefaults(obj, options)
        .then(cfg => expect(cfg.resolved).to.deep.eq({
          env:                        { },
      ...
      return it("sets config, envFile and env", function() {
  ...
  context(".setPluginResolvedOn", function() {
    it("resolves an object with single property", function() {
      const cfg = {};
      const obj = {
        foo: "bar"
      };
      config.setPluginResolvedOn(cfg, obj);
      return expect(cfg).to.deep.eq({
        foo: {
          value: "bar",
          from: "plugin"
        }
    ...
    it("resolves an object with multiple properties", function() {
      const cfg = {};
      const obj = {
        foo: "bar",
        baz: [1, 2, 3]
    ...
    return it("resolves a nested object", function() {
      ...
      config.setPluginResolvedOn(cfg, obj);
      return expect(cfg, "foo.bar gets value").to.deep.eq({
        foo: {
          bar: {
            value: 42,
            from: "plugin"
  ...
  context("_.defaultsDeep", () => it("merges arrays", function() {
    ...
    return expect(merged, "arrays are combined").to.deep.eq({
      list: [1, 2]
    });
  }));

  context(".updateWithPluginValues", function() {
    it("is noop when no overrides", () => expect(config.updateWithPluginValues({foo: 'bar'}, null)).to.deep.eq({
      foo: 'bar'
    }));

    it("is noop with empty overrides", () => expect(config.updateWithPluginValues({foo: 'bar'}, {})).to.deep.eq({
      foo: 'bar'
    }));

    it("updates resolved config values and returns config with overrides", function() {
      const cfg = {
        foo: "bar",
        baz: "quux",
        quux: "foo",
      ...
      return expect(config.updateWithPluginValues(cfg, overrides)).to.deep.eq({
        foo: "bar",
        baz: "baz",
        lol: 1234,
        quux: ["bar", "quux"],
    ...
    it("keeps the list of browsers if the plugins returns empty object", function() {
      const browser = {
    ...
    it("catches browsers=null returned from plugins", function() {
      const browser = {
        name: "fake browser name",
        family: "chromium",
        displayName: "My browser",
      ...
      return expect(errors.throw).to.have.been.calledWith("CONFIG_VALIDATION_ERROR");
    });

    return it("allows user to filter browsers", function() {
      const browserOne = {
        name: "fake browser name",
        family: "chromium",
        displayName: "My browser",
      ...
      return expect(updated, "all values").to.deep.eq({
        browsers: [browserTwo],
        resolved: {
          browsers: {
            value: [browserTwo],
  ...
  context(".parseEnv", () => it("merges together env from config, env from file, env from process, and env from CLI", function() {
    sinon.stub(config, "getProcessEnvVars").returns({
      version: "0.12.1",
      user: "bob",
    });
    ...
    return expect(config.parseEnv(obj, envCLI)).to.deep.eq({
      version: "0.14.0",
      project: "pie",
      host: "http://localhost:8888",
      user: "bob",
  ...
  context(".getProcessEnvVars", function() {
    ["cypress_", "CYPRESS_"].forEach(key => it(`reduces key: ${key}`, function() {
      const obj = {
      ...
      return expect(config.getProcessEnvVars(obj)).to.deep.eq({
        host: "http://localhost:8888",
        version: "0.12.0"
      });
    }));

    return it("does not merge reserved environment variables", function() {
      const obj = {
  ...
  context(".setUrls", function() {
    it("does not mutate existing obj", function() {
      const obj = {};
      return expect(config.setUrls(obj)).not.to.eq(obj);
    });

    it("uses baseUrl when set", function() {
      const obj = {
        port: 65432,
      ...
      return expect(urls.proxyUrl).to.eq("http://localhost:65432");
    });

    return it("strips baseUrl to host when set", function() {
      const obj = {
        port: 65432,
  ...
  context(".setScaffoldPaths", () => it("sets integrationExamplePath + integrationExampleName + scaffoldedFiles", function() {
    const obj = {
      integrationFolder: "/_test-output/path/to/project/cypress/integration"
    };
    sinon.stub(scaffold, "fileTree").resolves([]);

    return config.setScaffoldPaths(obj).then(result => expect(result).to.deep.eq({
      integrationFolder: "/_test-output/path/to/project/cypress/integration",
  ...
  context(".setSupportFileAndFolder", function() {
    it("does nothing if supportFile is falsey", function() {
      const obj = {
    ...
    it("sets the full path to the supportFile and supportFolder if it exists", function() {
    ...
    it("sets the supportFile to default index.js if it does not exist, support folder does not exist, and supportFile is the default", function() {
    ...
    it("sets the supportFile to false if it does not exist, support folder exists, and supportFile is the default", function() {
    ...
    it("throws error if supportFile is not default and does not exist", function() {
      const projectRoot = process.cwd();
      ...
      return config.setSupportFileAndFolder(obj)
      .catch(err => expect(err.message).to.include("The support file is missing or invalid."));
    });

    it("sets the supportFile to index.ts if it exists (without ts require hook)", function() {
    ...
    return it("uses custom TS supportFile if it exists (without ts require hook)", function() {
  ...
  context(".setPluginsFile", function() {
    it("does nothing if pluginsFile is falsey", function() {
      const obj = {
        projectRoot: "/_test-output/path/to/project"
      };
    ...
    it("sets the pluginsFile to default index.js if does not exist", function() {
      const projectRoot = path.join(process.cwd(), "test/support/fixtures/projects/no-scaffolding");
    ...
    it("sets the pluginsFile to index.ts if it exists", function() {
      const projectRoot = path.join(process.cwd(), "test/support/fixtures/projects/ts-proj");
    ...
    it("sets the pluginsFile to index.ts if it exists (without ts require hook)", function() {
      const projectRoot = path.join(process.cwd(), "test/support/fixtures/projects/ts-proj");
    ...
    it("set the pluginsFile to false if it does not exist, plugins folder exists, and pluginsFile is the default", function() {
      const projectRoot = path.join(process.cwd(), "test/support/fixtures/projects/empty-folders");
    ...
    it("throws error if pluginsFile is not default and does not exist", function() {
      const projectRoot = process.cwd();
      ...
      return config.setPluginsFile(obj)
      .catch(err => expect(err.message).to.include("The plugins file is missing or invalid."));
    });

    return it("uses custom TS pluginsFile if it exists (without ts require hook)", function() {
      const projectRoot = path.join(process.cwd(), "test/support/fixtures/projects/ts-proj-custom-names");
  ...
  context(".setParentTestsPaths", function() {
    it("sets parentTestsFolder and parentTestsFolderDisplay", function() {
      const obj = {
        projectRoot:       "/_test-output/path/to/project",
        integrationFolder: "/_test-output/path/to/project/cypress/integration"
      };

      return expect(config.setParentTestsPaths(obj)).to.deep.eq({
        projectRoot:       "/_test-output/path/to/project",
        integrationFolder: "/_test-output/path/to/project/cypress/integration",
    ...
    return it("sets parentTestsFolderDisplay to parentTestsFolder if they are the same", function() {
      const obj = {
        projectRoot:       "/_test-output/path/to/project",
  ...
  context(".setAbsolutePaths", function() {
    it("is noop without projectRoot", () => expect(config.setAbsolutePaths({})).to.deep.eq({}));
    ...
    it("does not mutate existing obj", function() {
      const obj = {};
      return expect(config.setAbsolutePaths(obj)).not.to.eq(obj);
    });

    it("ignores non special *folder properties", function() {
      const obj = {
        projectRoot: "/_test-output/path/to/project",
      ...
      return expect(config.setAbsolutePaths(obj)).to.deep.eq(obj);
    });

    return ["fileServerFolder", "fixturesFolder", "integrationFolder", "unitFolder", "supportFile", "pluginsFile"].forEach(folder => it(`converts relative ${folder} to absolute path`, function() {
      const obj = {
        projectRoot: "/_test-output/path/to/project"
      };
      obj[folder] = "foo/bar";
      ...
      return expect(config.setAbsolutePaths(obj)).to.deep.eq(expected);
    }));
  });

  return context(".setNodeBinary", function() {
    beforeEach(function() {
      this.findSystemNode = sinon.stub(findSystemNode, "findNodePathAndVersion");
      return this.nodeVersion = process.versions.node;
    });
    ...
    return it("sets current Node ver and warns if nodeVersion = system and findNodePathAndVersion rejects", function() {
      const err = new Error();
      ...
      .then(obj => {
        ...
        return expect(obj.resolvedNodePath).to.be.undefined;
      });
    });
  });
});

describe("lib/util/config", () => context(".isDefault", function() {
  it("returns true if value is default value", function() {
    settings = {baseUrl: null};
    const defaults = {baseUrl: null};
    const resolved = {};
    const merged = config.setResolvedConfigValues(settings, defaults, resolved);
    return expect(configUtil.isDefault(merged, "baseUrl")).to.be.true;
  });

  return it("returns false if value is not default value", function() {
    settings = {baseUrl: null};
    const defaults = {baseUrl: "http://localhost:8080"};
    const resolved = {};
    const merged = config.setResolvedConfigValues(settings, defaults, resolved);
    return expect(configUtil.isDefault(merged, "baseUrl")).to.be.false;
  });
}));
