...
describe("lib/browsers/electron", function() {
  beforeEach(function() {
    ...
    return this.stubForOpen = function() {
  ...
  context(".open", function() {
    ...
    it("calls render with url, state, and options", function() {
      ...
      .then(() => {
        ...
        return expect(electron._render).to.be.calledWith(
          this.url,
    ...
    it("returns custom object emitter interface", function() {
      ...
      .then(obj => {
        ...
        return expect(obj.pid).to.deep.eq([ELECTRON_PID]);
      });
    });
    ...
    it("is noop when before:browser:launch yields null", function() {
      ...
      .then(() => {
        ...
        return expect(options).to.include.keys("onFocus", "onNewWindow", "onCrashed");
      });
    });
    ...
    it("it merges in user preferences without removing essential options", function() {
      ...
      .then(() => {
        ...
        return expect(options).to.include.keys("foo", "onFocus", "onNewWindow", "onCrashed");
      });
    });

    return it("installs supplied extensions from before:browser:launch and warns on failure", function() {
      plugins.has.returns(true);
      ...
      .then(() => {
        ...
        return expect(Windows.removeAllExtensions).to.be.calledTwice;
      });
    });
  });

  context("._launch", function() {
    beforeEach(function() {
      sinon.stub(menu, "set");
    ...
    it("sets menu.set whether or not its in headless mode", function() {
      ...
      .then(() => expect(menu.set).to.be.calledWith({withDevTools: true})).then(() => {
        menu.set.reset();

        return electron._launch(this.win, this.url, { show: false });
      }).then(() => expect(menu.set).not.to.be.called);
    });

    it("sets user agent if options.userAgent", function() {
      return electron._launch(this.win, this.url, this.options)
      .then(() => expect(electron._setUserAgent).not.to.be.called).then(() => {
        return electron._launch(this.win, this.url, {userAgent: "foo"});
      }).then(() => {
        return expect(electron._setUserAgent).to.be.calledWith(this.win.webContents, "foo");
      });
    });

    it("sets proxy if options.proxyServer", function() {
      return electron._launch(this.win, this.url, this.options)
      .then(() => expect(electron._setProxy).not.to.be.called).then(() => {
        return electron._launch(this.win, this.url, {proxyServer: "foo"});
      }).then(() => {
        return expect(electron._setProxy).to.be.calledWith(this.win.webContents, "foo");
      });
    });
    ...
    it("calls win.loadURL with url", function() {
      return electron._launch(this.win, this.url, this.options)
      .then(() => {
        return expect(this.win.loadURL).to.be.calledWith(this.url);
      });
    });
  ...
  context("._render", function() {
    ...
    it("creates window instance and calls launch with window", function() {
      ...
      .then(() => {
        ...
        return expect(electron._launch).to.be.calledWith(this.newWin, this.url, this.options);
      });
    });

    return it("registers onRequest automation middleware", function() {
      sinon.spy(this.automation, "use");

      return electron._render(this.url, this.options.projectRoot, this.automation, this.options)
      .then(() => {
        expect(this.automation.use).to.be.called;
        return expect(this.automation.use.lastCall.args[0].onRequest).to.be.a("function");
      });
    });
  });

  context("._defaultOptions", function() {
    ...
    it(".onFocus", function() {
      ...
      opts.onFocus();
      return expect(menu.set).not.to.be.called;
    });

    return describe(".onNewWindow", function() {
      beforeEach(function() {
      ...
      it("passes along event, url, parent window and options", function() {
        ...
        return expect(electron._launchChild).to.be.calledWith(
          event, this.url, parentWindow, this.options.projectRoot, this.state, this.options
        );
      });

      return it("adds pid of new BrowserWindow to pid list", function() {
        const opts = electron._defaultOptions(this.options.projectRoot, this.state, this.options);
        ...
        .then(() => {
          ...
          .then(() => expect(instance.pid).to.deep.eq([ELECTRON_PID, NEW_WINDOW_PID]));
        });
      });
    });
  });
  ...
  context.skip("._launchChild", function() {
    beforeEach(function() {
      ...
      this.win.getPosition = () => [4, 2];

      return this.openNewWindow = options => {
    ...
    it("prevents default", function() {
      return this.openNewWindow().then(() => {
        return expect(this.event.preventDefault).to.be.called;
      });
    });
    ...
    it("creates child window", function() {
      return this.openNewWindow().then(() => {
        ...
        return expect(args.minHeight).to.equal(100);
      });
    });
    ...
    it("offsets it from parent by 100px", function() {
      return this.openNewWindow().then(() => {
        ...
        return expect(args.y).to.equal(102);
      });
    });
    ...
    it("passes along web security", function() {
      return this.openNewWindow({chromeWebSecurity: false}).then(() => {
        const args = Windows.create.lastCall.args[0];
        return expect(args.chromeWebSecurity).to.be.false;
      });
    });
    ...
    it("sets unique PROJECT type on each new window", function() {
      return this.openNewWindow().then(() => {
        ...
        return expect(firstArgs.type).not.to.equal(secondArgs.type);
      });
    });
    ...
    it("set newGuest on child window", function() {
      ...
      .then(() => Promise.delay(1)).then(() => {
        return expect(this.event.newGuest).to.equal(this.childWin);
      });
    });
    ...
    it("sets menu with dev tools on creation", function() {
      return this.openNewWindow().then(() => {
        ...
        return expect(menu.set).to.be.calledWith({withDevTools: true});
      });
    });
    ...
    it("it closes the child window when the parent window is closed", function() {
      ...
      .then(() => Promise.delay(1)).then(() => {
        this.win.emit("close");
        return expect(this.childWin.close).to.be.called;
      });
    });
    ...
    it("does not close the child window when it is already destroyed", function() {
      ...
      .then(() => Promise.delay(1)).then(() => {
        ...
        this.win.emit("close");
        return expect(this.childWin.close).not.to.be.called;
      });
    });

    return it("does the same things for children of the child window", function() {
      ...
      this.childWin.getPosition = () => [104, 102];
      ...
      return this.openNewWindow().then(() => {
        ...
        return expect(args.y).to.equal(202);
      });
    });
  });

  return context("._setProxy", () => it("sets proxy rules for webContents", function() {
    const webContents = {
      session: {
    ...
    .then(() => expect(webContents.session.setProxy).to.be.calledWith({
