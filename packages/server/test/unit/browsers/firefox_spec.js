...
describe("lib/browsers/firefox", function() {
  afterEach(() => mockfs.restore());
  return context("#open", function() {
    beforeEach(function() {
      mockfs({
        '/path/to/appData': {}
      });
      ...
      sinon.stub(Foxdriver, 'attach').resolves({listTabs() { return []; }});
    ...
    it("executes before:browser:launch if registered", function() {
      ...
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(plugins.execute).to.be.called;
      });
    });
    ...
    it("does not execute before:browser:launch if not registered", function() {
      ...
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(plugins.execute).not.to.be.called;
      });
    });
    ...
    it("uses default preferences if before:browser:launch returns falsy value", function() {
      ...
      plugins.execute.resolves(null);
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(FirefoxProfile.prototype.setPreference).to.be.calledWith("network.proxy.type", 1);
      });
    });
    ...
    it("sets preferences if returned by before:browser:launch", function() {
      ...
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(FirefoxProfile.prototype.setPreference).to.be.calledWith("foo", "bar");
      });
    });
    ...
    it("adds extensions returned by before:browser:launch, along with cypress extension", function() {
      ...
      return firefox.open(this.browser, "http://", this.options).then(() => {
        ...
        return expect(Marionette.Drivers.Tcp.prototype.send).calledWithMatch({name: 'Addon:Install', params: {path: "/path/to/user/ext"}});
      });
    });
    ...
    it("adds only cypress extension if before:browser:launch returns object with non-array extensions", function() {
      ...
      return firefox.open(this.browser, "http://", this.options).then(() => {
        expect(Marionette.Drivers.Tcp.prototype.send).calledWithMatch({name: 'Addon:Install', params: {path: "/path/to/ext"}});
        return expect(Marionette.Drivers.Tcp.prototype.send).not.calledWithMatch({name: 'Addon:Install', params: {path: "/path/to/user/ext"}});
      });
    });
    ...
    it("sets user-agent preference if specified", function() {
      ...
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(FirefoxProfile.prototype.setPreference).to.be.calledWith("general.useragent.override", "User Agent");
      });
    });

    it("writes extension", function() {
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(utils.writeExtension).to.be.calledWith(this.options.browser, this.options.isTextTerminal, this.options.proxyUrl, this.options.socketIoRoute, this.options.onScreencastFrame);
      });
    });
    ...
    it.skip("finds remote port for firefox debugger", function() {
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(firefoxUtil.findRemotePort).to.be.called;
      });
    });
    ...
    it("sets proxy-related preferences if specified", function() {
      ...
      return firefox.open(this.browser, "http://", this.options).then(() => {
        ...
        return expect(FirefoxProfile.prototype.setPreference).to.be.calledWith("network.proxy.no_proxies_on");
      });
    });
    ...
    it("does not set proxy-related preferences if not specified", function() {
      return firefox.open(this.browser, "http://", this.options).then(() => {
        ...
        return expect(FirefoxProfile.prototype.setPreference).not.to.be.calledWith("network.proxy.no_proxies_on");
      });
    });

    it("updates the preferences", function() {
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(FirefoxProfile.prototype.updatePreferences).to.be.called;
      });
    });

    it("launches with the url and args", function() {
      return firefox.open(this.browser, "http://", this.options).then(() => {
        return expect(utils.launch).to.be.calledWith(this.browser, null, [
          "-marionette",
          "-new-instance",
          "-foreground",
    ...
    it("resolves the browser instance", function() {
      return firefox.open(this.browser, "http://", this.options).then(result => {
        return expect(result).to.equal(this.browserInstance);
      });
    });
    ...
    it("does not clear user profile if already exists", function() {
      ...
      return firefox.open(this.browser, 'http://', this.options).then(() => expect(specUtil.getFsPath('/path/to/appData/firefox/interactive')).containSubset({
        'xulstore.json': '[foo xulstore.json]',
        'chrome': {'userChrome.css': '[foo userChrome.css]'}
      }));
    });
    ...
    it('creates chrome/userChrome.css if not exist', function() {
      return firefox.open(this.browser, 'http://', this.options).then(() => expect(specUtil.getFsPath('/path/to/appData/firefox/interactive/chrome/userChrome.css')).ok);
    });


    return it("clears browser cache", function() {
      mockfs({
        '/path/to/appData/firefox/interactive/': {
