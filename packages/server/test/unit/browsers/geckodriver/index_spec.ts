...
describe('lib/browsers/geckodriver', () => {
  ...
  describe('GeckoDriver.create', () => {
    ...
    it('sends the correct arguments to the session if running headfully in run mode', async () => {
      geckoDriverMockStart.resolves(geckoDriverMockProcess)
      waitPortPackageStub.resolves()
      const newSessionScope = nockContext.post('/session', {
        capabilities: {
          alwaysMatch: {
            acceptInsecureCerts: true,
            // no headless option here as we are in run mode running headfully
          },
        },
      }).reply(200, {
        value: {
      ...
      // we are running headfully in run mode
      mockOpts.isTextTerminal = true
      mockOpts.isHeadless = false
