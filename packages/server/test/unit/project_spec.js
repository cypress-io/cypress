...
describe('lib/project', () => {
  ...
  context('#watchSettings', () => {
    ...
    it('sets onChange event when {changeEvents: true}', function () {
      const options = { onSettingsChanged: sinon.spy() }

      this.project.watchSettingsAndStartWebsockets(options)
      ...
      expect(this.project.server.onConfigurationChange).to.be.calledWith('fake/file/path')
    })
  ...
  context('#watchPluginsFile', () => {
    beforeEach(function () {
      ...
      this.project.server = { onConfigurationChange: sinon.stub() }
      sinon.stub(plugins, 'init').resolves()
    ...
    it('calls onPluginsChanged and onConfigurationChange with file path when file changes', function () {
      const options = { onPluginsChanged: sinon.stub() }
