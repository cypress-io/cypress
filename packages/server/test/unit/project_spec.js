...
describe("lib/project", function() {
  ...
  context("#getConfig", function() {
    ...
    it("calls config.get with projectRoot + options + saved state", function() {
      ...
      .then(state => {
        ...
        .then(cfg => expect(cfg).to.deep.eq({
          integrationFolder,
          isNewProject: false,
          baz: "quux",
          state: {
  ...
  context("#open", function() {
    ...
    return it("updates config.state when saved state changes", function() {
  ...
  context(".add", function() {
    ...
    describe("if project at path has id", () => it("returns object containing path and id", function() {
      sinon.stub(settings, "read").resolves({projectId: "id-123"});
    ...
    return describe("if project at path does not have id", () => it("returns object containing just the path", function() {
  ...
  context(".remove", function() {
    ...
    return it("calls cache.removeProject with path", () => Project.remove("/_test-output/path/to/project").then(() => expect(cache.removeProject).to.be.calledWith("/_test-output/path/to/project")));
  });

  context(".id", () => it("returns project id", function() {
  ...
  context(".getOrgs", function() {
    ...
    return it("calls api.getOrgs", () => Project.getOrgs().then(function(orgs) {
  ...
  context(".paths", function() {
    ...
    return it("calls cache.getProjectRoots", () => Project.paths().then(function(ret) {
  ...
  context(".getPathsAndIds", function() {
    ...
    return it("returns array of objects with paths and ids", () => Project.getPathsAndIds().then(pathsAndIds => expect(pathsAndIds).to.eql([
      {
        path: "/path/to/first",
        id: "id-123"
      },
