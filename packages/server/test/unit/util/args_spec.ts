...
describe('lib/util/args', () => {
  ...
  describe('--spec', () => {
    ...
    it('throws if argument cannot be parsed', function () {
      ...
      try {
        ...
        return snapshot('invalid spec error', stripAnsi((err as Error).message))
      }
    })
  ...
  describe('--auto-cancel-after-failures', () => {
    ...
    it('throws error when a string is set', function () {
      try {
        ...
        return snapshot('invalid --auto-cancel-after-failures error', stripAnsi((err as Error).message))
      }
    })
    ...
    it('throws error when true is set', function () {
      try {
        ...
        return snapshot('invalid --auto-cancel-after-failures (true) error', stripAnsi((err as Error).message))
      }
    })
    ...
    it('throws error when a negative value is set', function () {
      try {
        ...
        return snapshot('invalid --auto-cancel-after-failures (negative value) error', stripAnsi((err as Error).message))
      }
    })
    ...
    it('throws error when a decimal value is set', function () {
      try {
        ...
        return snapshot('invalid --auto-cancel-after-failures (decimal value) error', stripAnsi((err as Error).message))
      }
    })
  })
  ...
  describe('--env', () => {
    ...
    it('throws if env string cannot be parsed', function () {
      ...
      try {
        ...
        return snapshot('invalid env error', stripAnsi((err as Error).message))
      }
    })
  ...
  describe('--reporterOptions', () => {
    ...
    it('throws if reporter string cannot be parsed', function () {
      ...
      try {
        ...
        return snapshot('invalid reporter options error', stripAnsi((err as Error).message))
      }
    })
  })
  ...
  describe('--config', () => {
    ...
    it('allows config properties', function () {
      ...
      expect(e2e.supportFile).to.eq('path/to/support_file')
      expect(component.supportFile).to.eq('path/to/support_file')
    ...
    it('throws if config string cannot be parsed', function () {
      ...
      try {
        ...
        return snapshot('invalid config error', stripAnsi((err as Error).message))
      }
    })
  })
  ...
  describe('.toObject', () => {
    ...
    it('coerces booleans', function () {
      expect(toObject(['--foo=true']).foo).be.true
    ...
    it('can transpose back to an array', function () {
      ...
      expect(toObject(args)).to.deep.eq({
        cwd: getCwd(),
        _: [],
        invokedFromCli: true,
        config,
  ...
  describe('--updating', () => {
    ...
    it('slurps up appPath + execPath if updating and these are omitted', () => {
      ...
      expect(toObject(argv)).to.deep.eq({
        cwd: getCwd(),
        _: [
          '/private/var/folders/wr/3xdzqnq16lz5r1j_xtl443580000gn/T/cypress/Cypress.app/Contents/MacOS/Cypress',
          '/Applications/Cypress.app',
  ...
  describe('with proxy', () => {
    beforeEach(function () {
      // originalEnv is set in spec_helper.js
