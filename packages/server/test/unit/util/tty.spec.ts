...
describe('lib/util/tty', () => {
  ...
  describe('.override', () => {
    ...
    it('is noop when not forcing in env', () => {
      ...
      expect(process.stdin.isTTY).toEqual('foo')
      expect(process.stdout.isTTY).toEqual('foo')

      expect(process.stderr.isTTY).toEqual('foo')
    })
    ...
    it('modifies isatty calls', () => {
      ...
      expect(isatty.mock.calls.length).toEqual(1)
