...
describe("lib/modes/run", function() {
  ...
  context(".getProjectId", function() {
    it("resolves if id", () => runMode.getProjectId("project", "id123")
  ...
  context(".openProjectCreate", function() {
    ...
    it("calls openProject.create with projectRoot + options", () => expect(openProject.create).to.be.calledWithMatch("/_test-output/path/to/project/foo", {
      port: 8080,
      projectRoot: "/_test-output/path/to/project/foo",
      env: {foo: "bar"}
    }, {
  ...
  context(".postProcessRecording", function() {
    ...
    return it("does not call video process if there are no failing tests and we have set not to upload video on passing", function() {
      const endVideoCapture = () => Promise.resolve();
  ...
  context(".waitForBrowserToConnect", () => it("throws TESTS_DID_NOT_START_FAILED after 3 connection attempts", function() {
  ...
  context(".waitForTestsToFinishRunning", function() {
    ...
    it("end event resolves with obj, displays stats, displays screenshots, sets video timestamps", function() {
      ...
      .then(function(obj) {
        ...
        return expect(setVideoTimestamp).calledWithMatch(startedVideoCapture, results.tests);
      });
    });
  ...
  context(".listenForProjectEnd", function() {
    it("resolves with end event + argument", function() {
      ...
      .then(obj => expect(obj).to.deep.eq({
        foo: "bar"
      }));
    });
  ...
  context(".run browser vs video recording", function() {
    ...
    it("shows no warnings for default browser", () => runMode.run()
    .then(() => expect(errors.warning).to.not.be.called));

    it("shows no warnings for electron browser", () => runMode.run({browser: "electron"})
    .then(() => expect(errors.warning).to.not.be.calledWith("CANNOT_RECORD_VIDEO_FOR_THIS_BROWSER")));

    it("disables video recording on interactive mode runs", () => runMode.run({headed: true})
    .then(() => expect(errors.warning).to.be.calledWith("CANNOT_RECORD_VIDEO_HEADED")));

    it("disables video recording for non-electron browser", () => runMode.run({browser: "chrome"})
    .then(() => expect(errors.warning).to.be.calledWith("CANNOT_RECORD_VIDEO_FOR_THIS_BROWSER")));

    return it("names video file with spec name", () => runMode.run()
    .then(() => {
  ...
  return context(".run", function() {
    ...
    it("no longer ensures user session", () => runMode.run()
    .then(() => expect(user.ensureAuthToken).not.to.be.called));

    it("resolves with object and totalFailed", () => runMode.run()
    ...
    it("passes headed to openProject.launch", function() {
      ...
      .then(() => expect(openProject.launch).to.be.calledWithMatch(
        browser,
        {
          name: "foo_spec.js",
          path: "cypress/integration/foo_spec.js",
    ...
    it("passes sys to runSpecs", () => runMode.run()
    .then(() => expect(runMode.runSpecs).to.be.calledWithMatch({
      sys: {
        osName: "osFoo",
    ...
    return it("passes browser to runSpecs", () => runMode.run()
    .then(() => expect(runMode.runSpecs).to.be.calledWithMatch({
      browser: {
        name: "fooBrowser",
        path: "path/to/browser",
