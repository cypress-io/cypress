...
describe('lib/saved_state', () => {
  context('#create', () => {
    ...
    it('resolves with a noop instance if isTextTerminal', () => {
      return savedState.create('/foo/bar', true).then((state) => {
        expect(state).to.equal(FileUtil.noopFile)
      })
    })

    it('caches state file instance per path', () => {
      return Promise.all([savedState.create('/foo/bar'), savedState.create('/foo/bar')]).spread((a, b) => {
        expect(a).to.equal(b)
      })
    })
    ...
    it('sets path to project name + hash if projectRoot', () => {
      return savedState.create('/foo/the-project-name').then((state) => {
