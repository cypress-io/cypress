...
describe('lib/util/status_code', () => {
  ...
  describe('.getText', () => {
    it('is OK', () => {
      expect(statusCode.getText(200)).toEqual('OK')
    })

    it('is Not Found', () => {
      expect(statusCode.getText(404)).toEqual('Not Found')
    })

    it('is Server Error', () => {
      expect(statusCode.getText(500)).toEqual('Internal Server Error')
    })

    it('is Unknown Status Code', () => {
      expect(statusCode.getText(1234)).toEqual('Unknown Status Code')
    })
  })
})
