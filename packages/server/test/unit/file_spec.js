...
describe("lib/util/file", function() {
  beforeEach(function() {
    this.dir = path.join(os.tmpdir(), "cypress", "file_spec");
    this.path = path.join(this.dir, "file.json");

    return fs.removeAsync(this.dir).catch(function() {});
  });
      //# ignore error if directory didn't exist in the first place

  it("throws if path is not specified", () => expect(() => new FileUtil()).to.throw("Must specify path to file when creating new FileUtil()"));
  ...
  context("#transaction", function() {
    beforeEach(function() {
      return this.fileUtil = new FileUtil({path: this.path});
    });

    return it("ensures returned promise completely resolves before moving on with queue", function() {
      return Promise.all([
      ...
      .then(() => {
        return this.fileUtil.transaction(tx => {
          return tx.get("items").then(items => expect(items).to.eql(["foo", "bar", "baz"]));
        });
      });
    });
  });
  ...
  context("#get", function() {
    ...
    it("resolves entire object if given no key", function() {
      return this.fileUtil.get().then(contents => expect(contents).to.eql({}));
    });
    ...
    it("resolves default value if given key is undefined", function() {
      return this.fileUtil.get("foo", "default").then(value => expect(value).to.equal("default"));
    });

    it("resolves undefined if value is undefined", function() {
      return this.fileUtil.get("foo").then(value => expect(value).to.be.undefined);
    });
    ...
    it("resolves cached contents when it can't get lock on file after an initial read", function() {
      ...
      .then(() => {
        sinon.stub(lockFile, "lockAsync").rejects({name: "", message: "", code: "EEXIST"});
        return this.fileUtil.get();
    }).then(contents => expect(contents).to.eql({foo: "bar"}));
    });
    ...
    it("debounces reading from disk", function() {
      ...
      .then(() => expect(fs.readJsonAsync).to.be.calledOnce);
    });

    it("locks file while reading", function() {
      sinon.spy(lockFile, "lockAsync");
      return this.fileUtil.get().then(() => expect(lockFile.lockAsync).to.be.called);
    });

    it("unlocks file when finished reading", function() {
      sinon.spy(lockFile, "unlockAsync");
      return this.fileUtil.get().then(() => expect(lockFile.unlockAsync).to.be.called);
    });

    it("unlocks file even if reading fails", function() {
      sinon.spy(lockFile, "unlockAsync");
      sinon.stub(fs, "readJsonAsync").rejects(new Error("fail!"));
      return this.fileUtil.get().catch(() => expect(lockFile.unlockAsync).to.be.called);
    });

    return it("times out and carries on if unlocking times out", function() {
  ...
  context("#set", function() {
    ...
    it("throws if 1st argument is not a string or plain object", function() {
      expect(() => this.fileUtil.set(1)).to.throw("Expected `key` to be of type `string` or `object`, got `number`");
      return expect(() => this.fileUtil.set([])).to.throw("Expected `key` to be of type `string` or `object`, got `array`");
    });
    ...
    }).then(contents => expect(JSON.parse(contents)).to.eql({foo: "bar"}));
    });

    it("locks file while writing", function() {
      sinon.spy(lockFile, "lockAsync");
      return this.fileUtil.set("foo", "bar").then(() => expect(lockFile.lockAsync).to.be.called);
    });

    it("unlocks file when finished writing", function() {
      sinon.spy(lockFile, "unlockAsync");
      return this.fileUtil.set("foo", "bar").then(() => expect(lockFile.unlockAsync).to.be.called);
    });

    return it("unlocks file even if writing fails", function() {
      sinon.spy(lockFile, "unlockAsync");
      sinon.stub(fs, "outputJsonAsync").rejects(new Error("fail!"));
      return this.fileUtil.set("foo", "bar").catch(() => expect(lockFile.unlockAsync).to.be.called);
    });
  });

  return context("#remove", function() {
    beforeEach(function() {
      return this.fileUtil = new FileUtil({path: this.path});
    });
    ...
    it("locks file while removing", function() {
      sinon.spy(lockFile, "lockAsync");
      return this.fileUtil.remove().then(() => expect(lockFile.lockAsync).to.be.called);
    });

    it("unlocks file when finished removing", function() {
      sinon.spy(lockFile, "unlockAsync");
      return this.fileUtil.remove()
      .then(() => expect(lockFile.unlockAsync).to.be.called);
    });

    return it("unlocks file even if removing fails", function() {
      sinon.spy(lockFile, "unlockAsync");
      sinon.stub(fs, "removeAsync").rejects(new Error("fail!"));

      return this.fileUtil.remove()
      .then(function() {
        throw new Error("should have caught!");}).catch(function(err) {
        expect(err.message).to.eq("fail!");
        return expect(lockFile.unlockAsync).to.be.called;
      });
    });
  });
});
