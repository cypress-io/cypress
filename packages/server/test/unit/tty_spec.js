...
describe("lib/util/tty", function() {
  context("getWindowSize", function() {
    ...
    return it("polyfills stdout and stderr getWindowSize", function() {
      ...
      return expect(base.window.width).to.equal(10);
    });
  });

  return context(".override", function() {
    beforeEach(function() {
      ...
      return process.stderr.isTTY = 'foo';
    });
    ...
    afterEach(function() {
      ...
      return process.stderr.isTTY = ttys[2];});

    it("is noop when not forcing in env", function() {
      ...
      return expect(process.stderr.isTTY).to.eq('foo');
    });

    it("forces process.stderr.isTTY to be true", function() {
      ttyUtil.override();

      expect(process.stdin.isTTY).to.be.true;
      expect(process.stdout.isTTY).to.be.true;
      return expect(process.stderr.isTTY).to.be.true;
    });

    return it("modifies isatty calls", function() {
      delete process.env.FORCE_STDERR_TTY;
      ...
      return expect(isatty.firstCall).to.be.calledWith(2);
    });
  });
});
