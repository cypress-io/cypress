...
const expectsName = name => expect(ciProvider.provider(), "CI providers detected name").to.eq(name);

const expectsCiParams = params => expect(ciProvider.ciParams(), "CI providers detected CI params").to.deep.eq(params);

const expectsCommitParams = params => expect(ciProvider.commitParams(), "CI providers detected commit params").to.deep.eq(params);

const expectsCommitDefaults = function(existing, expected) {
  expect(expected).to.be.an("object");
  return expect(ciProvider.commitDefaults(existing), "CI providers default git params").to.deep.eq(expected);
};

describe("lib/util/ci_provider", function() {
  let resetEnv = null;

  afterEach(() => // we need to reset environment
  // to avoid affecting tests in other suites
  typeof resetEnv === 'function' ? resetEnv() : undefined);

  it("null when unknown", function() {
  ...
  it("does not extract from commit environment variables yet", function() {
