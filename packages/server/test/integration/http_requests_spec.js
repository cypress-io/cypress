...
const replaceAbsolutePaths = content => content.replace(absolutePathRegex, "\"/<path-to-project>");

const removeSourceMap = content => content.replace(sourceMapRegex, ";");
...
const cleanResponseBody = body => replaceAbsolutePaths(removeWhitespace(body));

const trimString = s => _.trim(s);
const nonEmpty = s => !_.isEmpty(s);

sourceMapRegex = /\n\/\/# sourceMappingURL\=.*/;

const removeSourceMapContents = fileContents => fileContents.replace(sourceMapRegex, ";");

const browserifyFile = filePath => streamToPromise(
  browserify({
    entries: [filePath],
...
const browserifyFileTs = filePath => streamToPromise(
  browserify(filePath)
  .transform(coffeeify)
...
describe("Routes", function() {
  ...
  beforeEach(function() {
    ...
    return this.setup = (initialUrl, obj = {}) => {
      if (_.isObject(initialUrl)) {
        obj = initialUrl;
        initialUrl = null;
      }

      if (!obj.projectRoot) { obj.projectRoot = "/foo/bar/"; }
        ...
        const open = () => {
          ...
          return Promise.all([
            ...
            .spread(port => {
              ...
              return this.proxy = "http://localhost:" + port;
            })
          ]);
        };
  ...
  context("GET /", function() {
    ...
    it("redirects to config.clientRoute without a _remoteOrigin and without a proxy", function() {
      ...
      .then(function(res) {
        ...
        return expect(res.headers.location).to.eq("/__/");
      });
    });
    ...
    it("does not redirect with _remoteOrigin set", function() {
      ...
      .then(() => {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("</html>");
        });
      });
    });
    ...
    it("does not redirect when visiting http site which isnt cypress server", function() {
      ...
      .then(function(res) {
        expect(res.statusCode).to.eq(200);
        return expect(res.headers.location).not.to.eq("/__/");
      });
    });

    return it("proxies through https", function() {
      ...
      .then(() => {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("<body>https server</body>");
        });
      });
    });
  });

  context("GET /__", function() {
    beforeEach(function() {
    ...
    it("routes config.clientRoute to serve cypress client app html", function() {
      ...
      .then(function(res) {
        expect(res.statusCode).to.eq(200);
        return expect(res.body).to.match(/Runner.start/);
      });
    });
    ...
    it("sets title to projectName", function() {
      ...
      .then(function(res) {
        expect(res.statusCode).to.eq(200);
        return expect(res.body).to.include("<title>foobarbaz</title>");
      });
    });
    ...
    it("omits x-powered-by", function() {
      ...
      .then(function(res) {
        expect(res.statusCode).to.eq(200);
        return expect(res.headers["x-powered-by"]).not.to.exist;
      });
    });
    ...
    it("clientRoute routes to 'not launched through Cypress' without a proxy set", function() {
      ...
      .then(function(res) {
        expect(res.statusCode).to.eq(200);
        return expect(res.body).to.match(/This browser was not launched through Cypress\./);
      });
    });
    ...
    return it("sends Cypress.version", function() {
      return this.setup({baseUrl: "http://localhost:9999/app"})
      .then(() => {
        return this.rp("http://localhost:9999/__")
        .then(function(res) {
          ...
          return expect(configStr).to.include(pkg.version);
        });
      });
    });
  });

  context("GET /__cypress/runner/*", function() {
    beforeEach(function() {
    ...
    it("can get cypress_runner.js", function() {
      ...
      .then(function(res) {
        expect(res.statusCode).to.eq(200);
        return expect(res.body).to.match(/React/);
      });
    });

    return it("can get cypress_runner.css", function() {
      return this.rp("http://localhost:8443/__cypress/runner/cypress_runner.css")
      .then(function(res) {
        expect(res.statusCode).to.eq(200);
        return expect(res.body).to.match(/spec-iframe/);
      });
    });
  });

  context("GET /__cypress/files", function() {
    beforeEach(function() {
      Fixtures.scaffold("todos");
      return Fixtures.scaffold("ids");
    });

    describe("todos with specific configuration", function() {
      beforeEach(function() {
        return this.setup({
      ...
      return it("returns base json file path objects of only tests", function() {
        ...
        .then(files => {
          ...
          .then(files => {
            ...
            .then(function(res) {
              ...
              body.integration = _.map(body.integration, obj => _.pick(obj, "name", "relative"));

              return expect(res.body).to.deep.eq({
                integration: [
                  {
                    "name": "sub/a&b%c.js",
    ...
    return describe("ids with regular configuration", function() {
      ...
      return it("can ignore other files as well", function() {
        return this.setup({
          projectRoot: Fixtures.projectPath("ids"),
          config: {
  ...
  context("GET /__cypress/tests", function() {
    describe("ids with typescript", function() {
      beforeEach(function() {
      ...
      it("processes foo.coffee spec", function() {
        ...
        .then(function(res) {
          ...
          .then(file => checkTranspilation(res.body, file));
        });
      });
      ...
      it("processes dom.jsx spec", function() {
        ...
        .then(function(res) {
          ...
          .then(function(file) {
            ...
            return expect(res.body).to.include("React.createElement(");
          });
        });
      });

      return it("serves error javascript file when the file is missing", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("Cannot find module");
        });
      });
    });

    describe("ids without typescript", function() {
      beforeEach(function() {
        Fixtures.scaffold("ids");

        //# remove cached options
        delete require.cache[require.resolve("@cypress/browserify-preprocessor")];
        sinon.stub(resolve, 'typescript').callsFake(() => null);
      ...
      it("processes foo.coffee spec", function() {
        ...
        .then(function(res) {
          ...
          .then(file => expect(removeSourceMap(res.body)).to.equal(file.toString()));
        });
      });
    ...
    describe("failures", function() {
      ...
      return it("serves error javascript file when there's a syntax error", function() {
        return this.rp("http://localhost:2020/__cypress/tests?p=cypress/integration/syntax_error.coffee")
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          expect(res.body).to.include('Cypress.action("spec:script:error", {');
          return expect(res.body).to.include("ParseError");
        });
      });
    });

    return describe("no-server", function() {
      beforeEach(function() {
      ...
      return it("processes helpers/includes.js javascripts", function() {
  ...
  context("ALL /__cypress/xhrs/*", function() {
    beforeEach(function() {
      return this.setup();
    });
    ...
    describe("delay", function() {
      it("can set delay to 10ms", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(delay).to.be.calledWith(10);
        });
      });

      return it("does not call Promise.delay when no delay", function() {
        const delay = sinon.spy(Promise, "delay");

        return this.rp("http://localhost:2020/__cypress/xhrs/users/1")
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(delay).not.to.be.called;
        });
      });
    });

    describe("status", () => it("can set status", function() {
      return this.rp({
        url: "http://localhost:2020/__cypress/xhrs/users/1",
        headers: {
      ...
      .then(res => expect(res.statusCode).to.eq(401));
    }));
    ...
    describe("headers", function() {
      it("can set headers", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers["x-token"]).to.eq("123");
        });
      });

      return it("sets headers from response type", function() {
        const headers = JSON.stringify({
          "x-token": "123"
        });
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.deep.eq({foo: "bar"});
        });
      });
    });
    ...
    return describe("response", function() {
      it("sets response to json", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          expect(res.headers["content-type"]).to.match(/application\/json/);
          return expect(res.body).to.deep.eq([1,2,3]);
        });
      });
      ...
      it("sets response to text/html", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.eq("<html>foo</html>");
        });
      });
      ...
      it("sets response to text/plain", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          expect(res.headers["content-type"]).to.match(/text\/plain/);
          return expect(res.body).to.eq("foobarbaz");
        });
      });
      ...
      it("decodes responses", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.deep.eq({test: "We’ll"});
        });
      });
      ...
      context("fixture", function() {
        ...
        it("returns fixture contents", function() {
          ...
          .then(function(res) {
            expect(res.statusCode).to.eq(200);
            expect(res.headers["content-type"]).to.match(/application\/json/);
            return expect(res.body).to.deep.eq([{json: true}]);
          });
        });
        ...
        it("returns __error on fixture errors", function() {
          ...
          .then(function(res) {
            ...
            expect(res.headers["content-type"]).to.match(/application\/json/);
            return expect(res.body.__error).to.include("'bad_json' is not valid JSON.");
          });
        });

        return it("can change the fixture encoding", function() {
          ...
          .then(bin => {
            ...
            .then(function(res) {
              ...
              return expect(res.headers["content-length"]).to.eq("" + bin.length);
            });
          });
        });
      });

      context("PUT", () => it("can issue PUT requests", function() {
        return this.rp({
          method: "put",
          url: "http://localhost:2020/__cypress/xhrs/users/1",
          json: true,
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          expect(res.headers["content-type"]).to.match(/application\/json/);
          return expect(res.body).to.deep.eq({id: 123, name: "brian"});
        });
      }));

      context("POST", () => it("can issue POST requests", function() {
        return this.rp({
          method: "post",
          url: "http://localhost:2020/__cypress/xhrs/users/1",
          json: true,
      ...
      context("HEAD", () => it("can issue PUT requests", function() {
        return this.rp({
          method: "head",
          url: "http://localhost:2020/__cypress/xhrs/users/1"
        })
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers["content-type"]).to.match(/text\/plain/);
        });
      }));

      return context("DELETE", () => it("can issue DELETE requests", function() {
        return this.rp({
          method: "delete",
          url: "http://localhost:2020/__cypress/xhrs/users/1"
        })
  ...
  context("GET /__cypress/iframes/*", function() {
    beforeEach(function() {
    ...
    describe("todos", function() {
      ...
      it("renders iframe with variables passed in", function() {
        ...
        .then(function(res) {
          ...
          return expect(body).to.eq(contents);
        });
      });

      return it("can send back all tests", function() {
    ...
    describe("no-server", function() {
      ...
      return it("renders iframe with variables passed in", function() {
    ...
    describe("no-server with supportFile: false", function() {
      beforeEach(function() {
        return this.setup({
          projectRoot: Fixtures.projectPath("no-server"),
          config: {
      ...
      return it("renders iframe without support file", function() {
    ...
    describe("e2e", function() {
      ...
      return it("omits support directories", function() {
  ...
  context("GET *", function() {
    context("basic request", function() {
      ...
      return it("basic 200 html response", function() {
        nock(this.server._remoteOrigin)
        .get("/")
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.include("hello from bar!");
        });
      });
    });
    ...
    context("gzip", function() {
      ...
      return it("can ungzip utf-8 javascript and inject without corrupting it", function() {
        let js = "";
        ...
        return Promise.fromCallback(cb => {
          ...
          .then(function(res) {
            expect(res.statusCode).to.eq(200);
            return expect(res.body).to.deep.eq(js);
          });
        }).finally(() => {
          return Promise.fromCallback(cb => {
    ...
    context("accept-encoding", function() {
      beforeEach(function() {
        return this.setup("http://www.github.com");
      });
      ...
      it("strips unsupported deflate and br encoding", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.eq("<html>accept</html>");
        });
      });

      return it("removes accept-encoding when nothing is supported", function() {
        nock(this.server._remoteOrigin, {
    ...
    context("304 Not Modified", function() {
      beforeEach(function() {
        return this.setup("http://localhost:8080");
      });

      return it("sends back a 304", function() {
        nock(this.server._remoteOrigin)
        .get("/assets/app.js")
        .reply(304);
        ...
        .then(res => expect(res.statusCode).to.eq(304));
      });
    });
    ...
    context("redirects", function() {
      ...
      it("passes the location header through", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers["location"]).to.eq("/");
        });
      });
      ...
      it("doesnt redirect with query params or hashes which werent in location header", function() {
        ...
        .then(res => {
          expect(res.statusCode).to.eq(302);
          expect(res.headers["set-cookie"]).to.include("__cypress.initial=true; Domain=getbootstrap.com; Path=/");
          return expect(res.headers["location"]).to.eq("/css");
        });
      });
      ...
      it("does redirect with query params if location header includes them", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(302);
          expect(res.headers["set-cookie"]).to.include("__cypress.initial=true; Domain=getbootstrap.com; Path=/");
          return expect(res.headers["location"]).to.eq("/css?q=search");
        });
      });
      ...
      it("does redirect with query params to external domain if location header includes them", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(302);
          expect(res.headers["set-cookie"]).to.include("__cypress.initial=true; Domain=getbootstrap.com; Path=/");
          return expect(res.headers["location"]).to.eq("https://www.google.com/search?q=cypress");
        });
      });
      ...
      return [301, 302, 303, 307, 308].forEach(code => {
        return it(`handles direct for status code: ${code}`, function() {
          ...
          .then(() => {
            ...
            .then(function(res) {
              ...
              return expect(res.headers["set-cookie"]).to.include("__cypress.initial=true; Domain=example.com; Path=/");
            });
          });
        });
      });
    ...
    context("error handling", function() {
      ...
      it("passes through status code + content", function() {
        ...
        .then(function(res) {
          ...
          expect(res.body).to.include("document.domain = 'github.com'");
          return expect(res.headers["set-cookie"]).to.match(/__cypress.initial=;/);
        });
      });
      ...
      it("sends back socket hang up on request errors which match origin", function() {
        ...
        .then(function() {
          throw new Error("should not reach");}).catch(err => expect(err.message).to.eq('Error: socket hang up'));
      });
      ...
      it("sends back 401 when file server does not receive correct auth", function() {
        ...
        .then(() => {
          ...
          return expect(res.statusCode).to.eq(401);
        });
      });
      ...
      it("sends back 404 when file does not exist locally", function() {
        ...
        .then(() => {
          ...
          .then(res => {
            ...
            return expect(res.body).to.include("document.domain = 'localhost';");
          });
        });
      ...
      it("does not inject on file server errors when origin does not match", function() {
        ...
        .then(() => {
          ...
          .then(res => {
            expect(res.statusCode).to.eq(500);
            return expect(res.body).to.eq("server error");
          });
        });
      });

      return it("transparently proxies decoding gzip failures", function() {
        nock(this.server._remoteOrigin)
        .get("/index.html")
        ...
        .then(function() {
          throw new Error("should not reach");}).catch(err => expect(err.error.code).to.eq("ECONNRESET"));
      });
    });
    ...
    context("headers", function() {
      ...
      describe("when unload is true", () => it("automatically redirects back to clientRoute", function() {
        return this.rp({
      ...
      describe("when initial is true", () => it("sets back to false", function() {
        nock(this.server._remoteOrigin)
        .get("/app.html")
        .reply(200, "OK", {
          "Content-Type" : "text/html"
        ...
        .then(res => {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers["set-cookie"]).to.match(/initial=;/);
        });
      }));

      describe("when initial is false", () => it("does not reset initial or remoteHost", function() {
        nock(this.server._remoteOrigin)
        .get("/app.html")
        .reply(200, "OK", {
          "Content-Type" : "text/html"
        ...
        .then(function(res) {
          ...
          return expect(res.headers["set-cookie"]).not.to.exist;
        });
      }));
      ...
      it("sends with Transfer-Encoding: chunked without Content-Length", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers).not.to.have.property("content-length");
        });
      });
      ...
      it("forwards cookies from incoming responses", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers["set-cookie"]).to.match(/userId=123/);
        });
      });
      ...
      it("appends to previous cookies from incoming responses", function() {
        ...
        .then(function(res) {
          ...
          return expect(setCookie[1]).to.eq("__cypress.initial=; Domain=localhost; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT");
        });
      });
      ...
      it("appends cookies on redirects", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers["set-cookie"]).to.deep.eq([
            "userId=123; Path=/",
      ...
      it("forwards other headers from incoming responses", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers["x-token"]).to.eq("abc-123");
        });
      });
      ...
      it("omits x-frame-options", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers).not.to.have.property("x-frame-options");
        });
      });
      ...
      it("omits content-security-policy", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers).not.to.have.property("content-security-policy");
        });
      });
      ...
      it("omits document-domain from Feature-Policy header", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers['feature-policy']).not.to.include("document-domain 'none'");
        });
      });
      ...
      it("does not modify host origin header", function() {
        ...
        .then(() => {
          ...
          .then(res => expect(res.statusCode).to.eq(200));
        });
      });
      ...
      it("does not cache when initial response", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers["cache-control"]).to.eq("no-cache, no-store, must-revalidate");
        });
      });
      ...
      it("does cache requesting resource without injection", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers["cache-control"]).to.eq("max-age=86400");
        });
      });
      ...
      it("forwards origin header", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).not.to.include("Cypress");
        });
      });
      ...
      it("issue #222 - correctly sets http host headers", function() {
        ...
        return matches("http://localhost:8080/app.css", () => nock("http://localhost:8080")
        .matchHeader("host", "localhost:8080")
        .get("/app.css")
        .reply(200, "{}", {
          "Content-Type": "text/css"
        })).then(() => matches("http://127.0.0.1/app.css", () => nock("http://127.0.0.1")
        .matchHeader("host", "127.0.0.1")
        .get("/app.css")
        .reply(200, "{}", {
          "Content-Type": "text/css"
        }))).then(() => matches("http://127.0.0.1:80/app2.css", () => nock("http://127.0.0.1:80")
        .matchHeader("host", "127.0.0.1")
        .get("/app2.css")
        .reply(200, "{}", {
          "Content-Type": "text/css"
        }))).then(() => matches("https://www.google.com:443/app.css", () => nock("https://www.google.com")
        .matchHeader("host", "www.google.com")
        .get("/app.css")
        .reply(200, "{}", {
          "Content-Type": "text/css"
        }))).then(() => matches("https://www.apple.com/app.css", () => nock("https://www.apple.com")
      ...
      return context("authorization", function() {
        ...
        return it("doesn't attach auth headers to a diff protection space on the same origin", function() {
      ...
      it("passes the bytes through without injection on http servers", function() {
        ...
        .spread((size, bytes, setup) => {
          ...
          .then(function(res) {
            ...
            return expect(res.body).to.eq(bytes);
          });
        });
      });

      return it("passes the bytes through without injection on http servers with gzip", function() {
        const image  = Fixtures.projectPath("e2e/static/javascript-logo.png");
    ...
    context("woff", function() {
      beforeEach(() => Fixtures.scaffold());

      return it("passes the bytes through without injection", function() {
    ...
    context("svg", function() {
      beforeEach(function() {
        return this.setup("http://www.google.com");
      });

      return it("rewrites <svg> without hanging", function() {
    ...
    context("content injection", function() {
      beforeEach(function() {
        return this.setup("http://www.google.com");
      });
      ...
      it("injects when head is capitalized", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("<HTML> <HEAD> <script type='text/javascript'> document.domain = 'google.com';");
        });
      });
      ...
      it("injects when head missing but has <header>", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("</head> <body><nav>some nav</nav><header>header</header></body> </html>");
        });
      });
      ...
      it("injects when body is capitalized", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("</script> </head> <BODY>hello from bar!</BODY> </HTML>");
        });
      });
      ...
      it("injects when both head + body are missing", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("</head>hello from bar!</HTML>");
        });
      });
      ...
      it("injects even when html + head + body are missing", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("</head><div>hello from bar!</div>");
        });
      });
      ...
      it("injects after DOCTYPE declaration when no other content", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.include("<!DOCTYPE><head> <script");
        });
      });
      ...
      it("injects superdomain even when head tag is missing", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.eq("<html> <head> <script type='text/javascript'> document.domain = 'google.com'; </script> </head> <body>hello from bar!</body> </html>");
        });
      });
      ...
      it("injects content after following redirect", function() {
        ...
        .then(res => {
          ...
          .then(function(res) {
            ...
            return expect(res.body).to.include("Cypress.action(");
          });
        });
      });
      ...
      it("injects performantly on a huge amount of elements over http", function() {
        ...
        .then(res => {
          ...
          return expect(res.body).to.include("document.domain = 'google.com';");
        });
      });
      ...
      it("does not inject when not initial and not html", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.eq("<html><head></head></html>");
        });
      });
      ...
      it("injects into https://www.google.com", function() {
        ...
        .then(() => {
          this.server.onRequest((req, res) => nock("https://www.google.com")
          .get("/")
      ...
      it("works with host swapping", function() {
        ...
        .then(() => {
          ...
          .then(function(res) {
            ...
            const body = cleanResponseBody(res.body);
            return expect(body).to.eq(contents.replace("localhost", "foobar.com"));
          });
        });
      });
      ...
      it("injects document.domain on https requests to same superdomain but different subdomain", function() {
        ...
        .then(() => {
          ...
          .then(function(res) {
            ...
            const body = cleanResponseBody(res.body);
            return expect(body).to.eq("<html><head> <script type='text/javascript'> document.domain = 'foobar.com'; </script></head><body>https server</body></html>");
          });
        });
      });
      ...
      it("injects document.domain on other http requests", function() {
        ...
        .then(function(res) {
          ...
          return expect(body).to.eq("<html><head> <script type='text/javascript'> document.domain = 'google.com'; </script></head></html>");
        });
      });
      ...
      it("does not inject document.domain on non http requests", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.deep.eq({foo: "<html><head></head></html>"});
        });
      });
      ...
      it("does not inject document.domain on http requests which do not match current superDomain", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.eq("<html><head></head><body>hi</body></html>");
        });
      });
      ...
      it("does not inject anything when not text/html response content-type even when __cypress.initial=true", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers["set-cookie"]).not.to.match(/initial/);
        });
      });
      ...
      it("does not inject into x-requested-with request headers", function() {
        ...
        .then(function(res) {
          ...
          return expect(body).to.eq("<html><head></head></html>");
        });
      });

      return ["text/html", "application/xhtml+xml", "text/plain, application/xhtml+xml", "", null].forEach(type => it(`does not inject unless both text/html and application/xhtml+xml is requested: tried to accept: ${type}`, function() {
        nock(this.server._remoteOrigin)
        .get("/iframe")
        .reply(200, "<html><head></head></html>", {
          "Content-Type": "text/html"
    ...
    context("security rewriting", function() {
      describe("on by default", function() {
        ...
        it("handles multi-byte characters correctly when injecting", function() {
          ...
          .then(resp => {
            ...
            .then(function(res) {
              ...
              return expect(res.body).to.include(bytes + "</html>");
            });
          });
        });
        ...
        it("handles multi-byte characters correctly on JS files", function() {
          ...
          .then(resp => {
            ...
            .then(function(res) {
              ...
              return expect(res.body).to.eq(response);
            });
          });
        });
        ...
        it("handles gzip responses that are slightly truncated when injecting", function() {
          ...
          .then(resp => {
            ...
            .then(function(res) {
              ...
              return expect(res.body).to.include("I am a gzipped response</html>");
            });
          });
        });
        ...
        return it("does not die rewriting a huge JS file", function() {
          const pathToHugeAppJs = Fixtures.path("server/libs/huge_app.js");

          const getHugeFile = () => rp("https://s3.amazonaws.com/internal-test-runner-assets.cypress.io/huge_app.js")
          .then(resp => fs
          ...
          .then(hugeJsFile  => {
            ...
            .then(function(res) {
              ...
              return expect(reqTime).to.be.lt(500);
            });
          });
        });
      });

      return describe("off with config", function() {
        beforeEach(function() {
          return this.setup("http://www.google.com", {
            config: {
        ...
        return it("does not replaces obstructive code in JS files", function() {
          nock(this.server._remoteOrigin)
          .get("/app.js")
          .reply(200, "if (top !== self) { }", {
            "Content-Type": "application/javascript"
    ...
    context("FQDN rewriting", function() {
      beforeEach(function() {
        return this.setup("http://www.google.com");
      });
      ...
      it("does not rewrite html when initial", function() {
        ...
        .then(function(res) {
          ...
          return expect(body).to.include("<a href='http://www.google.com'>google</a>");
        });
      });

      return it("does not rewrite html when not initial", function() {
        nock(this.server._remoteOrigin)
        .get("/bar")
        .reply(200, "<html><body><a href='http://www.google.com'>google</a></body></html>", {
          "Content-Type": "text/html"
    ...
    context("file requests", function() {
      beforeEach(function() {
        Fixtures.scaffold();
        ...
        .then(() => {
          ...
          .then(function(res) {
            ...
            return expect(res.headers["last-modified"]).to.exist;
          });
        });
      });
      ...
      it("sets etag", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.headers["etag"]).to.be.a("string");
        });
      });
      ...
      it("sets last-modified", function() {
        return this.rp(this.proxy + "/assets/app.css")
        .then(res => {
          return expect(res.headers["last-modified"]).to.be.a("string");
        });
      });
      ...
      it("streams from file system", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.eq("html { color: black; }");
        });
      });
      ...
      it("sets content-type", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.headers["content-type"]).to.match(/text\/css/);
        });
      });
      ...
      it("can serve files with spaces in the path", function() {
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.eq("foo");
        });
      });
      ...
      it("sets x-cypress-file-path headers", function() {
        return this.rp(this.proxy + "/assets/app.css")
        .then(res => {
          return expect(res.headers).to.have.property("x-cypress-file-path", Fixtures.projectPath("no-server") + "/dev/assets/app.css");
        });
      });
      ...
      it("sets x-cypress-file-server-error headers on error", function() {
        ...
        .then(res => {
          expect(res.statusCode).to.eq(404);
          return expect(res.headers).to.have.property("x-cypress-file-server-error", "true");
        });
      });
      ...
      it("does not inject injects document.domain on 301 redirects to folders", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).not.to.include("document.domain = 'localhost';");
        });
      });
      ...
      it("does not inject document.domain on non http requests", function() {
        ...
        .then(function(res) {
          ...
          return expect(res.body).to.deep.eq({contents: "<html><head></head></html>"});
        });
      });

      return it("does not inject anything when not text/html response content-type even when __cypress.initial=true", function() {
        return this.rp({
          url: this.proxy + "/assets/foo.json",
          headers: {
            "Cookie": "__cypress.initial=true",
    ...
    context("http requests", function() {
      beforeEach(function() {
        return this.setup("http://getbootstrap.com")
        .then(() => {
          nock(this.server._remoteOrigin)
      ...
      return it("proxies http requests", function() {
        nock(this.server._remoteOrigin)
        ...
        .then(function(res) {
          expect(res.statusCode).to.eq(200);
          return expect(res.body).to.eq("html { color: #333 }");
        });
      });
    });

    context("when session is already set", function() {
      beforeEach(function() {
        return this.setup("http://getbootstrap.com")
        .then(() => {
      ...
      return it("proxies to the remote session", function() {
        nock(this.server._remoteOrigin)
        .get("/assets/css/application.css")
        .reply(200, "html { color: #333 }", {
          "Content-Type": "text/css"
    ...
    context("localhost", function() {
      ...
      it("makes requests to ipv6 when ipv4 fails", function(done) {
        ...
        return dns.lookup("localhost", {all: true}, (err, addresses) => {
          ...
          //# just skip this test
          if (!_.find(addresses, {family: 6})) { return done(); }
      ...
      return it("handles 204 no content status codes", function() {
    ...
    context("blacklisted hosts", function() {
      beforeEach(function() {
        return this.setup({
          config: {
            blacklistHosts: [
      ...
      return it("returns 503 and custom headers for all hosts", function() {
        const expectedHeader = (res, val) => expect(res.headers["x-cypress-matched-blacklisted-host"]).to.eq(val);
        ...
        .spread(function(...responses) {
          _.every(responses, function(res) {
            expect(res.statusCode).to.eq(503);
            return expect(res.body).to.be.empty;
          });
    ...
    context("client aborts", function() {
      beforeEach(function() {
        return this.setup("http://localhost:6565");
      });

      return it("aborts the proxied request", function(done) {
        fs
        ...
        .then(str => {
          var server = http.createServer(function(req, res) {
            ...
            //# properly
            req.on("aborted", () => server.close(() => done()));
    ...
    context("event source / server sent events / SSE", function() {
      let onRequest = null;
      ...
      beforeEach(function() {
        ...
        .then(() => {
          ...
          .then(() => {
            return this.port = this.srv.address().port;
          });
        });
      });
      ...
      return it("receives event source messages through the proxy", function(done) {
        onRequest = function(req, res) {
          ...
          const aborted = new Promise(resolve => req.on("aborted", () => resolve()));
    ...
    return context("when body should be empty", function() {
      this.timeout(1000);

      beforeEach(function(done) {
        this.httpSrv = http.createServer(function(req, res) {
      ...
      return [204, 304].forEach(function(status) {
        it(`passes through a ${status} response immediately`, function() {
          ...
          .then(function(res) {
            ...
            return expect(res.statusCode).to.eq(status);
          });
        });

        return it(`passes through a ${status} response with chunked encoding immediately`, function() {
          return this.rp({
  ...
  return context("POST *", function() {
    ...
    it("processes POST + redirect on remote proxy", function() {
      ...
      .then(function(res) {
        expect(res.statusCode).to.eq(302);
        return expect(res.headers["location"]).to.match(/dashboard/);
      });
    });
    ...
    it("processes POST + redirect on remote initial", function() {
      ...
      .then(function(res) {
        ...
        return expect(res.headers["set-cookie"]).to.match(/initial=true/);
      });
    });
    ...
    it("does not alter request headers", function() {
      ...
      .then(function(res) {
        ...
        return expect(res.body).to.eq("OK");
      });
    });
    ...
    return it("hands back 201 status codes", function() {
      nock(this.server._remoteOrigin)
      ...
      .then(res => expect(res.statusCode).to.eq(201));
    });
  });
});
