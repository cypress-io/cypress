  ...
  context('GET /__cypress/tests', () => {
    describe('ids with typescript', () => {
      ...
      it('processes spec into modern javascript', function () {
        return this.rp('http://localhost:2020/__cypress/tests?p=cypress/integration/es6.js').then((res) => {
          expect(res.statusCode).to.eq(200)
    ...
    context('content injection', () => {
      ...
      it('injects when head has attributes', async function () {
        ...
        const contents = removeWhitespace(
          Fixtures.get('server/expected_head_inject.html').replace('{{injection}}', injection)
        )
      ...
      it('injects even when head tag is missing', async function () {
        ...
        const injection = await runner.getInjectionContents()
        const contents = removeWhitespace(
          Fixtures.get('server/expected_no_head_tag_inject.html').replace('{{injection}}', injection)
        )
      ...
      it('injects into https server', async function () {
        ...
        const injection = await runner.getInjectionContents()
        const contents = removeWhitespace(
          Fixtures.get('server/expected_https_inject.html').replace('{{injection}}', injection)
        )
        const res = await this.rp({
          url: 'https://localhost:8443/',
          headers: {
          ...
          query: {
            bool: {
              must: [
                ...
                {
                  filtered: {
                    filter: { bool: { should: [{ term: { subcategory_id: 25 } }, { term: { subcategory_id: 21 } }] } },
                  },
                },
              ],
            },
          ...
          aggs: {
            ...
            'patterns.raw4': {
              ...
              aggs: {
                ...
                'patterns.raw_count': { cardinality: { field: 'patterns.raw' } },
              },
            },
            'custom_filters_a.raw5': {
              filter: {},
              aggs: {
                'custom_filters_a.raw': { terms: { field: 'custom_filters_a.raw', size: 50 } },
                'custom_filters_a.raw_count': { cardinality: { field: 'custom_filters_a.raw' } },
              },
            },
            'custom_filters_b.raw6': {
              filter: {},
              aggs: {
                'custom_filters_b.raw': { terms: { field: 'custom_filters_b.raw', size: 50 } },
                'custom_filters_b.raw_count': { cardinality: { field: 'custom_filters_b.raw' } },
              },
            },
          },
          size: 100,
