...
const record     = require(`${root}lib/modes/record`);
...
const snapshotConsoleLogs = function(name) {
  ...
  .map(innerArgs => innerArgs.join(" ")).join("\n")
  .value();
...
describe("lib/cypress", function() {
  ...
  context("test browsers", function() {
    // sanity checks to make sure the browser objects we pass during tests
    // all pass the internal validation function
    it("has valid browsers", () => expect(v.isValidBrowserList("browsers", TYPICAL_BROWSERS)).to.be.true);

    it("has valid electron browser", () => expect(v.isValidBrowserList("browsers", [ELECTRON_BROWSER])).to.be.true);

    it("allows browser major to be a number", function() {
      const browser = {
        name: 'Edge Beta',
        family: 'chrome',
      ...
      return expect(v.isValidBrowserList("browsers", [browser])).to.be.true;
    });

    return it("validates returned list", () =>
      browserUtils.getBrowsers().then(list => expect(v.isValidBrowserList("browsers", list)).to.be.true)
    );
  });
  ...
  context("--get-key", function() {
    ...
    it("logs error and exits when user isn't logged in", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("NOT_LOGGED_IN");
      });
    });
    ...
    it("logs error and exits when project does not have an id", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("NO_PROJECT_ID", this.pristinePath);
      });
    });
    ...
    it("logs error and exits when project could not be found at the path", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("NO_PROJECT_FOUND_AT_PROJECT_ROOT", "path/to/no/project");
      });
    });

    return it("logs error and exits when project token cannot be fetched", function() {
      return Promise.all([
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("CANNOT_FETCH_PROJECT_TOKEN");
      });
    });
  });

  context("--new-key", function() {
    it("writes out key and exits on success", function() {
      return Promise.all([
        user.set({name: "brian", authToken: "auth-token-123"}),
    ...
    return it("logs error and exits when project token cannot be fetched", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("CANNOT_CREATE_PROJECT_TOKEN");
      });
    });
  });
  ...
  context("--run-project", function() {
    beforeEach(function() {
      ...
      return sinon.stub(commitInfo, "getRemoteOrigin").resolves("remoteOrigin");
    });
    ...
    describe("strips --", function() {
      beforeEach(() => sinon.spy(argsUtil, "toObject"));
      ...
      return it("strips in the middle", function() {
    ...
    it("does not generate a project id even if missing one", function() {
      ...
      .then(() => {
        ...
        .then(function() {
          throw new Error("should have caught error but did not");}).catch(err => expect(err.type).to.eq("NO_PROJECT_ID"));
      });
    });
    ...
    it("does not add project to the global cache", function() {
      ...
      .then(projects => {
        ...
        return cypress.start([`--run-project=${this.todosPath}`]);
    }).then(() => cache.getProjectRoots()).then(projects =>
        //# still not projects
        expect(projects.length).to.eq(0)
      );
    });
    ...
    it("runs project by limiting spec files via config.testFiles as a JSON array of string glob patterns", function() {
      ...
      .then(() => {
        return expect(browsers.open).to.be.calledWithMatch(ELECTRON_BROWSER, {url: "http://localhost:8888/__/#/tests/integration/test2.coffee"});
    }).then(() => {
    ...
    it("scaffolds out integration and example specs if they do not exist when not runMode", function() {
      ...
      .then(cfg => {
        ...
        .then(function() {
          throw new Error("integrationFolder should not exist!");}).catch(() => {
    ...
    it("does not scaffold when headless and exits with error when no existing project", function() {
      const ensureDoesNotExist = function(inspection, index) {
        ...
        return expect(inspection.reason()).to.have.property("code", "ENOENT");
      };
      ...
      .then(() => {
        return this.expectExitWithErr("CONFIG_FILE_NOT_FOUND", this.pristinePath);
      });
    });
    ...
    it("scaffolds out fixtures + files if they do not exist", function() {
      ...
      .then(cfg => {
        ...
        .then(function() {
          throw new Error("fixturesFolder should not exist!");}).catch(() => {
          return cypress.start([`--run-project=${this.pristinePath}`, "--no-run-mode"]);
        }).then(() => {
          return fs.statAsync(cfg.fixturesFolder);
        }).then(() => {
    ...
    it("scaffolds out support + files if they do not exist", function() {
      ...
      .then(cfg => {
        ...
        .then(function() {
          throw new Error("supportFolder should not exist!");}).catch({code: "ENOENT"}, () => {
          return cypress.start([`--run-project=${this.pristinePath}`, "--no-run-mode"]);
        }).then(() => {
          return fs.statAsync(supportFolder);
        }).then(() => {
    ...
    it("removes fixtures when they exist and fixturesFolder is false", function(done) {
      ...
      .then(cfg => {
        ...
        return fs.statAsync(this.cfg.fixturesFolder)
        .then(function() {
          throw new Error("fixturesFolder should not exist!");}).catch(() => done());
      });
    });
    ...
    it("logs warning when projectId and key but no record option", function() {
      ...
      .then(() => {
        ...
        return expect(console.log).to.be.calledWithMatch("https://on.cypress.io/recording-project-runs");
      });
    });
    ...
    it("logs warning when removing old browser profiles fails", function() {
      ...
      .then(() => {
        ...
        return expect(console.log).to.be.calledWithMatch(err.message);
      });
    });
    ...
    it("does not log warning when no projectId", function() {
      ...
      .then(() => {
        ...
        return expect(console.log).not.to.be.calledWithMatch("cypress run --key <record_key>");
      });
    });
    ...
    it("logs error when supportFile doesn't exist", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("SUPPORT_FILE_NOT_FOUND", "Your `supportFile` is set to `/does/not/exist`,");
      });
    });
    ...
    it("logs error when browser cannot be found", function() {
      ...
      .then(() => {
        ...
        const found1 = _.find(argsSet, args =>
          _.find(args, arg =>
            arg.message && arg.message.includes(
        ...
        return expect(found2, "browser names should be listed").to.be.ok;
      });
    });
    ...
    it("logs error and exits when spec file was specified and does not exist", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("NO_SPECS_FOUND", "We searched for any files matching this glob pattern:");
      });
    });
    ...
    it("logs error and exits when spec absolute file was specified and does not exist", function() {
      ...
      .then(() => {
        return this.expectExitWithErr("NO_SPECS_FOUND", "tests/path/to/spec");
      });
    });
    ...
    it("logs error and exits when no specs were found at all", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("NO_SPECS_FOUND", "cypress/specs");
      });
    });
    ...
    it("logs error and exits when project has cypress.json syntax error", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("ERROR_READING_FILE", this.todosPath);
      });
    });
    ...
    it("logs error and exits when project has invalid cypress.json values", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("SETTINGS_VALIDATION_ERROR", "cypress.json");
      });
    });
    ...
    it("logs error and exits when project has invalid config values from the CLI", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("CONFIG_VALIDATION_ERROR", "We found an invalid configuration value");
      });
    });
    ...
    it("logs error and exits when using an old configuration option: trashAssetsBeforeHeadlessRuns", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("RENAMED_CONFIG_OPTION", "trashAssetsBeforeRuns");
      });
    });
    ...
    it("logs error and exits when using an old configuration option: videoRecording", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("RENAMED_CONFIG_OPTION", "video");
      });
    });
    ...
    it("logs error and exits when using screenshotOnHeadlessFailure", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("SCREENSHOT_ON_HEADLESS_FAILURE_REMOVED", "You now configure this behavior in your test code");
      });
    });
    ...
    it("logs error and exits when project folder has read permissions only and cannot write cypress.json", function() {
      ...
      .then(() => {
        ...
        return this.expectExitWithErr("ERROR_READING_FILE", path.join(permissionsPath, "cypress.json"));
      });
    });
    ...
    it("logs error and exits when reporter does not exist", function() {
      ...
      .then(() => {
        return this.expectExitWithErr("INVALID_REPORTER_NAME", "foobarbaz");
      });
    });
    ...
    describe("state", function() {
      beforeEach(function() {
        ...
        .then(() => {
          ...
          return this.statePath = appData.projectsPath(statePathStart);
        });
      });

      return it("does not save project state", function() {
        return cypress.start([`--run-project=${this.todosPath}`, `--spec=${this.todosPath}/tests/test2.coffee`])
        .then(() => {
    ...
    describe("config overrides", function() {
      ...
      return it("can override values in plugins", function() {
    ...
    describe("plugins", function() {
      beforeEach(function() {
        ...
        ee.kill = () =>
          ...
          ee.emit("exit")
        ;
        ee.destroy = () => ee.emit("closed");
        ee.isDestroyed = () => false;
        ee.loadURL = function() {};
        ee.focusOnWebView = function() {};
        ee.webContents = {
          debugger: {
            on: sinon.stub(),
            attach: sinon.stub(),
        ...
        return sinon.stub(Windows, "create").returns(ee);
      });

      return context("before:browser:launch", function() {
        it("chrome", function() {
          ...
          .then(() => {
            ...
            return expect(chromeBrowser._connectToChromeRemoteInterface).to.have.been.calledOnce;
          });
        });

        return it("electron", function() {
    ...
    describe("--port", function() {
      beforeEach(() => runMode.listenForProjectEnd.resolves({stats: {failures: 0} }));
      ...
      return it("logs error and exits when port is in use", function() {
        ...
        .then(() => {
          ...
          return this.expectExitWithErr("PORT_IN_USE_LONG", "5555");
        });
      });
    });
    ...
    describe("--env", function() {
      ...
      return it("parses environment variables with empty values", function() {
        return cypress.start([
          `--run-project=${this.todosPath}`,
          "--video=false",
    ...
    return describe("--config-file", function() {
      ...
      return it("with a custom config file fails when it doesn't exist", function() {
        ...
        return fs.statAsync(path.join(this.todosPath, this.filename))
          .then(() => {
            ...
            return cypress.start([
              ...
              return this.expectExitWithErr("CONFIG_FILE_NOT_FOUND", this.filename, this.todosPath);
            });
        });
      });
    });
  ...
  context("--record or --ci", function() {
    ...
    return it("errors and exits when run is stale", function() {
      const err = new Error();
      err.statusCode = 422;
      err.error = {
        code: "STALE_RUN",
  ...
  context("--return-pkg", function() {
    beforeEach(function() {
      console.log.restore();
      return sinon.stub(console, "log");
    });

    return it("logs package.json and exits", function() {
  ...
  context("--version", function() {
    ...
    return it("logs version and exits", function() {
  ...
  context("--smoke-test", function() {
    ...
    return it("logs pong value and exits", function() {
  ...
  context("interactive", function() {
    beforeEach(function() {
      ...
      return sinon.stub(electron.ipcMain, "on");
    });

    it("passes options to interactiveMode.ready", function() {
    ...
    it("passes filtered options to Project#open and sets cli config", function() {
      ...
      .then(() => {
        ...
        return expect(cfg.resolved.env.baz).to.deep.eq({
          value: "baz",
          from: "cli"
        });
      });
    ...
    it("sends warning when baseUrl cannot be verified", function() {
      ...
      const open = sinon.stub(Server.prototype, "open").resolves([2121, warning]);
    ...
    }).then(() => expect(event.sender.send.withArgs("response").firstCall.args[1].data).to.eql(warning));
    });

    return describe("--config-file", function() {
      beforeEach(function() {
        this.filename = "foo.bar.baz.asdf.quux.json";
        return this.open = sinon.stub(Server.prototype, "open").resolves([]);
      });
      ...
      it("reads config from a custom config file", function() {
        ...
        .then(() => {
          ...
          return expect(cfg.port).to.equal(2020);
        });
      });

      return it("creates custom config file if it does not exist", function() {
        const write = sinon.spy(settings, "_write");
        ...
        .then(() => {
          ...
          .then(json => {
            return expect(json, "json file is empty").to.deep.equal({});
          });
        });
      });
    });
  ...
  context("--cwd", function() {
    beforeEach(function() {
      ...
      return sinon.spy(errors, "warning");
    });

    it("shows warning if Cypress has been started directly", () =>
      cypress.start().then(function() {
        ...
        return expect(console.log).to.be.calledWithMatch("https://on.cypress.io/installing-cypress");
      })
    );

    return it("does not show warning if finds --cwd", () =>
      cypress.start(["--cwd=/foo/bar"]).then(() => expect(errors.warning).not.to.be.called)
    );
  });

  return context("no args", function() {
    beforeEach(function() {
      sinon.stub(electron.app, "on").withArgs("ready").yieldsAsync();
      return sinon.stub(interactiveMode, "ready").resolves();
    });

    return it("runs interactiveMode and does not exit", () =>
      cypress.start().then(() => expect(interactiveMode.ready).to.be.calledOnce)
    );
  });
});
