  ...
  context('interactive', () => {
    ...
    it('passes filtered options to Project#open and sets cli config', function () {
      ...
      .then((json) => {
        ...
        const { supportFile, specPattern, specExcludePattern, ...rest } = json

        return settings.writeForTesting(this.todosPath, { ...rest, e2e: { supportFile, specPattern, specExcludePattern } })
      }).then(() => {
        return cypress.start([
          '--port=2121',
          '--config',
