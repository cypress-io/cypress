 ...
 */
describe("xhrs", function() {
  it("can encode + decode headers", function() {
    const getResp = () => ({
      "test": "We’ll"
    });
    ...
    return cy
      ...
      .window().then(function(win) {
        ...
        return xhr.send();}).wait("@getApi")
        .its("url").should("include", "api/v1");
  });

  it("ensures that request headers + body go out and reach the server unscathed", () => cy
    ...
    return xhr.onload = () => resolve(JSON.parse(xhr.response));
  })).then(function(resp) {
      ...
      return expect(resp.headers).to.have.property("content-type", "application/json");
  }));

  it("does not inject into json's contents from http server even requesting text/html", () => cy
    .visit("http://localhost:1919")
    .window().then(win => new Cypress.Promise(function(resolve) {
    const xhr = new win.XMLHttpRequest;
    xhr.open("POST", "/html");
  ...
  it("does not inject into json's contents from file server even requesting text/html", () => cy
    .visit("/")
    .window().then(win => new Cypress.Promise(function(resolve) {
    const xhr = new win.XMLHttpRequest;
  ...
  it("works prior to visit", () => cy.server());

  //# https://github.com/cypress-io/cypress/issues/5431
  it("can stub a 100kb response", function(done) {
    ...
    .then(function(win) {
      ...
      xhr.onload = finish;
      return xhr.onerror = finish;
    });
  });

  it("spawns tasks with original NODE_OPTIONS", () => cy.task('assert:http:max:header:size', 8192));

  return describe("server with 1 visit", function() {
    before(() => cy.visit("/xhr.html"));
    ...
    .as("createUser")
    .get("#create").click()
    .then(() => win.location.href = '/index.html').wait("@createUser").its("canceled").should("be.true")));
  });
});
