 ...
 */
describe("redirects + requests", function() {
  it("gets and sets cookies from cy.request", function() {
    ...
    return cy
      ...
      .getCookies().then(function(cookies) {
        ...
        return expect(cookies[1]).to.deep.eq({
          domain: "localhost",
          name: "2293-session",
          value: "true",
          httpOnly: false,
  ...
  it("visits to a different superdomain will be resolved twice", () => cy
  ...
  it("automatically follows redirects", () => cy
    .request("http://localhost:2294/redirect")
    .then(function(resp) {
      expect(resp.status).to.eq(200);
      return expect(resp.body).to.eq("<html>home</html>");
  }));

  //# https://github.com/cypress-io/cypress/issues/5654
  it("can turn off following redirects that set a cookie", () => cy
    .request({
      url: "http://localhost:2294/redirectWithCookie",
      followRedirect: false
    })
    .then(resp => expect(resp.status).to.eq(302)));

  it("can turn off automatically following redirects", () => cy
    .request({
    ...
    .then(function(resp) {
      ...
      return expect(resp.redirectedToUrl).to.eq("http://localhost:2294/home");
  }));

  it("follows all redirects even when they change methods", () => cy
    .request({
      method: "POST",
  ...
  it("can submit form url encoded body", () => cy
    .request({
      method: "POST",
  ...
  it("can send qs query params", () => cy
    .request({
  ...
  it("passes even on non 2xx or 3xx status code", () => cy
    .request({
  ...
  it("sets Accept header to */* by default", () => cy
  ...
  it("can override the accept header", () => cy
    .request({
  ...
  return it("issue #375: does not duplicate request cookies on 302 redirect", () => cy
