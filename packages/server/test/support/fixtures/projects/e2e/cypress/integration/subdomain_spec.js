...
describe("subdomains", function() {
  beforeEach(() => cy.visit("http://www.foobar.com:2292"));

  it("can swap to help.foobar.com:2292", () => cy
    .get("a").click()
    .get("h1").should("contain", "Help"));

  it("can directly visit a subdomain in another test", () => cy
    ...
    .document().then(function(document) {
      ...
      return document.cookie = "foo=bar";}).getCookies().then(cookies => expect(cookies.length).to.eq(3)));

  it("issue: #207: does not duplicate or hostOnly cookies as a domain cookie", () => cy
    ...
    xhr.send();
    return xhr.onload = () => resolve(JSON.parse(xhr.response).cookie);
      ...
      return expect(occurences).to.have.length(1);
  }));

  it("correctly sets domain based cookies", () => cy
  ...
  it.skip("issue #362: do not set domain based (non hostOnly) cookies by default", () => cy
    .setCookie("foobar", "1", {
  ...
  it.skip("sets a hostOnly cookie by default", () => cy
  ...
  it("issue #361: incorrect cookie synchronization between cy.request redirects", () => cy
  ...
  it("issue #362: incorrect cookie synchronization between cy.visit redirects", () => cy
  ...
  return it("issue #600 can visit between nested subdomains", () => cy
