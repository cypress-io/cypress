...
const getElapsed = () => Math.round((Number(new Date()) - start)/1000);

let onVisit = null;
let counts = {};

const launchBrowser = (url, opts = {}) => launcher.detect().then(function(browsers) {
...
describe("e2e network error handling", function() {
    ...
    servers: [
      {
        ...
        onServer(app) {
          ...
          return app.get('/javascript-logo.png', function(req, res) {
  ...
  context.skip("Google Chrome", function() {
    const testRetries = path => launchBrowser(`http://127.0.0.1:${PORT}${path}`)
    .then(proc => Promise.fromCallback(cb => onVisit = function() {
      if (counts[path] >= 3) {
        return cb();
      }
    }).then(function() {
      proc.kill(9);
      return expect(counts[path]).to.be.at.least(3);
    }));

    const testNoRetries = path => launchBrowser(`http://localhost:${PORT}${path}`)
    .delay(6000)
    .then(function(proc) {
      proc.kill(9);
      return expect(counts[path]).to.eq(1);
    });

    it("retries 3+ times when receiving immediate reset", () => testRetries('/immediate-reset'));

    it("retries 3+ times when receiving reset after headers", () => testRetries('/after-headers-reset'));

    it("does not retry if reset during body", () => testNoRetries('/during-body-reset'));

    return context("behind a proxy server", function() {
      const testProxiedRetries = url => launchBrowser(url, { withProxy: true })
      .then(proc => Promise.fromCallback(cb => onVisit = function() {
        if (counts[url] >= 3) {
          return cb();
        }
        ...
        proc.kill(9);
        return expect(counts[url]).to.be.at.least(3);
      }));

      const testProxiedNoRetries = url => launchBrowser("http://during-body-reset.invalid/", { withProxy: true })
      .delay(6000)
      .then(function(proc) {
        proc.kill(9);
        return expect(counts[url]).to.eq(1);
      });

      it("retries 3+ times when receiving immediate reset", () => testProxiedRetries("http://immediate-reset.invalid/"));

      it("retries 3+ times when receiving reset after headers", () => testProxiedRetries("http://after-headers-reset.invalid/"));

      it("does not retry if reset during body", () => testProxiedNoRetries("http://during-body-reset.invalid/"));

      it("does not retry on '500 Internal Server Error'", () => testProxiedNoRetries("http://proxy-internal-server-error.invalid/"));

      it("does not retry on '502 Bad Gateway'", () => testProxiedNoRetries("http://proxy-bad-gateway.invalid/"));

      return it("does not retry on '503 Service Unavailable'", () => testProxiedNoRetries("http://proxy-service-unavailable.invalid/"));
    });
  });

  return context("Cypress", function() {
    beforeEach(function() {
    ...
    it("tests run as expected", function() {
      return e2e.exec(this, {
        ...
        return expect(counts).to.deep.eq({
    ...
    it("retries HTTPS passthrough behind a proxy", function() {
      ...
      const onConnect = function({ host, port, socket }) {
        ...
        if (connectCounts[dest] == null) { connectCounts[dest] = 0; }
      ...
      .then(() => {
        ...
        .then(function() {
          ...
          return expect(connectCounts[`localhost:${ERR_HTTPS_PORT}`]).to.be.gte(4);
        });
      });
    });

    it("does not connect to the upstream proxy for the SNI server request", function() {
      const onConnect = sinon.spy(() => true);
      ...
      .then(() => {
        ...
        .then(function() {
          ...
          return expect(onConnect.secondCall).to.be.calledWithMatch({
            host: 'localhost',
            port: HTTPS_PORT
          });
        });
    ...
    return context("does not delay a 304 Not Modified", function() {
      ...
      return it("behind a proxy with transfer-encoding: chunked", function() {
        mitmProxy = mitmProxy();

        mitmProxy.onRequest((ctx, callback) => callback());
