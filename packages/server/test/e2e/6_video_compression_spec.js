...
describe("e2e video compression", function() {
  e2e.setup();
        ...
        .tap(function() {
          ...
          .then(function(files) {
            ...
            .then(function({ duration }) {
              ...
              return expect(durationMs).to.be.closeTo(EXPECTED_DURATION_MS, humanInterval('15 seconds'));
            });
          });}).get("stdout")
        .then(stdout => expect(stdout).to.match(/Compression progress:\s+\d{1,3}%/));
      }
    });
});
});
