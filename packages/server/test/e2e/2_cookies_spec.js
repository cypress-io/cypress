...
const onServer = function(app) {
  ...
  app.all("/requestCookies", (req, res) => res.send(req.cookies));

  app.all("/requestCookiesHtml", (req, res) => res.type('html').send(req.cookies));
  ...
  app.get("/invalidCookies", function(req, res) {
    res.header("Set-Cookie", "foo=bar; domain=nope.not.this.one");

    return res.send("<html></html>");
  });

  app.get("/setCascadingCookies", function(req, res) {
  ...
  app.get("/setDomainCookie", function(req, res) {
  ...
  app.get("/samesite/:value", function(req, res) {
...
describe("e2e cookies", function() {
  ...
  .forEach(([
    ...
    return context(`with ${format} urls`, function() {
