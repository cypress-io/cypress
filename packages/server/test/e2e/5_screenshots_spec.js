...
const onServer = function(app) {
  app.get("/color/:color", (req, res) => e2e.sendHtml(`\
...
describe("e2e screenshots", function() {
  e2e.setup({
    servers: {
      port: 3322,
      onServer
  ...
  return e2e.it("passes", {
    ...
    onRun(exec, browser) {
      return exec()
      .then(function() {
        const screenshot = (...paths) => path.join(e2ePath, "cypress", "screenshots", "screenshots_spec.js", ...paths);
        ...
        .then(function(sizes) {
          ...
          return expect(sizes[0]).not.to.be.closeTo(sizes[1], 5000);}).then(() => Promise.all([
          sizeOf(screenshot1),
          sizeOf(screenshot2),
          sizeOf(screenshot3),
          sizeOf(screenshot4),
          ...
          if (browser === "electron") {
            ...
            return _.each(dimensions, dimension => expect(dimension).to.deep.eq({width: 1280, height: 720, type: "png"}));
          }
        });
      });
    }
