...
Our API server is only accessible to Cypress employees at the moment. If you want to work with the code, we recommend working within the Cypress tests for the springboard. There are lots of tests mocking our API server around logging in, seeing runs, and setting up projects.
...
```bash
## from repo root
yarn workspace @packages/springboard cypress:run
```
...
```bash
yarn dev --project ~/git/cypress/packages/springboard/
```
