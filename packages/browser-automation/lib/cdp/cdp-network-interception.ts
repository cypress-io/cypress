...
function hasFetchInterceptPatterns (client: CriClient): boolean {
  return client.queue.enableCommands.some(({ command, params }) => {
    return command === 'Fetch.enable' && _.isEqual(params, FETCH_ENABLE_PATTERNS)
  })
}

function isFetchDomainEnabled (client: CriClient): boolean {
  return client.isFetchDomainEnabled?.() ?? hasFetchInterceptPatterns(client)
}
 ...
 *
 *
 * **Proxy-only (not implemented on CDP):** HTML injection (`modifyObstructiveCode`),
 * framebusting / security stripping, cross-origin cookie jar sync, network command-log entries,
 * and Test Replay stream capture. Those run on proxy response middleware via
 * {@link ProxyNetworkServices} driven ports when the MITM proxy is enabled.
 *
 *
...
export class CDPNetworkInterception {
  ...
  constructor (
    ...
    /**
     * Origins whose requests should be passed through without interception.
     * Used to exempt the Cypress server itself (which serves the runner app)
     * from `interceptResponse` overhead — those resources are internal and
     * do not need cy.intercept visibility.
     */
    private readonly skipOrigins: string[] = [],
  ) {}
  ...
  async enable (): Promise<void> {
    ...
    debug('enabling CDPNetworkInterception (skipOrigins=%o)', this.skipOrigins)
    debug('isFetchDomainEnabled=%s hasFetchInterceptPatterns=%s', isFetchDomainEnabled(client), hasFetchInterceptPatterns(client))

    if (!isFetchDomainEnabled(client) || !hasFetchInterceptPatterns(client)) {
      debug('sending Fetch.enable with urlPattern:*')
      await client.send('Fetch.enable', FETCH_ENABLE_PATTERNS)
    } else {
      debug('Fetch intercept patterns already enabled on client; skipping Fetch.enable')
    }

    client.on('Fetch.requestPaused', this.handlePausedRequest)
    this.enabled = true
    debug('CDPNetworkInterception enabled')
  }
  ...
  async disable (): Promise<void> {
    ...
    // Do not call Fetch.disable — it is global to the session and would tear down
    // cdp_automation's Document Fetch consumer on the same page client.
  ...
  private handleRequestStagePause = async (params: Protocol.Fetch.RequestPausedEvent): Promise<void> => {
    debug('request-stage pause: %s %s (resourceType=%s)', params.request.method, params.request.url, params.resourceType)

    // Document loads are owned by cdp_automation._handlePausedRequests (AUT-frame tagging).
    // Do not call intercept or issue CDP egress here — that races the other Fetch consumer.
    if (params.resourceType === 'Document') {
      debug('skipping Document (owned by cdp_automation): %s', params.request.url)

      return
    }

    // Requests to Cypress-internal origins (e.g. the runner server) are not test traffic.
    // Pass them through without interceptResponse to avoid the CDP body round-trip overhead
    // that would otherwise block the runner from loading.
    if (this.skipOrigins.length > 0) {
      try {
        const { origin } = new URL(params.request.url)

        if (this.skipOrigins.includes(origin)) {
          debug('fast-path continueRequest (skipOrigins match %s): %s', origin, params.request.url)
          await swallow(() => this.client.send('Fetch.continueRequest', { requestId: params.requestId }))

          return
        }
      } catch {
        // Malformed URL — fall through to normal interception.
      }
    }

    debug('forwarding to intercept.handle: %s %s', params.request.method, params.request.url)

    const { client } = this
    const httpRequest = cdpFetch.toHttpRequest(params)
  ...
  private handleResponseStagePause = async (params: Protocol.Fetch.RequestPausedEvent): Promise<void> => {
    debug('response-stage pause: requestId=%s status=%s', params.requestId, params.responseStatusCode)
