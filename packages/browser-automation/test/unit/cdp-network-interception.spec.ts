...
describe('CDPNetworkInterception', () => {
  ...
  it('skips Fetch.enable when intercept patterns are already active on the client', async () => {
    ...
    expect(client.getCommands('Fetch.enable')).toHaveLength(1)
    expect(client.listenerCount('Fetch.requestPaused')).toBe(1)
  })

  it('sends Fetch.enable when Fetch is active with different patterns', async () => {
    await client.send('Fetch.enable', {
      patterns: [{
    ...
    expect(client.getCommands('Fetch.enable')).toHaveLength(2)
    expect(client.getLastCommand('Fetch.enable')?.params).toEqual({
      patterns: [{
        urlPattern: '*',
        requestStage: 'Request',
      }],
  ...
  it('does not call Fetch.disable on disable so other Fetch consumers stay active', async () => {
    await interception.enable()
    await interception.disable()

    expect(client.getCommands('Fetch.disable')).toHaveLength(0)
    expect(client.isFetchDomainEnabled()).toBe(true)
  })

  it('does not send Fetch.disable when Fetch.enable was skipped', async () => {
    await client.send('Fetch.enable', {
      patterns: [{
        urlPattern: '*',
        requestStage: 'Request',
  ...
  it('yields Document request-stage pauses to cdp_automation without calling intercept', async () => {
    await interception.enable()
    client.emit('Fetch.requestPaused', createFetchPausedEvent({ resourceType: 'Document' }))
