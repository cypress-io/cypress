...
describe('commands', { viewportHeight: 1000 }, () => {
  ...
  context('clicking command', () => {
    it('pins the command', () => {
      ...
      cy.wrap(runner.emit).should('be.calledWith', 'runner:pin:snapshot', { testId: 'r3', logId: 2 })
      cy.percySnapshot()
    })
    ...
    it('prints to console', () => {
      cy.spy(runner, 'emit')
      cy.contains('#exists').click()
      cy.wrap(runner.emit).should('be.calledWith', 'runner:console:log', { testId: 'r3', logId: 2 })
    })

    it('shows the snapshot', () => {
      cy.spy(runner, 'emit')
      cy.contains('#exists').click()
      cy.wrap(runner.emit).should('be.calledWith', 'runner:show:snapshot', { testId: 'r3', logId: 2 })
    })
  ...
  context('command group', () => {
    ...
    context('pinning group', () => {
      it('pins the command', () => {
        ...
        cy.wrap(runner.emit).should('be.calledWith', 'runner:pin:snapshot', { testId: 'r3', logId: fakeIdForTest })
        cy.percySnapshot()
      })
      ...
      it('prints to console', () => {
        ...
        cy.wrap(runner.emit).should('be.calledWith', 'runner:console:log', { testId: 'r3', logId: fakeIdForTest })
      })

      it('shows the snapshot', () => {
        cy.spy(runner, 'emit')
        cy.get('.command-name-within').click('top')
        cy.wrap(runner.emit).should('be.calledWith', 'runner:show:snapshot', { testId: 'r3', logId: fakeIdForTest })
      })
  ...
  context('mousing over', () => {
    ...
    it('shows snapshot after 50ms passes', () => {
      ...
      cy.tick(50)
      cy.wrap(runner.emit).should('be.calledWith', 'runner:show:snapshot', { testId: 'r3', logId: 1 })
    ...
    describe('then mousing out', () => {
      ...
      it('hides the snapshot after 50ms pass without another mouse over', () => {
        cy.tick(50)
        cy.wrap(runner.emit).should('be.calledWith', 'runner:hide:snapshot', { testId: 'r3', logId: 1 })
      })
