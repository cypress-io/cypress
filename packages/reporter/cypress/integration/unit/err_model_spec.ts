...
describe('Err model', () => {
  ...
  context('.isCommandErr', () => {
    it('returns true if an AssertionError', () => {
      const err = new ErrModel({ name: 'AssertionError', message: 'Something went wrong' })

      expect(err.isCommandErr).to.be.true
    })

    it('returns true if an CypressError', () => {
      const err = new ErrModel({ name: 'CypressError', message: 'Something went wrong' })
  ...
  context('#update', () => {
    ...
    beforeEach(() => {
      err = new ErrModel({ name: 'BadError', message: 'Something went wrong' })
    })
