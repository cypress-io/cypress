...
const Reporter: React.FC<SingleReporterProps> = observer(({ appState = appStateDefault, runner, className, error, runMode = 'single', studioEnabled, autoScrollingEnabled, isSpecsListOpen, showFetchRequests, studioTooltipDismissed, resetStatsOnSpecChange, renderReporterHeader = (props: ReporterHeaderProps) => <Header {...props} />, runnerStore }) => {
  ...
  useEffect(() => {
    ...
    action('set:studio:tooltip:dismissed', () => {
      appState.setStudioTooltipDismissed(studioTooltipDismissed ?? false)
    })()
