...
describe('events', () => {
  ...
  context('from runner', () => {
    ...
    it('sends runnable started on before:test:run:async', () => {
      runner.on.withArgs('before:test:run:async').callArgWith(1, 'the runnable')
      expect(runnablesStore.runnableStarted).to.have.been.calledWith('the runnable')
    })

    it('sends runnable finished on after:test:run', () => {
      runner.on.withArgs('after:test:run').callArgWith(1, 'the runnable')
      expect(runnablesStore.runnableFinished).to.have.been.calledWith('the runnable')
    })

    it('increments the stats count on after:test:run', () => {
      runner.on.withArgs('after:test:run').callArgWith(1, { state: 'passed' })
