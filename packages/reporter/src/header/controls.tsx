  ...
  return (
    <div className='controls'>
      ...
      {ifThen(appState.isRunning && !appState.isPaused, (
        <Tooltip placement='bottom' title={<p>Stop Running <span className='kbd'>S</span></p>} className='cy-tooltip' visible={appState.extendingTest ? false : null}>
          <button aria-label='Stop' className='stop' onClick={emit('stop')} disabled={!!appState.extendingTest}>
            <i className='fas fa-stop' />
          </button>
        </Tooltip>
      ))}
      {ifThen(!appState.isRunning, (
        <Tooltip placement='bottom' title={<p>Run All Tests <span className='kbd'>R</span></p>} className='cy-tooltip' visible={appState.extendingTest ? false : null}>
          <button aria-label='Rerun all tests' className='restart' onClick={emit('restart')} disabled={!!appState.extendingTest}>
