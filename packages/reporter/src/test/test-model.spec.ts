...
describe('Test model', () => {
  context('.state', () => {
    it('is the "state" when it exists', () => {
      const test = new Test({ state: 'passed' }, 0)
  ...
  context('#commandMatchingErr', () => {
    it('returns last command matching the error', () => {
      const test = new Test({ err: { name: 'SomeError' } as Err }, 0)

      test.addCommand(new Command({ err: { name: 'SomeError' } as Err }), 'some hook')
      test.addCommand(new Command({ err: {} as Err }), 'some hook')
      test.addCommand(new Command({ err: { name: 'SomeError' } as Err }), 'some hook')
      test.addCommand(new Command({ err: {} as Err }), 'another hook')
      test.addCommand(new Command({ name: 'The One', err: { name: 'SomeError' } as Err }), 'another hook')
