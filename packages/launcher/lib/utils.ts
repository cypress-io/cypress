...
export const spawnWithArch = <T extends cp.SpawnOptionsWithStdioTuple<cp.StdioNull, cp.StdioPipe, cp.StdioPipe>>(cmd: string, args: string[], opts: T) => {
...
export const getOutput = (cmd: string, args: string[]): Promise<{ stdout: string, stderr?: string }> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const proc = spawnWithArch(cmd, args, { stdio: ['ignore', 'pipe', 'pipe'], env: process.env })
