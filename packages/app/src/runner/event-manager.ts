...
const socketToDriverEvents = 'net:stubbing:event request:event script:error cross:origin:cookies dev-server:on-spec-updated dev-server:compile:success dev-server:specs:unchanged dev-server:jit-recompile:queued'.split(' ')
...
export class EventManager {
  ...
  addGlobalListeners (state: MobxRunnerStore, options: AddGlobalListenerOptions) {
    ...
    this.ws.on('dev-server:compile:success', ({ specFile, studioCompileRerun, jitRecompile }) => {
