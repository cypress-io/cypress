...
export class TapManager implements TapBindingContract {
   ...
   * `ok: false`; the handler's own return value is the `ok: true` payload. Any
  ...
  async exec (command: string, args: Record<string, string> = {}, options: Record<string, string> = {}): Promise<TapExecResult> {
    ...
    try {
      ...
      // throwing TapCommandError; surface it as ok: false so the CLI renders it
      // on stderr and exits non-zero. Any other throw is a real binding bug.
