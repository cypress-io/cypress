<template>
  <div class="p-0 m-0 border-0 h-full" :class="$attrs.class">
    <div class="relative rounded-md h-full">
      <div v-if="hasPrefix" class="absolute inset-y-0 left-0 pl-4 flex items-center">
        <span class="text-gray-500 flex items-center justify-center">
          <slot name="prefix">
            ...
            />
            <IconMagnify v-else-if="type === 'search'" :class="prefixIconClasses"/>
          </slot>
        </span>
      </div>
      <input
      ...
      <div v-if="hasSuffix" class="absolute inset-y-0 right-0 pr-3 flex items-center">
        <span class="text-gray-500 flex items-center justify-center">
          <slot name="suffix">
            <component :is="suffixIcon" class="pointer-events-none" :class="suffixIconClasses"/>
          </slot>
        </span>
      </div>
    </div>
