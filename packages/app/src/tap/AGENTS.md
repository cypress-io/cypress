...
- `tap-manager-data-source.ts` — the single seam onto runner-window globals: the event manager (`getEventManager` → `getCypress().runner`, `runComplete`), `__RUN_MODE_SPECS__`, the AUT iframe, and the snapshot-pin machinery (`pinSnapshot`/`changeSnapshotState`/`unpinSnapshot`/`getPinnedSnapshot`). `getPinnedSnapshot` reads the app's own snapshot store, which is where every pin lands — tap's and a click on a command in the reporter alike — so the pin `pin` and `status` report is the one the AUT frame is actually showing, whoever made it. The pin is derived from that store on every read and never tracked here; the app owns the pinned DOM, including putting the live page back on release. Commands reach globals only through it — never `window.getEventManager` or the runner stores directly — and component tests stub its methods instead of the real globals. `getRunner` serves only a run that has **started** (the driver's `getStartTime` is non-null): a runner exists from the moment its spec is installed, but until mocha starts it its state is empty while `runComplete` still describes the previous run, so an ungated read reports one run's verdict under another's name.
- `contract.ts` — the frozen cross-process contract. The shared half lives in `@packages/cypress-sessions`; the CLI reads the same contract from that package's compiled build. Do not fork it here.
...
- `commands/*.ts` — one module per subcommand (`tests`, `commands`, `command`, `reporter`, `pin`, …); nothing but commands lives here.
- `test-state.ts` — serialization of runner test state, shared by the `tests`, `commands`, `command`, `reporter`, `run-state`, and `pin` commands.
