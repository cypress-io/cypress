 ...
 *   description, params) with a schema version.
 ...
 * `{ error }` values (see `TapExecResult`); a method that actually throws
 * is treated by callers as a binding bug.
 */

/**
 * Internal negotiation signal handed to the CLI via `getSchema` so it can
 * refuse a Cypress whose tap contract it doesn't understand. Bump ONLY when
 * the schema shapes themselves change incompatibly — adding commands or params
 * never requires a bump. It is deliberately kept out of the public `cli/types`
 * surface — nothing user-facing should expose it.
 */
 ...
 * unrecognized command, a malformed args/options payload, or positionals/
 * options that do not satisfy the command's schema. Domain failures carry a
 * command-defined code instead (via `TapCommandError`), so `error.code` below
 * stays an open `string` — those codes are not part of this frozen surface,
 * the same way per-command result shapes are not.
 */
export type TapExecDispatchFailureCode = 'UNKNOWN_COMMAND' | 'INVALID_PAYLOAD' | 'INVALID_ARGUMENTS' | 'INVALID_OPTIONS'

/**
 * The wire envelope `exec` resolves with. `{ result }` carries the command's
 * success payload, shaped by each command. `{ error }` carries a failure:
 ...
 * code). The CLI renders any `{ error }` on stderr and exits non-zero, so the
