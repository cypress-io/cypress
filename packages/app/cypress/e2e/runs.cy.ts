...
describe('App: Runs', { viewportWidth: 1200 }, () => {
  ...
  context('Runs - Cannot Find Project', () => {
    beforeEach(() => {
      cy.withCtx(async (ctx) => {
        await ctx.actions.file.writeFileInProject('cypress.config.js', 'module.exports = {\'projectId\': \'abcdef42\', clientRoute: \'\/cy-child-client\/\'}')
      })
  ...
  context('Runs - Unauthorized Project', () => {
    beforeEach(() => {
      cy.withCtx(async (ctx) => {
        await ctx.actions.file.writeFileInProject('cypress.config.js', 'module.exports = {\'projectId\': \'abcdef\', clientRoute: \'\/cy-child-client\/\' }')
      })
  ...
  context('Runs - No Runs', () => {
    it('when no runs and not connected, shows connect to dashboard button', () => {
      cy.withCtx(async (ctx) => {
        await ctx.actions.file.writeFileInProject('cypress.config.js', 'module.exports = {projectId: null, clientRoute: \'\/cy-child-client\/\' }')
      })
