...
describe('App: Specs', () => {
  ...
  describe('Testing Type: Component', {
    ...
    context('Create from component card', () => {
      ...
      context('project with custom spec pattern', () => {
        beforeEach(() => {
          cy.scaffoldProject('no-specs-custom-pattern-vue')
          cy.openProject('no-specs-custom-pattern-vue')
          cy.startAppServer('component')
          cy.visitApp()
        ...
        it('Lists Vue components in the project', () => {
          ...
          cy.findByText('10 Matches').should('be.visible')
        ...
        it('Allows for the user to search through their components', () => {
          ...
          cy.findByText('*.vue').should('be.visible')
          cy.findByText('10 Matches').should('be.visible')
          cy.findByLabelText('file-name-input').type('Calendar')

          cy.findByText('Calendar').should('be.visible')
          cy.findByText('1 of 10 Matches').should('be.visible')
          cy.findByText('App').should('not.exist')
        })
        ...
        it('shows success modal when component spec is created', () => {
          ...
          cy.findByText('Calendar').should('be.visible').click()

          cy.findByRole('dialog', {
            name: defaultMessages.createSpec.successPage.header,
          }).as('SuccessDialog').within(() => {
            cy.contains(getPathForPlatform('src/specs-folder/Calendar.cy.js')).should('be.visible')
            cy.findByRole('button', { name: 'Close' }).should('be.visible')

            cy.findByRole('link', { name: 'Okay, run the spec' })
            .should('have.attr', 'href', `#/specs/runner?file=src/specs-folder/Calendar.cy.js`)
