...
Cypress.on('window:before:load', (win) => {
  ...
  // Trap reads/writes of the flag so we can see (a) whether this handler runs
  // at all, (b) what the urql client observes at boot, and (c) who clobbers it.
  // eslint-disable-next-line no-console
  console.error('[gql-no-socket] SPEC before:load fired; flag was:', win.__CYPRESS_GQL_NO_SOCKET__, 'href:', win.location.href)

  let value: string | undefined = 'true'

  Object.defineProperty(win, '__CYPRESS_GQL_NO_SOCKET__', {
    configurable: true,
    get () {
      // eslint-disable-next-line no-console
      console.error('[gql-no-socket] read ->', value)

      return value
    },
    set (next) {
      // eslint-disable-next-line no-console
      console.error('[gql-no-socket] set ->', next, '|', new Error('setter').stack?.split('\n').slice(1, 4).join(' | '))
      value = next
    },
  })
})
...
describe('App - Debug Page', () => {
  // DIAGNOSTIC(proxy-off debug pair): remove before merge.
  // If this line does not appear in the browser console, the served spec
  // bundle is stale and none of the instrumentation is actually running.
  before(() => {
    // eslint-disable-next-line no-console
    console.error('[gql-no-socket] INSTRUMENTED SPEC BUNDLE IS LIVE')
  })
