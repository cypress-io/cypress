...
describe('detectFramework', () => {
  ...
  ;['2.0.0', '3.0.0'].forEach((v) => {
    it(`Lit and Vite v${v}`, async () => {
      const projectPath = await scaffoldMigrationProject('lit-vite-unconfigured')

      fakeDepsInNodeModules(projectPath, [
        { dependency: 'lit', version: '2.0.0' },
        { dependency: 'vite', version: v },
      ])

      const actual = await detectFramework(projectPath)

      expect(actual.framework?.type).to.eq('lit')
      expect(actual.bundler?.type).to.eq('vite')
    })
  })

  it(`Lit and Webpack`, async () => {
    const projectPath = await scaffoldMigrationProject('lit-webpack-unconfigured')
