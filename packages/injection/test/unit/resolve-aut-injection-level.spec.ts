...
describe('resolveAutInjectionLevel', () => {
  ...
  describe('non-AUT frames', () => {
    it('returns "partial" when neither window.name nor frameElement.id identify the AUT frame', () => {
      expect(resolveAutInjectionLevel(marker, signals({
    ...
    it('returns "partial" when the marker appears but is not a prefix of window.name', () => {
      expect(resolveAutInjectionLevel(marker, signals({
    ...
    it('returns "partial" when no identifying signals are present', () => {
      expect(resolveAutInjectionLevel(marker, signals())).toEqual('partial')
    })
  })
})
