...
BiDi, the server, or any transport: the *actual* injection happens elsewhere. Keeping "what to inject / when" separate from "how to
...
| Injection-level decision | server-side, per response (`resolveInjectionLevel` in `@packages/network-interception`) | in-page, per document, from browser signals (`resolveAutInjectionLevel`) |
...
3. A bridge adapter registers this package's bundled
   script over its automation client (For example, CDP  uses `Page.addScriptToEvaluateOnNewDocument`) so it
...
- **`resolveAutInjectionLevel`** (`lib/resolve-aut-injection-level.ts`) — the pure decision
  (`none` | `full` | `cross-origin`) from browser-readable signals; the in-page
  analogue of the server-side `resolveInjectionLevel` (`@packages/network-interception`).
- **`installAutBridgeInFrame`** (`lib/install-aut-bridge-in-frame.ts`) — gathers the
  per-frame signals (`window.name`, `frameElement.id`, the frame's **own** origin) and dispatches
  to the resolved level. The origin comparison is the frame's own origin vs. the supplied
  `primaryOrigin`, both reduced through the injector's `getOrigin` — it deliberately does **not**
  read `window.top.location`, which a cross-subdomain AUT frame can't access under web security
  (it throws, so it would always miss the very case we need). So when `document.domain` injection
  is on it matches on the shared **superdomain** (e.g. `app.foobar.com` and `www.foobar.com` both
  reduce to `foobar.com` → full injection); with it off, the distinct origins resolve to cross-origin.
...
- **Automation-agnostic** — no CDP / BiDi / `@packages/server` imports.
