...
describe('<BrowserIcon />', () => {
  ...
  it('displays correct logo for supported browsers', () => {
    ...
    _.each(browsers, (browser, i) => {
      cy.get('.browser-icon').eq(i).should('have.attr', 'src').and('include', _.kebabCase(browser))
    })
  })
  ...
  it('displays family logo for other variants', () => {
    ...
    cy.get('.browser-icon').eq(0).should('have.attr', 'src').and('include', 'chrome')

    cy.get('.browser-icon').eq(1).should('have.attr', 'src').and('include', 'edge')

    cy.get('.browser-icon').eq(2).should('have.attr', 'src').and('include', 'electron')

    cy.get('.browser-icon').eq(3).should('have.attr', 'src').and('include', 'firefox')

    cy.get('.browser-icon').eq(4).should('have.attr', 'src').and('include', 'chromium')
  })
