...
export type GetLongLivedPreRequestCb = (browserPreRequest?: BrowserPreRequest) => void

type PendingRequest = {
  ctxDebug
  callback: GetLongLivedPreRequestCb
...
export class LongLivedPreRequests {
  ...
  get (req: CypressIncomingRequest, ctxDebug, callback: GetLongLivedPreRequestCb) {
    ...
    if (pendingPreRequest) {
      ...
      debugVerbose('Incoming request %s matches known pre-request: %o', key, pendingPreRequest)
    ...
    const pendingRequest: PendingRequest = {
      ...
      callback,
      timeout: setTimeout(() => {
        debugVerbose('Never received pre-request for request %s after waiting %sms. Continuing without one.', key, this.requestTimeout)
