...
export const AllCypressErrors = {
  ...
  CLOUD_PROTOCOL_UPLOAD_STREAM_STALL_FAILURE: (error: Error & { chunkSizeKB: number, maxActivityDwellTime: number }) => {
    ...
    return errTemplate`\
        ...
        If this error occurs often, the sampling period may be configured by setting the ${fmt.highlightSecondary('CYPRESS_ENV_PROTOCOL_UPLOAD_SAMPLING_INTERVAL')} environment variable to a higher value than ${fmt.stringify(error.maxActivityDwellTime)}.
