...
- **Adding or removing keys in `src/errors.ts`** changes the GraphQL schema — regenerate and commit `packages/data-context/schemas/schema.graphql` in the same PR, or downstream type-checking breaks. Mechanism and command: [`packages/data-context/AGENTS.md`](../data-context/AGENTS.md).
