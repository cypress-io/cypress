 ...
 *
 * Usage:
 *   ts-node parse-snyk-summary.ts <snyk-json-file>
 *   ts-node parse-snyk-summary.ts --json <snyk-json-file>
 *
...
async function main(): Promise<void> {
  ...
  for (const arg of argv) {
    if (arg === '--json') jsonOutput = true
      ...
      console.log(
        'Usage: ts-node parse-snyk-summary.ts [--json] <snyk-json-file>'
      )
      process.exit(0)
  ...
  if (Array.isArray(data)) {
    ...
    const totalVulns = projects.reduce(
      (sum: number, p: any) => sum + p.vulnCount,
      0
    )
    const affected = projects.filter((p: any) => p.vulnCount > 0)
    ...
    if (jsonOutput) {
      ...
      console.log(
        `${data.length} projects scanned, ${totalVulns} total vulnerabilities`
      )
    ...
    if (jsonOutput) {
      console.log(
        JSON.stringify({ ok: data.ok, vulnCount: vulns }, null, 2)
      )
    } else {
