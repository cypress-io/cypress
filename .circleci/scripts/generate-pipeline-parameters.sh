...
emit_json() {
  # NOTE: do not emit `publish-binary-branch` or `force-persist-artifacts` here.
  # Both are also declared as parameters on the setup pipeline (.circleci/config.yml).
  # When a setup-pipeline parameter is explicitly set at trigger time (e.g. an
  # API trigger that passes `force-persist-artifacts: true`), CircleCI auto-
  # propagates it to the continuation pipeline parameter of the same name.
  # Re-emitting the same param here causes CircleCI's continuation API to reject
  # the request with "Conflicting pipeline parameters." Let auto-propagation do
  # its job — only emit params that exist *only* on the continuation pipeline.
  cat <<EOF
