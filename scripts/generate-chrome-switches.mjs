...
// Regenerates packages/server/lib/util/chrome-switches.json — records, for each
// switch in @packages/server's DEFAULT_FLAGS, that it's a real Chromium
// command-line switch in every pinned Chrome version. chromium_flags_spec reads
// it to catch typo'd or removed flags offline.
//
// Chromium silently ignores unrecognized switches, so a misspelled or removed
// flag (e.g. `disable-prompt-on-repos` instead of `disable-prompt-on-repost`) is
// a no-op with no error. This script scrapes the switch literals defined in
// Chromium's `*_switches.cc` files at the pinned refs, intersects them across
// every tested version, and writes the subset of DEFAULT_FLAGS found valid (the
// "verdict"). Committing only that subset — not Chromium's full switch list —
// keeps the artifact small and the spec offline while preserving coverage: a
// DEFAULT_FLAG that isn't a real switch simply won't appear, so the spec fails.
//
// Usage:
...
// the source of truth for the switches Cypress passes; the generated artifact is
// the subset of these proven valid, so it's what we read, validate, and write.
const FLAGS_MODULE_PATH = path.join(SERVER_DIR, 'lib', 'util', 'chromium_flags.ts')

// written into the artifact so the committed JSON explains itself
const ALLOWLIST_COMMENT = 'Generated by scripts/generate-chrome-switches.mjs — do not edit by hand. `validatedFlags` is the subset of @packages/server DEFAULT_FLAGS (chromium_flags.ts) proven to be real Chromium command-line switches in every pinned Chrome version below (the intersection of stable + beta). chromium_flags_spec asserts every default flag appears here; Chromium silently ignores unknown switches, so a typo or removed flag simply will not appear and the spec fails. Regenerate with `yarn workspace @packages/server generate-chrome-switches --write` (requires network).'
...
const loadCypressSwitchNames = () => {
  ...
  try {
    stdout = execFileSync(process.execPath, ['-r', '@packages/ts/register', '-e', script], {
      cwd: SERVER_DIR,
      encoding: 'utf8',
    ...
    throw new Error(`could not load DEFAULT_FLAGS from ${FLAGS_MODULE_PATH}: ${String(err.stderr || err.message).trim()}`)
  }

  return [...new Set(JSON.parse(stdout).map((flag) => flag.split('=')[0]))]
}
...
// a DEFAULT_FLAG missing from the verdict is either a switch whose literal
// Chromium moved to a file we don't scrape, or one Chromium removed. Consult
// peter.sh to tell the two apart and print an actionable hint per flag.
// Best-effort: any failure here warns and returns without throwing, so it never
// blocks generation.
const diagnoseMissingFlags = async (missing, refs) => {
  console.log(`\n[diagnose] ${missing.length} DEFAULT_FLAG(s) not valid in the pinned Chrome version(s); consulting peter.sh to classify them:`)
...
const main = async () => {
  ...
  // guard on the *scrape* size (hundreds of switches), not the verdict (~few
  // dozen), so broken extraction is still caught before we derive the subset.
  if (intersection.size < MIN_EXPECTED_SWITCHES) {
    throw new Error(`only ${intersection.size} switches in the intersection (expected >= ${MIN_EXPECTED_SWITCHES}). Refusing to trust a likely-corrupt scrape; check the per-file counts above.`)
  }

  // the committed artifact is just the subset of DEFAULT_FLAGS proven to be real
  // switches in every pinned version — the "verdict" — not Chromium's full list.
  const cypressSwitches = loadCypressSwitchNames()
  const validatedFlags = cypressSwitches.filter((name) => intersection.has(name)).sort()
  const missing = cypressSwitches.filter((name) => !intersection.has(name))

  console.log(`\n${validatedFlags.length}/${cypressSwitches.length} DEFAULT_FLAGS valid across all ${chromes.length} tested version(s)`)

  // explain (via peter.sh) any flag that didn't validate, with a fix hint
  if (missing.length) await diagnoseMissingFlags(missing, chromes.map((c) => c.ref))

  if (args.write) {
    const committed = readCommitted()

    committed._comment = ALLOWLIST_COMMENT
    committed.versions = chromes.map(({ channel, version, ref }) => ({ channel, version, ref }))
    committed.generatedAt = new Date().toISOString()
    committed.validatedFlags = validatedFlags
    delete committed.switches
    ...
    console.log(`wrote ${validatedFlags.length} validated flags to ${ALLOWLIST_PATH}`)

    return
  }

  // --check: fail if the committed verdict drifts from a fresh evaluation
  const committed = new Set(readCommitted().validatedFlags)
  const fresh = new Set(validatedFlags)
  const removed = [...committed].filter((s) => !fresh.has(s))
  const added = [...fresh].filter((s) => !committed.has(s))

  if (removed.length || added.length) {
    console.error('\nvalidated-flags list is stale. Re-run with --write to update.')
    if (removed.length) console.error(`  no longer valid in pinned Chromium: ${removed.join(', ')}`)

    if (added.length) console.error(`  newly valid (regenerate to record): ${added.join(', ')}`)

    process.exit(1)
  }

  console.log('validated-flags list is up to date.')
}
