...
const canRecordVideo = () => os.platform() !== "win32";

const shouldSkipProjectTest = () => os.platform() === "win32";
...
const runSmokeTest = function(buildAppExecutable, timeoutSeconds = 30) {
  ...
  .catch(function(err) {
    ...
    throw err;}).then(function({stdout}) {
...
const runFailingProjectTest = function(buildAppExecutable, e2e) {
  ...
  const spawn = () => new Promise(function(resolve, reject) {
    const env = _.omit(process.env, "CYPRESS_INTERNAL_ENV");
...
const test = function(buildAppExecutable) {
  ...
  .then(() => runProjectTest(buildAppExecutable, e2e)).then(() => runFailingProjectTest(buildAppExecutable, e2e)).then(() => Fixtures.remove());
};
