...
const remapProjects = function(projectsByProvider) {
  ...
  _.mapValues(projectsByProvider, function(provider, name) {
    const remapPlatform = (platform, repos) => repos.forEach(repo => list.push({
      repo,
      provider: name,
      platform
    }));

    if (provider.win32) { remapPlatform("win32", provider.win32); }
    if (provider.linux) { remapPlatform("linux", provider.linux); }
    if (provider.darwin) { return remapPlatform("darwin", provider.darwin); }
  });
...
const remapMain = function(projectsByProvider) {
  ...
  _.mapValues(projectsByProvider, (provider, name) => list.push({
    repo: provider.main,
    provider: name
  }));
...
const awaitEachProjectAndProvider = function(projects, fn, filter = R.identity) {
  ...
  return Promise.mapSeries(filteredProjects, project => fn(project.repo, project.provider, creds));
};
...
const getFilterByProvider = function(providerName, platformName) {
  let platformFilter, providerFilter;
  if (providerName) {
...
`;
      ...
      // try default branch
      makeEmptyGithubCommit(defaultOptions)).then(createGithubCommitStatusCheck);
    };
