...
const getDependencyPathsToRemove = async ({ entryPoints, external }) => {
  ...
  esbuildResult.warnings.forEach((warning) => {
    ...
    console.log(warningSubject)
    console.log(warning.location.file)
...
const cleanup = async (buildAppDir) => {
  ...
  const keptDependencies = await getDependencyPathsToRemove({
    entryPoints: [
      './packages/server/index.js',
      './packages/server/hook-require.js',
      './packages/server/lib/plugins/child/require_async_child.js',
  ...
  console.log(potentiallyRemovedDependencies.length)
  console.log(keptDependencies.length)

  await Promise.all(potentiallyRemovedDependencies.map(async (dependency) => {
    if (![...keptDependencies, 'package.json', 'packages/server/server-entry.js'].includes(dependency.slice(2)) && !dependency.includes('marionette-client')) {
