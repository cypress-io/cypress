...
const macZip = (src, dest) => printFileSizes(src)
.then(function() {
  ...
  const onZipFinished = () => console.log("✅ ditto finished");
...
const megaBytes = bytes => 1024 * 1024 * bytes;
...
const linuxZipAction = function(parentFolder, dest, relativeSource) {
  ...
  const onZipFinished = () => console.log("✅ zip finished");
...
const linuxZip = function(src, dest) {
  ...
  .then(renamedSource => printFileSizes(renamedSource)
  .then(R.always(renamedSource))).then(function(renamedSource) {
...
const windowsZipAction = function(src, dest) {
  ...
  const onZipFinished = () => console.log("✅ 7z finished");
...
const windowsZip = (src, dest) => renameFolder(src)
.then(renamedSource => windowsZipAction(renamedSource, dest));
