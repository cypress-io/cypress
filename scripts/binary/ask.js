...
const deployNewVersion = () => {
  return fs.readJsonAsync('./package.json').then((json) => {
    return prompt(getQuestions(json.version)).then((answers) => {
...
const whichZipFile = () => {
  return prompt(getZipFile()).get('zipFile')
}
...
const whichPlatform = () => {
  return prompt(getPlatformQuestion()).get('platform')
}

const whichBumpTask = () => {
  return prompt(getBumpTasks()).get('task')
}

const toCommit = ({ version }) => {
  return prompt(getCommitVersion(version)).get('commit')
}
