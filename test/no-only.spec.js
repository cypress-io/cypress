...
it.only('lint js with only', async () => {
  ...
  expect(result.messages[0].message).toContain('it.only')
  expect(result.messages[1].message).toContain('describe.only')
  expect(result.messages[2].message).toContain('context.only')
