...
sudoku._search = function (candidates, reverse) {
  ...
  if (!reverse) {
    for (let vi in min_candidates) {
      ...
      const candidates_next = sudoku._search(
...
sudoku._eliminate = function (candidates, square, val) {
  ...
  if (nr_candidates === 1) {
    ...
    for (let pi in SQUARE_PEERS_MAP[square]) {
      ...
      const candidates_new = sudoku._eliminate(candidates, peer, target_val)
  ...
  for (let ui in SQUARE_UNITS_MAP[square]) {
    ...
    if (val_places.length === 1) {
      const candidates_new = sudoku._assign(candidates, val_places[0], val)
...
sudoku.print_board = function (board) {
  ...
  console.log(display_string) // eslint-disable-line no-console
}
