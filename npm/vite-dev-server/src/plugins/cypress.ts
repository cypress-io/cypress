    ...
    transform (code, id, options?) {
      try {
        if (/\.js$/.test(id) && !/\/\/# sourceMappingURL=/i.test(code)) {
          // The Vite dev server serves esnext, meaning many JS files aren't transpiled thus no sourcemaps are sent along. However,
          // a sourcemap is generated by Vite which is in effect an "identity" sourcemap mapping 1-to-1 to the output file. We can
          // grab this and pass it along as a sourcemap we want Vite to embed into the output, giving Cypress a sourcemap to use
          // for codeFrame lookups.
