...
export async function generateTsConfig (devServerConfig: AngularWebpackDevServerConfig, buildOptions: BuildOptions): Promise<string> {
  ...
  const getProjectFilePath = (...fileParts: string[]): string => {
    return toPosix(path.join(...fileParts))
  }

  const includePaths = [
    ...specPattern.map((pattern) => getProjectFilePath(projectRoot, pattern)),
  ]
  ...
  if (buildOptions.polyfills) {
    ...
    includePaths.push(
      ...polyfills.map((p: string) => getProjectFilePath(workspaceRoot, p)),
    )
  }

  const cypressTypes = getProjectFilePath(
    workspaceRoot,
    'node_modules',
    'cypress',
    'types',
...
async function getAngularCliWebpackConfig (devServerConfig: AngularWebpackDevServerConfig) {
  ...
  const context = createFakeContext(
    projectConfig.buildOptions.workspaceRoot || projectRoot,
    projectConfig,
  )
      ...
      stylesConfig.module.rules.forEach((rule: RuleSetRule) => {
        rule.rules?.forEach((ruleSet) => {
          ...
          ruleSet.use.map((loader) => {
            ...
            const root =
              projectConfig.buildOptions.workspaceRoot ||
              path.join(
