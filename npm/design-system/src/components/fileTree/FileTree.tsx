...
export const FileTree = <T extends FileBase>({
  ...
  const ParentComponent = useMemo(() => onRenderFolder ?? createDefaultFolderComponent(leftOffset), [
    leftOffset,
    onRenderFolder,
  ])
  const LeafComponent = useMemo(() => onRenderFile ?? createDefaultFileComponent(leftOffset), [
    leftOffset,
    onRenderFile,
  ])

  const onNodePress = useMemo<OnNodePress<TreeFile<T>, TreeFolder<T>> | undefined>(
    () => (node, event) => {
  ...
  const onNodeKeyDown = useMemo<OnNodeKeyDown<TreeFile<T>, TreeFolder<T>> | undefined>(
    () =>
      onFolderKeyDown || onFileKeyDown
        ? (node, event) => {
            if (node.type === 'parent') {
...
const createDefaultFolderComponent = <T extends FileBase>(leftOffset = 0) => ({
  parent: { id, name, indexes },
  depth,
  isOpen,
}: ParentProps<TreeFolder<T>>) => (
  <CollapsibleGroupHeader
    className={styles.node}
    style={
      depth > 0 || leftOffset !== 0
        ? {
    ...
    title={<NameWithHighlighting name={name} path={id} indexes={indexes ?? []} />}
    tooltipTitle={id}
    expanded={isOpen}
    icons={icons}
    lineHeight="tight"
