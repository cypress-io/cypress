...
const runSmokeTest = () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('exit', (code) => {
        if (code === 0) {
          ...
          reject(new VerificationFailedError(stderr))
        }
      })
    })
  }
  ...
  if (needsXvfb) {
    ...
    .then(spawn)
    .catch((err) => {
      throw new VerificationFailedError(`Could not start XVFB. Make sure it is installed on your system.\n\n${err.stack}`)
    })
  } else {
    return spawn()
  }
  ...
  .then((verifiedVersion) => {
    if (verifiedVersion !== packageVersion) {
      ...
      return runSmokeTest().then(() => {
        log()
        log(chalk.green('✓ Verified Cypress executable'))
        return writeVerifiedVersion(packageVersion)
      })
    }
  })
  .catch(VerificationFailedError, (err) => {
