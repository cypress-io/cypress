(function() {
  window.Eclectus = (function($, _) {
    ...
    methods = {
      ...
      assert: function(obj, expr, msg, negateMsg, expected, actual) {
        var assertion;
        assertion = new Eclectus.Assertion(obj.contentWindow.document, obj.channel, obj.runnable);
        return assertion.log(expr, msg, negateMsg, expected, actual);
      },
      server: function(obj) {
        var server;
...
(function() {
  ...
  Eclectus.Assertion = (function($, _, Eclectus) {
    ...
    Assertion = (function(_super) {
      ...
      Assertion.prototype.log = function(expr, msg, negateMsg, expected, actual) {
        var body;
        body = this.$("body").clone(true, true);
        body.find("script").remove();
  ...
  this.App.module("Utilities", function(Utilities, App, Backbone, Marionette, $, _) {
    ...
    API = {
      start: function() {
        ...
        if (chai && chai.AssertionError) {
          chai.Assertion.prototype.assert = _.wrap(chai.Assertion.prototype.assert, function() {
            var args, orig;
            orig = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
            Ecl.assert.apply(Ecl, args);
