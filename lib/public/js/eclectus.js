        ...
        return this.runner.runSuite = _.wrap(this.runner.runSuite, function(runSuite, suite, fn) {
          ...
          suite.eachTest((function(_this) {
            return function(test) {
              ...
              return test.fullTitle = _.wrap(test.fullTitle, function(origTitle) {
                var title;
                title = origTitle.apply(this);
                return title + " [" + test.cid + "]";
              });
            };
          })(this));
  ...
  this.App.module("Entities", function(Entities, App, Backbone, Marionette, $, _) {
    Entities.Stats = (function(_super) {
      ...
      Stats.prototype.countTestState = function(test) {
        return this.increment(test.state);
      };
...
(function() {
  ...
  this.App.module("Entities", function(Entities, App, Backbone, Marionette, $, _) {
    ...
    Entities.Suite = (function(_super) {
      ...
      Suite.prototype.anyArePending = function(states) {
      ...
      Suite.prototype.updateState = function() {
        ...
        state = (function() {
          switch (false) {
            case !this.anyArePending(states):
              return "pending";
  ...
  this.App.module("TestIframeApp.Show", function(Show, App, Backbone, Marionette, $, _) {
    return Show.Controller = (function(_super) {
      ...
      Controller.prototype.initialize = function(options) {
        var runner, socket, view;
        runner = options.runner;
        socket = App.request("socket:entity");
        view = this.getView();
        this.loadIframe = _.partial(this.loadIframe, view, runner);
        this.listenTo(socket, "test:changed", this.loadIframe);
        this.listenTo(runner, "load:iframe", this.loadIframe);
        return this.show(view);
      };
    ...
    return List.Controller = (function(_super) {
      ...
      Controller.prototype.initialize = function(options) {
        ...
        this.listenTo(runner, "test:end", function(test) {
          ...
          return suites.getSuiteByTest(test).updateState();
        });
        suitesView = this.getSuitesView(suites);
        this.listenTo(suitesView, "childview:suite:clicked", function(iv, args) {
          var suite;
          suite = args.model;
          suites.unchooseTests();
          return suite.choose();
        });
        this.listenTo(suitesView, "childview:childview:test:clicked", function(iv, iv2, args) {
          var chosen, test;
          test = args.model;
          chosen = suites.getFirstChosen();
          if (chosen) {
            suites.unchoose(chosen);
          }
          test.choose();
          return runner.setChosenId(test.id);
        });
  ...
  this.App.module("TestSpecsApp.List", function(List, App, Backbone, Marionette, $, _) {
    ...
    List.Test = (function(_super) {
      ...
      Test.prototype.stateChanged = function(model, value, options) {
        this.$el.removeClass("pending failed passed").addClass(value);
        if (value === "passed") {
          this.checkDuration();
        }
        if (value === "failed") {
    ...
    List.Suite = (function(_super) {
      ...
      Suite.prototype.stateChanged = function(model, value, options) {
        return this.$el.removeClass("pending failed passed").addClass(value);
      };
