  ...
  window.Eclectus = (function($, _) {
    ...
    methods = {
      ...
      assert: function(obj, passed, message, value, actual, expected) {
        ...
        return assertion.log(passed, message, value, actual, expected);
      },
      server: function(obj) {
        var server;
        this.sandbox._server = server = obj.contentWindow.sinon.fakeServer.create();
        this.sandbox.server = new Eclectus.Xhr(server, obj.contentWindow.document, obj.channel, obj.runnable);
  ...
  Eclectus.Dom = (function($, _, Eclectus) {
    ...
    Dom = (function(_super) {
      ...
      function Dom(document, channel, runnable) {
        ...
        this.instanceId = _.uniqueId("domInstance");
      }

      Dom.prototype.find = function(el) {
        var body;
        this.$el = this.$el ? this.$el.find(el) : this.$(el);
      ...
      Dom.prototype.within = function(el, fn) {
        var body;
        this.$el = this.$el ? this.$el.find(el) : this.$(el);
        this.selector = this.$el.selector;
        body = this.$("body").clone(true, true);
...
(function() {
  ...
  Eclectus.Xhr = (function($, _, Eclectus) {
    ...
    Xhr = (function(_super) {
      ...
      function Xhr(server, document, channel, runnable) {
        var _this;
        this.server = server;
        this.document = document;
        this.channel = channel;
        ...
        this.onRequests = [];
        _this = this;
        this.server.addRequest = _.wrap(this.server.addRequest, function(addRequestOrig, xhr) {
      ...
      Xhr.prototype.invokeOnRequest = function(xhr) {
        var found, response, _i, _len, _ref;
        found = false;
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          response.response(xhr, function(options) {
            found = true;
            return options.onRequest.call(xhr);
          });
        }
        ...
        return this.server.respondWith((function(_this) {
          return function(request, fn) {
            ...
            if (_this.requestMatchesResponse(request, options)) {
              ...
              body = _this.$("body").clone(true, true);
              body.find("script").remove();
              return _this.channel.trigger("xhr", _this.runnable, {
...
(function() {
  ...
  Eclectus.Assertion = (function($, _, Eclectus) {
    ...
    Assertion = (function(_super) {
      ...
      function Assertion(document, channel, runnable) {
        this.document = document;
        this.channel = channel;
        this.runnable = runnable;
        this.instanceId = _.uniqueId("instance");
      }

      Assertion.prototype.log = function(passed, message, value, actual, expected) {
        var body;
        body = this.$("body").clone(true, true);
        body.find("script").remove();
        return this.channel.trigger("assertion", this.runnable, {
          dom: body,
          method: "assert",
          value: value,
          message: message,
...
(function() {
  ...
  this.App.module("Utilities", function(Utilities, App, Backbone, Marionette, $, _) {
    var API, Reporter, df, overloadChaiAssertions, overloadMochaRunnableEmit, overloadMochaRunnerUncaught;
    df = null;
    overloadMochaRunnableEmit = function() {
      return Mocha.Runnable.prototype.emit = _.wrap(Mocha.Runner.prototype.emit, function(orig, event, err) {
        if (event === "error") {
          throw err;
        }
    ...
    overloadMochaRunnerUncaught = function() {
      return Mocha.Runner.prototype.uncaught = _.wrap(Mocha.Runner.prototype.uncaught, function(orig, err) {
        throw err;
    ...
    overloadChaiAssertions = function(Ecl) {
      return chai.use(function(_chai, utils) {
        return _chai.Assertion.prototype.assert = _.wrap(_chai.Assertion.prototype.assert, function() {
  ...
  this.App.module("Entities", function(Entities, App, Backbone, Marionette, $, _) {
    Entities.Command = (function(_super) {
      ...
      Command.prototype.setParent = function(parent) {
        this.parent = parent;
        return this.set("hasParent", true);
      };
      ...
      Command.prototype.setResponse = function(response) {
        ...
        this.set("response", this.xhr.responseText);
        this.set("method", "resp");
      ...
      Command.prototype.getAssertion = function() {
        var a;
        a = [];
        if (this.value !== this.actual || _.isUndefined(this.expected)) {
          a.push(["Subject:  ", this.value]);
        }
        if (!_.isUndefined(this.expected)) {
          a.push(["Expected: ", this.expected]);
          a.push(["Actual:   ", this.actual]);
        }
        a.push(["Message:  ", this.get("message")]);
        return a;
      };
    ...
    Entities.CommandsCollection = (function(_super) {
      ...
      CommandsCollection.prototype.parentExistsFor = function(instanceId) {
        var found;
        found = this.filter(function(command) {
          return command.isParent() && command.get("instanceId") === instanceId;
        });
        return _(found).last();
      };

      CommandsCollection.prototype.lastCommandIsNotRelatedTo = function(command) {
        return this.last().get("instanceId") !== command.get("instanceId");
      };

      CommandsCollection.prototype.insertParent = function(parent) {
        var clone;
        clone = parent.clone();
        _.each(["el", "dom", "xhr", "response", "parent"], function(prop) {
      ...
      CommandsCollection.prototype.addAssertion = function(attrs) {
        var actual, command, dom, expected, value;
        dom = attrs.dom, actual = attrs.actual, expected = attrs.expected, value = attrs.value;
      ...
      CommandsCollection.prototype.addDom = function(attrs) {
        ...
        command.dom = dom;
        command.el = el;
        if (parent = this.parentExistsFor(attrs.instanceId)) {
          command.setParent(parent);
          command.indent();
          if (this.lastCommandIsNotRelatedTo(command)) {
      ...
      CommandsCollection.prototype.addXhr = function(attrs) {
        var command, dom, parent, response, xhr;
      ...
      CommandsCollection.prototype.add = function(attrs, type, runnable) {
        ...
        try {
          if (attrs instanceof Entities.Command) {
            return CommandsCollection.__super__.add.call(this, attrs);
          }
        } catch (_error) {}
        if (_.isEmpty(attrs)) {
          return;
...
(function() {
  ...
  this.App.module("Entities", function(Entities, App, Backbone, Marionette, $, _) {
    ...
    Entities.Runnable = (function(_super) {
      ...
      Runnable.prototype.addCommand = function(command) {
        return this.get("commands").add(command);
      };
  ...
  this.App.module("Entities", function(Entities, App, Backbone, Marionette, $, _) {
    ...
    Entities.Runner = (function(_super) {
      ...
      Runner.prototype.setTestRunner = function(runner) {
        ...
        return this.runner.runSuite = _.wrap(this.runner.runSuite, function(runSuite, suite, fn) {
          ...
          suite.eachTest(function(test) {
            return _.each([test.parent, test], function(type) {
              var data, df;
              if (type.root || type.added) {
                return;
              }
              if (type.cid == null) {
              ...
              type.originalTitle = function() {
                return this.title.replace(runner.getIdToAppend(type.cid), "");
              };
              df = $.Deferred();
              df.done(function() {
                var event;
                ...
                type.added = true;
                event = type.type || "suite";
                return runner.trigger("" + event + ":add", type);
              });
              if (!type.cid) {
                type.fullTitle = _.wrap(type.fullTitle, function(origTitle) {
                  var title;
                  title = origTitle.apply(this);
                  return title + runner.getIdToAppend(type.cid);
                });
      ...
      Runner.prototype.runIframeSuite = function(iframe, contentWindow) {
        ...
        this.patchSandbox(this.contentWindow);
    ...
    API = {
      getRunner: function(testRunner, patch, sandbox) {
        var runner;
    ...
    return App.reqres.setHandler("runner:entity", function(testRunner, patch, sandbox) {
      return API.getRunner(testRunner, patch, sandbox);
    });
  });
  ...
  this.App.module("TestCommandsApp.List", function(List, App, Backbone, Marionette, $, _) {
    List.Command = (function(_super) {
      ...
      Command.prototype.onShow = function() {
        this.ui.wrapper.css("padding-left", this.model.get("indent"));
        if (this.model.hasParent()) {
          return this.ui.wrapper.addClass("command-child");
        } else {
          return this.$el.addClass("command-parent");
        }
      };
...
(function() {
  ...
  this.App.module("TestSpecsApp.List", function(List, App, Backbone, Marionette, $, _) {
    return List.Controller = (function(_super) {
      ...
      Controller.prototype.initialize = function(options) {
        var commands, container, root, runnablesView, runner, suites;
        runner = options.runner;
        ...
        this.listenTo(commands, "add", function(command) {
          var model;
          model = container.get(command.get("testId"));
          if (model) {
            return model.addCommand(command);
          }
        });
          ...
          if (runner.hasChosen()) {
            return container.each((function(_this) {
              return function(model) {
                if (model.isChosen()) {
                  model.reset();
                  return console.warn("model is reset", model);
                }
              };
            })(this));
          } else {
