...
At Cypress, we use two week sprints. During each sprint, a rotating group of developers is assigned to the triage team. Their responsibilities are only to focus on triaging issues entered into the [Cypres repo](https://github.com/cypress-io/cypress/). Each issue is assigned to a team member for analysis and assessment and follows this general path:  
...
- **Prioritization** - Teams meet regularly to review tickets that have been routed to them via triage. They evaluate each ticket based on the a number of criteria outlined in our [prioritization rubric](#how-we-prioritize-issues). Each dimension of our prioritization rubric carries a weight. The following equation is then used to give us a **relative priority**. The relative priority score is then divided by the estimated effort to address the ticket to give us a **priority-to-effort** score which we then use to determine which tickets are highest priority. 
...
| Severity | 1.0 | What does the problem prevent users from doing? Is it an edge case? A primary test flow? A wrong label or errant CSS? |
...
| Regression | 1.0 | Did this used to work? How long ago did it last work? Was this a regression within this last (current) major version? |
...
- **Not a bug or feature request** - Issues entered into the [Cypress repo](https://github.com/cypress-io/cypress) are for bugs and feature requests for the Cypress App only. Updates to [documentation](https://github.com/cypress-io/cypress-documentation), our [example-kitchensink app](https://github.com/cypress-io/cypress-example-kitchensink), or [another repository](https://github.com/cypress-io) should be made in the appripriate repository. 
The best place for asking questions is our [Discord server](https://on.cypress.io/discord) which has a very active community of folks with a diverse set of knowledge. Other available channels to explore include [Cypress GitHub discussions](https://github.com/cypress-io/cypress/discussions), [community chat](https://on.cypress.io/chat), and [Stack Overflow](https://stackoverflow.com/questions/tagged/cypress).
We also offer support via email with our [paid plans](https://www.cypress.io/pricing/).

- **Feature request for Cypress Dashboard** - Thank you for your support! The issue has been routed to our dashboard team's ticketing system and your customer success representative will be able to follow up with you directly via email.
