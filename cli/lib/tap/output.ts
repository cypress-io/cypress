...
export const renderFailure = (err: { code: string, message: string }): void => {
  // A failed connection is shown as a plain user-facing sentence; the other
  // codes keep their diagnostic prefix in the text output.
  logger.error(err.code === 'BINDING_NOT_FOUND' ? err.message : `${err.code}: ${err.message}`)
}
...
const INSTANCES_USAGE = [
  ...
  '',
  'Options:',
  '  -P, --project <project-path>  only list runners for this project',
