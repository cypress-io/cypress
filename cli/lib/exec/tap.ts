...
const validateSchema = (value: unknown): TapSchema => {
  ...
  if (!schema || typeof schema !== 'object' || typeof schema.protocolVersion !== 'number' || !Array.isArray(schema.commands)) {
    throw new TapTransportError(
      'INVALID_SCHEMA',
      `${TAP_SCHEMA_METHOD} returned an unrecognizable schema. The running Cypress version may not support \`cypress tap\`.`,
    )
  }
  ...
  if (schema.protocolVersion !== TAP_PROTOCOL_VERSION) {
    throw new TapTransportError(
      ...
      `The running Cypress version requires a more recent CLI. Update the CLI and try again.`,
    )
  }
...
const validateExecResult = (value: unknown): TapExecResult => {
  ...
  if (!outcome || typeof outcome !== 'object' || typeof outcome.ok !== 'boolean') {
    throw new TapTransportError(
      'INVALID_EXEC_RESULT',
      `${TAP_EXEC_METHOD} returned an unrecognizable result. The running Cypress version may not support \`cypress tap\`.`,
    )
  }
 ...
 * and defaults to all projects (the discovery flags only narrow it when
 * supplied). Renders the live runners as the same JSON `renderResult` uses for
 * command results, each keyed by the pid a user can hand back to `--instance`.
 */
const listInstances = async (options: TapCliOptions, wantsHelp: boolean): Promise<number> => {
  ...
  // Pass the raw `--project` through, NOT a cwd-defaulted root: an absent
  // `--project` means "every project", not "the current directory".
  const runners = await listLiveRunners({ projectRoot: options.project, instance: options.instance })
...
const tapModule = {
  async start (operands: string[] = [], options: TapCliOptions = {}): Promise<number> {
    ...
    try {
      ...
      const selection = await resolveRunner({ project: options.project, instance: options.instance, cwd: process.cwd() })
      ...
      if ((wantsHelp || !command) && err instanceof RunnerDiscoveryError) {
