...
const start = (options = {}) => {
  ...
  .then((binaryVersion) => {
    if (!binaryVersion) {
      ...
      Cannot read binary version from: ${chalk.cyan(
    state.getBinaryPkgPath(binaryDir)
  )}
    `)
    }

    debug(
      `Found binary version ${chalk.green(
        binaryVersion
      )} installed in: ${chalk.cyan(binaryDir)}`
    )
  ...
  binaryVersion
)} does not match the expected package version ${chalk.green(
  packageVersion
)}
