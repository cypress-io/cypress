...
describe('lib/tap/build-program', () => {
  ...
  it('registers the CLI-native `instances`, `status`, and `frame` commands first, then one subcommand per advertised command', () => {
    const program = buildTapProgram(schema, vi.fn())

    expect(program.commands.map((command) => command.name())).toEqual(['instances', 'status', 'frame', 'health', 'run', 'open'])
  })
