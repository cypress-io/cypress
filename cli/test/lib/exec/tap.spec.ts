...
describe('lib/exec/tap', () => {
  ...
  describe('dispatching a command', () => {
    ...
    it('resolves the target from --project, --instance, and the cwd, then opens a session against it', async () => {
      mockSession()
      const { runner } = mockResolved()

      await tap.start(['health'], { project: 'some/relative/dir', instance: 1234 })

      // --project is passed raw (resolveRunner owns path resolution); the cwd is
      // only a tiebreak it falls back to.
      expect(resolveRunner).toHaveBeenCalledWith({ project: 'some/relative/dir', instance: 1234, cwd: process.cwd() })
    ...
    it('passes no project filter when --project is absent, leaving the cwd as the only tiebreak', async () => {
      mockSession()

      await tap.start(['health'], {})

      expect(resolveRunner).toHaveBeenCalledWith({ project: undefined, instance: undefined, cwd: process.cwd() })
    })
  ...
  describe('the CLI-native instances command', () => {
    ...
    it('forwards --project and --instance as discovery filters, raw (no cwd default)', async () => {
      vi.mocked(listLiveRunners).mockResolvedValue([])

      await tap.start(['instances'], { project: 'some/relative/dir', instance: 1234 })

      expect(listLiveRunners).toHaveBeenCalledWith({ projectRoot: 'some/relative/dir', instance: 1234 })
    })

    it('lists across all projects by default — no project filter when --project is absent', async () => {
      vi.mocked(listLiveRunners).mockResolvedValue([])

      await tap.start(['instances'], {})

      expect(listLiveRunners).toHaveBeenCalledWith({ projectRoot: undefined, instance: undefined })
    })
  ...
  describe('failure rendering', () => {
    ...
    it('renders transport errors with their code and exits 1', async () => {
      mockSession()
      failSession(new TapTransportError('CDP_UNREACHABLE', 'Could not reach the browser.'))

      expect(await tap.start(['health'], {})).toBe(1)
      expect(logger.print()).toBe('CDP_UNREACHABLE: Could not reach the browser.')
    })

    it('renders a failed connection as a plain message, without the diagnostic code', async () => {
      mockSession()
      failSession(new TapTransportError('BINDING_NOT_FOUND', 'Failed to connect to Cypress. The runner may still be loading.'))

      expect(await tap.start(['health'], {})).toBe(1)
      expect(logger.print()).toBe('Failed to connect to Cypress. The runner may still be loading.')
    })
