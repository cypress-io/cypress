...
describe('exec open', function () {
  context('.start', function () {
    ...
    it('calls spawn with correct options', function () {
      return open.start({ dev: true }).then(() => {
    ...
    it('spawns with --config-file false', function () {
      return open.start({ configFile: false }).then(() => {
        expect(spawn.start).to.be.calledWith(['--config-file', false])
      })
    })

    it('spawns with --config-file set', function () {
      return open.start({ configFile: 'special-cypress.json' }).then(() => {
        expect(spawn.start).to.be.calledWith(['--config-file', 'special-cypress.json'])
      })
    })
    ...
    it('spawns without --project if not installed globally and passing --global option', function () {
      ...
      return open.start({ global: true }).then(() => {
        expect(spawn.start).not.to.be.calledWith(['--project', process.cwd()])
      })
    })
