...
describe('lib/exec/versions', function () {
  ...
  beforeEach(function (): void {
    ...
    state.getBinaryDir.mockImplementation(() => {
      return binaryDir
    })
    ...
    // @ts-expect-error - mockImplementation
    util.pkgVersion.mockImplementation(() => '4.5.6')

    // @ts-expect-error - mockImplementation
    util.pkgBuildInfo.mockImplementation(() => ({ stable: true }))
  })
  ...
  describe('.getVersions', () => {
    ...
    // @ts-expect-error - mockImplementation
      util.pkgBuildInfo.mockImplementation(() => ({ stable: false }))
    ...
    // @ts-expect-error - mockImplementation
      util.pkgBuildInfo.mockImplementation(() => undefined)
      const version = await versions.getVersions()
