...
- Fixed an issue in Chrome and Electron where [`cy.setCookie()`](https://on.cypress.io/setcookie) appeared to ignore the cookie properties it was given (for example `expiry`) when a cookie with the same name already existed on a host-only or parent domain (such as an `httpOnly` and `secure` cookie set by the server). Cypress now reads the cookie back from the exact domain it was written to, so the resolved cookie reflects what was actually set. Fixes [#5599](https://github.com/cypress-io/cypress/issues/5599).
