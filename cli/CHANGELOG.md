...
- Fixed an issue where a command whose promise rejected with a value that was not an error object (for example `cy.wrap(null).then(Cypress.Promise.reject)`, which rejects with `null`) failed with a misleading internal Cypress error instead of failing the test gracefully. Such rejections now surface a clear `An unknown error has occurred` message. Fixes [#3283](https://github.com/cypress-io/cypress/issues/3283).
