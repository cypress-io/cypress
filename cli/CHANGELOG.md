...
- Fixed an issue where, with [`experimentalRunAllSpecs`](https://docs.cypress.io/app/references/experiments) enabled, the file watchers created when running "Run All Specs" were not cleaned up after the selected subset of specs changed. Changing a file that was no longer part of the active subset would still trigger a reload. The watchers for specs that are no longer being run are now torn down. Fixes [#24856](https://github.com/cypress-io/cypress/issues/24856).
