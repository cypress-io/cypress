...
- Fixed an issue where calling a reporter helper such as mochawesome's `addContext(this, ...)` directly inside a test did not attach the context to the generated report. Context added to a test is now passed through to the reporter, so it appears in the report for both passing and failing tests. Fixes [#3654](https://github.com/cypress-io/cypress/issues/3654).
