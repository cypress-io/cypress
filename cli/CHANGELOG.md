...
- Fixed an intermittent `ENOENT: no such file or directory, open <path>/bundle.tar-<rand>` error during `cy.prompt` and Studio bundle initialization, caused by OS-level temp-dir cleaners racing the download/extract window. Bundles now stream-verify and extract directly into `<cypress-cache>/bundles/<kind>/`, removing the on-disk tar artifact; `cypress cache clear` removes them and `cypress cache prune` ignores them. Fixed in [#33748](https://github.com/cypress-io/cypress/pull/33748/).
