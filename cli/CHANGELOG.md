...
- Fixed an issue in `cypress open` where, when Cypress could not connect to a Chromium-based browser (for example, when remote debugging is disabled by the `RemoteDebuggingAllowed` enterprise or group policy), the launchpad would spin indefinitely on "Opening …" with the failure only reported in the terminal. The connection error is now surfaced in the GUI and the browser status is reset so you can try again. Fixes [#32671](https://github.com/cypress-io/cypress/issues/32671).
