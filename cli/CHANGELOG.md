...
- Fixed an issue where [`cy.contains()`](https://on.cypress.io/contains) did not find `input[type=submit]` elements by a partial substring of their `value`. For example, `cy.contains('User')` found a button labeled `Username` but failed against an `input[type=submit]` with `value="Username"`. Submit inputs are now matched by substring of their value, the same as other elements. Fixes [#34305](https://github.com/cypress-io/cypress/issues/34305).
