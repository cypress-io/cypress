...
- [`cy.request()`](https://on.cypress.io/request) now accepts a `cookies` option. When set to `false`, Cypress no longer synchronizes cookies with the browser for that request — it skips reading the browser's cookies onto the outgoing request and skips persisting `Set-Cookie` response headers back to the browser, including across redirects. This avoids the extra browser round-trips for requests that do not rely on the browser's session and can make those requests noticeably faster. The option defaults to `true`, so existing behavior is unchanged. Addresses [#23475](https://github.com/cypress-io/cypress/issues/23475).
