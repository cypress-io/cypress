...
- The browser-emitted `ResizeObserver loop completed with undelivered notifications.` and `ResizeObserver loop limit exceeded` messages no longer fail tests or appear as uncaught exceptions in the command log. These are benign notifications the browser fires when resize observations cannot settle within a single frame, so Cypress now ignores them by default without the need for a `Cypress.on('uncaught:exception')` handler. Fixes [#31479](https://github.com/cypress-io/cypress/issues/31479).
