...
- [`scrollBehavior`](https://docs.cypress.io/app/references/configuration#Actionability) now accepts a per-axis value, such as `{ block: 'start', inline: 'nearest' }`, so the vertical and horizontal alignment used to scroll an element into view before an action command can be set independently. The per-axis form takes the same values as the native `scrollIntoView`: `'start'`, `'end'`, `'center'`, and `'nearest'`. A single value still applies to both axes, and `'start'` and `'end'` are now accepted there as well. Addresses [#34460](https://github.com/cypress-io/cypress/issues/34460).

**Bugfixes:**

- Fixed a regression in [15.18.1](#15-18-1) where action commands (such as [`.click()`](https://on.cypress.io/click) and [`.type()`](https://on.cypress.io/type)) also scrolled the element's scrollable container horizontally, which could shift layouts the application positions itself, such as carousels, even when the element was already visible. The `'top'` and `'bottom'` values of [`scrollBehavior`](https://docs.cypress.io/app/references/configuration#Actionability) again align only the vertical axis, matching the behavior of `scrollIntoView(alignToTop)`. To align both axes as 15.18.1 through 15.19.1 did, use `'start'` or `'end'`. Fixes [#34460](https://github.com/cypress-io/cypress/issues/34460).
