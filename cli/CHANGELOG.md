...
- Fixed an issue in Chrome and Electron where a run could intermittently hang between tests when a new tab or popup was opened (for example via `window.open` or a `target="_blank"` link), with no error or timeout. The run now continues as expected. Fixes [#32956](https://github.com/cypress-io/cypress/issues/32956).
