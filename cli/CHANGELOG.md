...
- Fixed a memory leak in the proxy's service worker request correlation where pre-requests that were potentially controlled by a service worker but never received a matching fetch event (for example, slow or `404` requests such as missing source maps) were retained until the spec ended. On pages that issue many such requests, this could contribute to the proxy process running out of memory over the course of a long spec. Addresses [#33296](https://github.com/cypress-io/cypress/issues/33296).
