...
- Fixed an issue where the `Proxy-Connection` request header that the browser adds when routing through Cypress's proxy was forwarded to the origin server, causing some backends to reject requests made from the application under test. The header is now stripped before requests are sent to the origin. Fixes [#7290](https://github.com/cypress-io/cypress/issues/7290).
