...
- Fixed an issue where, with test retries enabled, `cypress run` printed the attempt line twice for a single attempt when both a `beforeEach` and an `afterEach` hook failed during that attempt, making the attempt counter look broken. Each attempt is now printed only once. Fixes [#26143](https://github.com/cypress-io/cypress/issues/26143).
