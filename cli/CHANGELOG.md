...
- Fixed an issue where [`cy.visit()`](https://on.cypress.io/visit) could hang indefinitely with no error when a dynamic `baseUrl` (for example, a different subdomain set inside a `before` or `beforeEach` hook) caused every run of the spec to resolve the visit to a new origin, putting the test runner into an infinite reload loop. Cypress now detects the loop and fails with a clear cross-origin error that points to [`cy.origin()`](https://on.cypress.io/origin) and the `injectDocumentDomain` option. Fixes [#33233](https://github.com/cypress-io/cypress/issues/33233).
