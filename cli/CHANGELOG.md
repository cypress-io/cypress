...
- Fixed an issue where [`cy.contains()`](https://on.cypress.io/contains) inside a [`cy.document().within()`](https://on.cypress.io/within) block failed with `TypeError: subject.each is not a function` rather than searching the document. Fixes [#24178](https://github.com/cypress-io/cypress/issues/24178).
