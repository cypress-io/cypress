...
- Fixed an issue where Angular component testing failed to type-check application code that referenced global type augmentations (for example, declaring `Cypress` on `Window` in a `src/*.d.ts` file), reporting errors such as `Property 'Cypress' does not exist on type 'Window & typeof globalThis'`. The generated TypeScript configuration now preserves the project's ambient declaration files. Fixes [#23940](https://github.com/cypress-io/cypress/issues/23940).
