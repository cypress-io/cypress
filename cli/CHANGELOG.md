...
- Fixed an issue where the browser's renderer process could crash (`We detected that the Chrome Renderer process just crashed`) when the application under test rendered a large number of dynamically-shown form fields with a dropdown open (for example, an Element Plus dialog with roughly ten or more fields) while the command log was visible. The command log now renders in an isolated document so it no longer shares a layout pass with the application under test. Fixes [#33962](https://github.com/cypress-io/cypress/issues/33962).
