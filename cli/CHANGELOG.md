...
- Fixed an issue where commands like [`cy.task()`](https://on.cypress.io/task), [`cy.exec()`](https://on.cypress.io/exec), [`cy.readFile()`](https://on.cypress.io/readfile), and [`cy.writeFile()`](https://on.cypress.io/writefile) could incorrectly fail with an error such as "`cy.task()` must only be invoked from the spec file or support file" when called from a custom command or a [`cy.origin()`](https://on.cypress.io/origin) callback. These commands now run as expected in those cases. Fixes [#27784](https://github.com/cypress-io/cypress/issues/27784).
