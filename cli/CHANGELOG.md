...
- Fixed an issue where the `before:spec` plugin event fired twice for the same spec when `cy.visit()` triggered a cross-origin navigation (or no `baseUrl` was set), causing the runner to reload the top window mid-spec. The event now fires exactly once per spec regardless of any mid-spec page reloads. Fixes [#26300](https://github.com/cypress-io/cypress/issues/26300).
