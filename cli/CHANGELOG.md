...
- When [`test isolation`](https://docs.cypress.io/guides/core-concepts/test-isolation) is on, navigation to `about:blank` will now occur at the end of tests rather than at the beginning of tests. This may affect test behavior when you are turning `test isolation` off and on in the same spec.
