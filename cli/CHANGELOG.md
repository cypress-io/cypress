...
- Fixed an issue where launching Cypress with `--browser <path>` pointing at a custom browser in a non-default location (for example a downloaded Chromium build) could display the wrong browser in the GUI, or launch a same-family browser installed in the default location instead of the one provided. The custom browser is now shown and selected correctly. Fixes [#25755](https://github.com/cypress-io/cypress/issues/25755).
