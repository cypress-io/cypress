...
- Fixed an issue where a WebSocket connection opened by your application could intermittently fail with `Connection closed before receiving a handshake response`, most often on the first connection to a host or after a period of inactivity. Proxied WebSocket upgrades are no longer served from a keep-alive connection pool, so a stale, already-closed socket can no longer be reused for the upgrade. Fixes [#7664](https://github.com/cypress-io/cypress/issues/7664).
