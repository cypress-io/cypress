...
- Fixed an issue where running a TypeScript spec that compiled to a modern target such as `ES2022` (the default for Angular 15+) could fail to bundle with `Module parse failed: Unexpected character '#'`, or emit class private fields and optional chaining that older supported browsers could not run. TypeScript spec output is now downleveled to the same browser baseline as JavaScript specs. Fixes [#26554](https://github.com/cypress-io/cypress/issues/26554).
