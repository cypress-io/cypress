...
- Fixed an issue in Chrome and Electron where a run could intermittently hang between tests when a new tab or popup was opened (for example via `window.open` or a `target="_blank"` link). Such targets are attached in a paused state, and if Cypress's connection to one hung, the target was never resumed and the run stalled indefinitely. Connecting to these targets is now time-bounded so the target is always resumed and the run continues. Fixes [#32956](https://github.com/cypress-io/cypress/issues/32956).
