...
- Removed the `experimentalSourceRewriting` configuration option and the experimental AST-based source rewriter it enabled. The default regex-based source rewriting, which has always been used when the option was not enabled, is now the only implementation, so default behavior is unchanged. Remove the option from your config if you have it set. Addresses [#34213](https://github.com/cypress-io/cypress/issues/34213).
