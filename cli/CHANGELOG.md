...
- Fixed an issue where focusing the browser window from the Cypress App (for example, via the **Focus the browser** action while Firefox is open) did nothing on Linux. Cypress now brings the Firefox window to the foreground on Linux using `xdotool` when it is available. Fixes [#21703](https://github.com/cypress-io/cypress/issues/21703).
