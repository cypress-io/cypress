...
- `cypress run` now reports how many tests were flaky. A test is flaky when [test retries](https://on.cypress.io/test-retries) produced both a failed and a passed attempt for it. The per-spec `(Results)` table gains a `Flaky` row and the final `(Run Finished)` summary table gains a `Flaky` column, both shown only when the run produced flake. The count is also available programmatically as `stats.flaky` on each run result and `totalFlaky` on the run result of the [Module API](https://on.cypress.io/module-api), [`after:spec`](https://on.cypress.io/after-spec-api), [`after:run`](https://on.cypress.io/after-run-api) and the JSON written by [`--output-path`](https://on.cypress.io/command-line#cypress-run-output-path-lt-path-gt). Addressed in [#33865](https://github.com/cypress-io/cypress/pull/33865).
