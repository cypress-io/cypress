...
- Fixed an issue where, during a [`cy.origin()`](https://on.cypress.io/origin) block, session cookies set on the primary origin were not included in the identity provider's callback request back to the primary origin (for example, `POST /auth/callback` in an OAuth Authorization Code flow). Chrome's CDP frame tree can be transiently stale during a cross-origin navigation, causing the proxy to miss the `X-Cypress-Is-AUT-Frame` signal on the redirect-follow request and drop the primary-domain cookies. The proxy now detects this redirect-back scenario explicitly and re-attaches the primary-domain cookies as expected. Fixes [#29719](https://github.com/cypress-io/cypress/issues/29719). Fixed in [#34059](https://github.com/cypress-io/cypress/pull/34059).
