...
- Fixed an issue where WebKit launched using the host machine's `devicePixelRatio` instead of a standard value of `1`. WebKit now uses a `devicePixelRatio` of `1`, matching Chrome and Electron, so screenshots are consistent regardless of the host machine's DPI (for example 2x locally versus 1x in CI) and text is no longer fuzzy on high-DPI displays. This only applies when running with [`experimentalWebKitSupport`](https://docs.cypress.io/app/references/experiments) enabled. Fixes [#23808](https://github.com/cypress-io/cypress/issues/23808).
