...
- Fixed an issue where a failing assertion on a spy or stub with a very large call history (for example a spy on a frequently-invoked method such as `window.postMessage`) could appear to hang for many minutes instead of failing after the assertion timeout, because the failure message serialized every recorded call on each retry. The number of calls listed in the message is now capped, so the assertion fails promptly with a truncated call list. Fixes [#15005](https://github.com/cypress-io/cypress/issues/15005).
