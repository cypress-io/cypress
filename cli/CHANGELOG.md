...
- Fixed an issue where a command that failed immediately after a `.then()` callback containing an assertion could display a code frame that pointed at the previous `.then()` instead of the command that actually failed. Fixes [#31505](https://github.com/cypress-io/cypress/issues/31505).
