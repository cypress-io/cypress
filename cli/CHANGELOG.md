...
- Fixed an issue where a dependency loaded with [`Cypress.require()`](https://on.cypress.io/origin) inside a [`cy.origin()`](https://on.cypress.io/origin) callback could throw `process is not defined` (for example, when requiring `otplib`) even with [`experimentalOriginDependencies`](https://docs.cypress.io/app/references/experiments) enabled. The webpack `process` and `Buffer` global injections are now preserved when bundling the callback. Fixes [#30646](https://github.com/cypress-io/cypress/discussions/30646).
