...
- Fixed an issue where navigating the browser history across origins with [`cy.go()`](https://on.cypress.io/go) (for example `cy.go('back')` or `cy.go('forward')`) did not preserve the current run state before the runner reloaded on the new origin, causing the spec to restart from the first test. The run state is now preserved on a cross-origin history navigation, matching [`cy.visit()`](https://on.cypress.io/visit), so the in-progress test resumes as expected. Fixes [#23736](https://github.com/cypress-io/cypress/issues/23736).
