...
- Fixed an issue where focusing the browser window from the Cypress App (for example, via the **Focus the browser** action while Firefox is open) did nothing, most notably on Linux. Cypress now focuses the Firefox window through the native WebDriver BiDi `browsingContext.activate` command, matching how Chromium-based browsers are focused. Fixes [#21703](https://github.com/cypress-io/cypress/issues/21703).
