...
- Fixed a race during `cypress open` config reload where the internal HTTP server would begin accepting requests before the primary remote state had been initialized, occasionally crashing in-flight browser requests (iframe loads, app HTML bootstrap, proxied responses) on an empty `remoteStates` map. The server now creates the file server and sets the primary remote state synchronously after `listen()` resolves and before any further async work, eliminating the window in which a request could route without a primary.
