...
- Fixed a regression in [14.0.0](#14-0-0) where every message sent from the Cypress server to a Chromium-based browser (including Electron) permanently retained memory in the browser's renderer process until the page navigated or reloaded. During long specs — especially with [`testIsolation`](https://docs.cypress.io/app/core-concepts/writing-and-organizing-tests#Test-Isolation) disabled — this buildup could exhaust the renderer's memory and crash the browser (`We detected that the Chrome Renderer process just crashed`). Fixes [#34226](https://github.com/cypress-io/cypress/issues/34226).
