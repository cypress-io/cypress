...
- Fixed a race during Firefox launch where geckodriver returned from `newSession` before the WebDriver BiDi WebSocket was ready, causing the first BiDi command (`sessionSubscribe`) to fail with `No connection to WebDriver Bidi was established` and exit the run with code 1 before any spec executed. The BiDi subscribe is now retried briefly until the connection is established, and the launch-retry path also recovers from this transient failure. Fixed in [#33770](https://github.com/cypress-io/cypress/pull/33770).
