...
- Fixed an issue where, during `cypress run` with [`experimentalSingleTabRunMode`](https://docs.cypress.io/app/references/experiments) and the experimental WebKit browser, only the first spec's video was recorded for component tests. Each spec is now recorded to its own video. Fixes [#23815](https://github.com/cypress-io/cypress/issues/23815).
