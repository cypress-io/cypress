...
- Fixed an issue where, with the `experimentalSourceRewriting` option enabled, a `location = ...` assignment to a locally-scoped variable named `location` (for example `var location` declared inside a function, as used by react-router's `history.js`) was incorrectly rewritten to set the global location's `href`. This left the local variable undefined and could cause commands such as `cy.visit()` to hang until timing out. Only assignments to the global `location` are now rewritten. Fixes [#7906](https://github.com/cypress-io/cypress/issues/7906).
