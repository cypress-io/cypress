...
- Fixed an issue in Chromium-based browsers where [`cy.go()`](https://on.cypress.io/go) could navigate to a browser history entry belonging to Cypress rather than to your application, unloading Cypress mid-spec and restarting or hanging the run. This happened when the application under test had no page left to navigate to, most often after a [`cy.visit()`](https://on.cypress.io/visit) to a new origin, which reloads Cypress at that origin. `cy.go()` now fails with an error instead of navigating. Fixes [#23736](https://github.com/cypress-io/cypress/issues/23736).
