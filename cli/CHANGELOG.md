...
- Fixed two situations that produced an unhelpful `TypeError: The "path" argument must be of type string. Received undefined` crash at startup. First, when argument parsing left `projectRoot` unset (for example, because an unset CI environment variable caused an adjacent CLI flag to consume the `--project` token), Cypress now falls back to the current working directory instead of passing `undefined` to every downstream `path.join` call. Second, passing `--browser` without a value resulted in a boolean `true` reaching `path.basename`, which also threw; the check now guards the type before calling `path.basename` and falls through to a clear "browser not found" diagnostic. Fixes [#15418](https://github.com/cypress-io/cypress/issues/15418).
