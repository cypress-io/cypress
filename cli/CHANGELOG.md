...
- Fixed a race during `cypress open` config reload where rapid changes to `cypress.config.js` could leave the specs list stale. Two overlapping config-reload calls would tear down each other's IPC child processes (surfacing as `ERR_STREAM_DESTROYED`), causing both reloads to fail and the specs list to never refresh. Config reloads are now serialized so concurrent triggers coalesce into a single follow-up reload after the in-flight one drains. Fixed in [#33775](https://github.com/cypress-io/cypress/pull/33775).
