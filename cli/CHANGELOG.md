...
- Upgraded bundled Node.js version from `18.17.0` to `20.18.0`.
- It is no longer possible to do things like `cy.window().then((win) => win.fetch('<some-url>')` without doing some form of initial navigation via `cy.visit()`. `cy.request` will still work in this scenario.

**Dependency Updates:**

- Upgraded `electron` from `27.3.10` to `32.2.0`.
- Upgraded bundled Chromium version from `118.0.5993.159` to `128.0.6613.178`.
