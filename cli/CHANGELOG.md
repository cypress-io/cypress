...
- Fixed an issue where screenshots taken while using the "Run All Specs" feature were all saved under a single `All E2E Specs` directory instead of under a directory matching each screenshot's own spec. Cypress now saves each screenshot under its originating spec's directory. Fixes [#2319](https://github.com/cypress-io/cypress/issues/2319).
