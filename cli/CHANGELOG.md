...
- Fixed an issue where [`cy.selectFile()`](https://on.cypress.io/selectfile) with a file larger than roughly 400 MB would crash the browser — a fatal `SIGSEGV` in Electron — because the file's contents exceeded the maximum size that can be transferred to the browser. Cypress now fails with a clear error naming the file and its size instead of crashing. Fixes [#24583](https://github.com/cypress-io/cypress/issues/24583).
