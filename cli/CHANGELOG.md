...
- Fixed an issue where the console output printed when clicking a [`cy.env()`](https://on.cypress.io/env) command in the Command Log listed the requested environment variable names under numbered keys and placed them above the `Command:` header. The names are now printed under a single `Env vars` label. Addressed in [#34615](https://github.com/cypress-io/cypress/pull/34615).
