...
- Fixed an issue where a `cy.*` command error message that interpolated a value containing a `$` character (for example an error echoing input such as `price $5`) could render incorrectly, with a leaked `{{...}}` placeholder or duplicated or dropped surrounding text. Such values are now inserted verbatim. Fixed in [#34221](https://github.com/cypress-io/cypress/pull/34221).
