...
- Fixed an issue where opening an unconfigured project from a sub-directory of a git repository (such as a package within a monorepo) could skip project setup: `cypress open --component` would ask you to pick a framework even though one was installed, and `cypress open --e2e` would hang on "Initializing config…". The framework is now detected and the config is scaffolded as they are when opening the project through the launchpad or from the repository root. Fixes [#27410](https://github.com/cypress-io/cypress/issues/27410) and [#29544](https://github.com/cypress-io/cypress/issues/29544).
