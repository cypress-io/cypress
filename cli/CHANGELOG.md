...
- When `cy.visit()` fails because the server did not send its initial response in time (`ESOCKETTIMEDOUT`/`ETIMEDOUT`), Cypress now shows a dedicated error message that reports the configured `responseTimeout`, explains that this timeout is governed by `responseTimeout` (not `pageLoadTimeout` or the `timeout` option), and shows how to increase it. Other network failures continue to show the existing message. The `responseTimeout` option is now also documented on the `cy.visit()` TypeScript definition. Addresses [#7062](https://github.com/cypress-io/cypress/issues/7062).
