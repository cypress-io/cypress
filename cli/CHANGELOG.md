...
- Fixed an issue where a failing assertion on a Sinon spy or stub (for example `cy.get('@postMessage').should('be.calledWithMatch', ...)`) could appear to hang for many minutes instead of failing after the assertion timeout. This happened when the spy was attached to a frequently-invoked method (such as `window.postMessage`) and accumulated a very large call history, which Sinon serialized in full into the failure message on every retry. The number of calls listed in the message is now capped so message construction stays bounded. Fixes [#15005](https://github.com/cypress-io/cypress/issues/15005).
