...
- Fixed an issue where a custom reporter (such as `mochawesome`) installed in a parent directory's `node_modules` could not be loaded in nested projects or monorepos, causing the run to fail with `Could not load reporter by name`. Such reporters are now found and loaded as expected. Fixes [#4536](https://github.com/cypress-io/cypress/issues/4536).
