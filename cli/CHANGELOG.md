...
- Rewrote the experimental `experimentalFastVisibility` algorithm to delegate to the browser's native `Element.checkVisibility()` with a zero-dimension guard. Shadow DOM and slotted content are now supported. The algorithm no longer detects ancestor `overflow` clipping, scroll-out-of-bounds, sibling occlusion, or `clip-path` clipping as hidden. See the [migration guide](https://github.com/cypress-io/cypress/blob/develop/packages/driver/src/dom/visibility/MIGRATION_GUIDE.md). Fixes [#33046](https://github.com/cypress-io/cypress/issues/33046). Addressed in [#33781](https://github.com/cypress-io/cypress/pull/33781).
