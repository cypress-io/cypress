...
- Fixed an issue where [`cy.session()`](https://on.cypress.io/session) could intermittently fail with `cy.then() timed out after waiting 20000ms` and `Your callback function returned a promise that never resolved`, most often when test isolation was enabled. Fixes [#31988](https://github.com/cypress-io/cypress/issues/31988).
