...
- [`blockHosts`](https://on.cypress.io/configuration#blockHosts) can no longer be set via [`Cypress.config()`](https://on.cypress.io/config) during test execution. Mutating it this way affected the *next* test rather than the current one. Set `blockHosts` in the test configuration of a `describe`/`context` or `it` block instead.
