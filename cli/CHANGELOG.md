...
- Fixed an issue where a missing or disconnected Cypress browser extension could block the runner with a "The Cypress extension is missing." or "The Cypress extension has disconnected." screen in Chromium-based browsers (Chrome, Edge, Electron) and WebKit. These browsers automate via CDP/BiDi and do not require the extension, so the blocking views are now only shown for Firefox. Addresses [#28932](https://github.com/cypress-io/cypress/issues/28932).
