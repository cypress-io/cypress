...
- Fixed an issue where focusing the browser window from the Cypress App (for example, via the **Focus the browser** action while Firefox is open) did nothing, most notably on Linux. The Firefox window is now brought to the foreground as expected. Fixes [#21703](https://github.com/cypress-io/cypress/issues/21703).
