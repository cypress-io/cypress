...
- Fixed an issue where an error thrown from a [`before:browser:launch`](https://on.cypress.io/browser-launch-api) plugin handler caused Cypress to hang while launching the browser, with no indication of what went wrong. Cypress now surfaces the error in the launcher instead of hanging. Fixes [#32775](https://github.com/cypress-io/cypress/issues/32775).
