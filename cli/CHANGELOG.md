...
- Fixed an issue where Cypress reported the WebKit browser version as 0 (for example, "WebKit 0") with `playwright@1.60.0` and newer, which no longer ship the `wkBrowser.js` source file the version was read from. The WebKit version is now also read from `playwright-core`'s `browsers.json` as a fallback. Fixes [#33953](https://github.com/cypress-io/cypress/issues/33953).
