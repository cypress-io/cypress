...
- TypeScript now accepts every function [`cy.clock()`](https://on.cypress.io/clock) can override in the browser, such as `requestAnimationFrame`, `requestIdleCallback`, `performance`, and `Intl`, and accepts `null` as the first argument, as in `cy.clock(null, ['setTimeout', 'clearTimeout'])`. These calls previously reported a type error. The `tick()` method on the yielded clock is now typed as returning the clock's new time in milliseconds instead of `void`.
