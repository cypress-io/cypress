...
- Fixed an issue where a form created dynamically and submitted programmatically via `form.submit()` with a `target` of `_top` or `_parent` would navigate out of the Cypress test iframe and send the runner back to the spec list. Such submissions now stay within the application under test. Fixes [#26029](https://github.com/cypress-io/cypress/issues/26029).
