...
- Fixed an issue where a failing test with a large command log slowed down every subsequent test in the same run — including during `cypress run` — because the reporter kept each prior failure's command log mounted in the DOM, making the per-command auto-scroll force an increasingly expensive layout reflow. The reporter now only renders a finished test's command log while it is scrolled into view, so performance stays consistent regardless of earlier failures. Fixes [#6881](https://github.com/cypress-io/cypress/issues/6881).
