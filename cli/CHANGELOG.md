...
- Fixed an issue where visiting a `localhost` virtual host such as [`cy.visit('http://myapp.localhost')`](https://on.cypress.io/visit) failed with a network-level error even though the same URL loaded in the browser. Per RFC 6761, browsers resolve `localhost` and any `*.localhost` subdomain to the loopback address, but the operating system resolver only handles the bare `localhost` label. Cypress now resolves these names to loopback itself when the system cannot, while still honoring any `hosts` configuration or `/etc/hosts` override. Fixes [#24458](https://github.com/cypress-io/cypress/issues/24458).
