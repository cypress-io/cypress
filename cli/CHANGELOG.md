...
- Fixed an issue where calling [`cy.contains()`](https://on.cypress.io/contains) inside a [`cy.document().within()`](https://on.cypress.io/within) block threw a `TypeError` instead of failing with the expected assertion error when the content could not be found. Fixes [#24178](https://github.com/cypress-io/cypress/issues/24178).
