...
- Fixed an issue where, when a browser failed to launch in `cypress open`, the launch screen would hang indefinitely on "Opening..." with no explanation. Cypress now displays the launch error and resets the launch button so it can be retried. Fixes [#24372](https://github.com/cypress-io/cypress/issues/24372).
