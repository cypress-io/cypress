...
- Fixed an issue where [`Cypress.spec`](https://docs.cypress.io/api/cypress-api/spec) returned the placeholder `{ name: 'All E2E Specs', relative: '__all', absolute: '__all' }` for the entire run when using the experimental "Run all specs" feature, instead of the spec currently executing. `Cypress.spec` now reflects the spec each test belongs to, so plugins and code that derive file names (such as snapshots or screenshots) from it work correctly when running all specs. Fixes [#3090](https://github.com/cypress-io/cypress/issues/3090).
- Fixed an issue where screenshots taken while using the experimental "Run all specs" feature were all saved under a single `All E2E Specs` directory instead of under a directory matching each screenshot's own spec. Cypress now saves each screenshot under its originating spec's directory. Fixes [#2319](https://github.com/cypress-io/cypress/issues/2319).
