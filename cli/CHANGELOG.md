...
- Fixed an issue where the TypeScript 7 support added in [15.18.0](#15-18-0) did not work in the released binary: every TypeScript spec and support file in a project using TypeScript 7 failed to compile with `Error: Cannot find module '@babel/preset-typescript'`, because the copy of `@babel/preset-typescript` bundled in the Cypress binary was missing its `package.json`. Fixes [#34359](https://github.com/cypress-io/cypress/issues/34359).
