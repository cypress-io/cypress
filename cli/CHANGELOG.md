...
- Fixed an issue where [`cy.visit()`](https://on.cypress.io/visit) and [`cy.request()`](https://on.cypress.io/request) did not normalize redirect `Location` URLs that contained dot-segments (for example a redirect to `http://localhost:3000/./page`), so Cypress followed the literal path rather than the normalized path a browser would request. This could result in unexpected `404` responses that only appeared under test. Redirect URLs are now normalized before being followed, matching browser behavior. Fixes [#31890](https://github.com/cypress-io/cypress/issues/31890).
