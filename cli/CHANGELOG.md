...
- Fixed an issue where clicking "back to projects" or switching projects while a project's initial config load was still in flight could surface a sticky red `Expected projectRoot to be set in ProjectLifecycleManager` error overlay that did not clear on its own. Fixed in [#33810](https://github.com/cypress-io/cypress/pull/33810).
