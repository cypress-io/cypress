...
- Fixed an issue where `cypress run` could log a `We failed to trash the existing run results` warning with a stack trace when clearing previous screenshots, videos, or downloads (for example, on macOS running under Apple Silicon without Rosetta, or on Windows with nested screenshot folders). Cypress now falls back to deleting these assets directly when the operating system cannot move them to the trash, so the cleanup succeeds and the warning is only shown if the assets genuinely cannot be removed. Fixes [#2876](https://github.com/cypress-io/cypress/issues/2876).
