...
- Fixed an issue where a browser launched from an explicit binary path (for example `--browser /path/to/chrome`) displayed the wrong icon and name in the Test Runner UI and browser selector — commonly seen on cloud VMs and CI where the browser is not in a standard install location. Browsers detected by path are labeled "Custom <Name>" (for example "Custom Chrome"), and the UI now resolves them to the correct browser icon instead of falling back to a generic or unrelated icon. Fixes [#32924](https://github.com/cypress-io/cypress/issues/32924).
