...
- Fixed an issue where [`cy.screenshot()`](https://on.cypress.io/screenshot) could capture a clipped (for example 1px tall), blank, or mis-stitched image on pages that use CSS `scroll-behavior: smooth`. Cypress now forces instant scrolling while capturing element and full-page screenshots, so the image is taken after the page has finished scrolling, then restores the original `scroll-behavior`. Fixes [#28674](https://github.com/cypress-io/cypress/issues/28674).
