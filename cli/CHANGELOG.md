...
- Cypress's jQuery has been updated to now return the width and height excluding scrollbars. This matches jQuery 3.2.0+ behavior and more closely aligns with the CSS box model. `cy.scrollTo()` will now take these new calculations into account when scrolling and will scroll to a more accurate position. Also calls to`Cypress.$().height()` or `Cypress.$().width()` may return different, more correct values due to this change. Addresses [#30345](https://github.com/cypress-io/cypress/pull/30345).
