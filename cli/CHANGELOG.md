...
- [`blockHosts`](https://on.cypress.io/configuration#blockHosts) can no longer be set via [`Cypress.config()`](https://on.cypress.io/config) during test execution. Mutating it this way affected the *next* test rather than the current one. Set `blockHosts` in the test configuration of a `describe`/`context` or `it` block instead. Addressed in [#34553](https://github.com/cypress-io/cypress/pull/34553).
...
- The `experimentalMemoryManagement` configuration option has graduated out of experimental status and been replaced by [`manageBrowserMemory`](https://docs.cypress.io/app/references/configuration), which defaults to `true`. Memory management is now enabled by default in Chromium-based browsers. Setting `experimentalMemoryManagement` prints a warning and is otherwise ignored — if you previously set `experimentalMemoryManagement: false` to opt out, you must now set `manageBrowserMemory: false`. Addresses [#34385](https://github.com/cypress-io/cypress/issues/34385).
- Chrome, Chromium, and Edge intercept test traffic on the native browser network, so your application negotiates HTTP/2 or HTTP/3 with your server as it does in production, rather than being downgraded to HTTP/1.1. Firefox, WebKit, and Electron continue to use the legacy network path. Some behaviors differ on the native path — notably `req.httpVersion` in [`cy.intercept()`](https://on.cypress.io/intercept) is no longer reported, and `res.statusMessage` is an empty string for an HTTP/2 or HTTP/3 response, since those protocols carry no reason phrase on the wire — and [`forceHttp1`](https://docs.cypress.io/app/references/configuration#forceHttp1) routes every browser back through the legacy path while you migrate. See [Native network interception](https://docs.cypress.io/app/guides/native-network-interception) and the [Migration Guide](https://on.cypress.io/migration-guide). Addresses [#3708](https://github.com/cypress-io/cypress/issues/3708).
