...
- [`cy.reload()`](https://on.cypress.io/reload) now accepts `onBeforeLoad` and `onLoad` callbacks in its options object, matching the callbacks already available on [`cy.visit()`](https://on.cypress.io/visit). This makes it possible to stub or inspect the reloaded page's `window` object before and after it loads. Addresses [#1895](https://github.com/cypress-io/cypress/issues/1895).
